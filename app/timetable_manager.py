...
<div><button class='tt-btn' type='submit' onclick="this.disabled=true;this.innerHTML='⏳ Generating…';this.form.querySelectorAll('select,input').forEach(function(el){el.disabled=true;});">🚀 Generate</button><div class='tt-muted' style='margin-top:8px'>Generation may take a few seconds while the timetable solver checks teacher, class, room and period conflicts.</div></div></form></div>
