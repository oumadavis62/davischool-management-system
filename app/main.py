...
app = FastAPI()
app.add_middleware(SessionMiddleware, secret_key="davischool-v23-3-fixed-prompts")
...
def dashboard(request: Request):
    ...
    con = get_db(); cur = con.cursor(); cur.execute("SELECT COUNT(*) as c FROM schools"); total = cur.fetchone()["c"]
    ...
    rows = ""
    for s in recent: rows += f"<tr><td style='padding:12px; border-bottom:1px solid #f1f5f9'>🏫 {s['name']}</td><td style='padding:12px; border-bottom:1px solid #f1f5f9'>📍 {s['location']}</td><td style='padding:12px; border-bottom:1px solid #f1f5f9'>✅ Active</td><td style='padding:12px; border-bottom:1px solid #f1f5f9'>Today</td></tr>"
    if not rows: rows = "<tr><td colspan=4 style='padding:20px; text-align:center; color:#999'>No schools yet</td></tr>"
    content = f"<div style='padding:24px'><h2>📊 School Overview</h2><p>Welcome {name}</p><div style='display:grid; grid-template-columns:repeat(4,1fr); gap:16px; margin:20px 0'><div style='background:white; border:1px solid #e2e8f0; border-radius:14px; padding:18px'>🏫 TOTAL {total}</div><div style='background:white; border:1px solid #e2e8f0; border-radius:14px; padding:18px'>✅ ACTIVE {total}</div><div style='background:white; border:1px solid #e2e8f0; border-radius:14px; padding:18px'>💰 KES {total*15000:,}</div><div style='background:white; border:1px solid #e2e8f0; border-radius:14px; padding:18px'>🎓 {total*350:,}</div></div><div style='background:white; border:1px solid #e2e8f0; border-radius:14px; overflow:hidden'><div style='padding:16px; border-bottom:1px solid #f1f5f9; display:flex; justify-content:space-between'><b>🏫 Recently Added</b><a href='/schools/manage'>View All →</a></div><table style='width:100%; border-collapse:collapse'><tr style='background:#f8fafc; font-size:11px; color:#64748b'><th style='padding:10px; text-align:left'>Name</th><th>Location</th><th>Status</th><th>Date</th></tr>{rows}</table></div></div>"
...
def school_dashboard(request: Request):
    ...
    return HTMLResponse(f"<html><body style='font-family:Arial; margin:0; background:#f8fafc'><div style='background:white; padding:10px 20px'><b>🏫 {school_obj['name']}</b></div><div style='padding:24px'><h2>Welcome {school_obj['name']}!</h2><a href='/logout'>Logout</a></div></body></html>")
...
def profile(request: Request, tab: str = "personal"):
    ...
    hdr = header_html(initials, name, email) if is_super else f"<div style='background:white; border-bottom:1px solid #e2e8f0; padding:10px 20px'><b>🏫 {sname}</b></div>"
    ...
    if tab=="security":
        ...
        log_rows = "".join([f"<div style='padding:12px 14px; border-bottom:1px solid #f1f5f9; display:flex; justify-content:space-between'><div><div style='font-size:12px; font-weight:600'>{l['action']}</div><div style='font-size:11px; color:#64748b'>{l['details']}</div></div><span style='font-size:10px; color:#94a3b8'>{l['timestamp']}</span></div>" for l in logs]) or "<div style='padding:24px; text-align:center; color:#999'>No activity</div>"
...
def manage_schools(request: Request, show: str = "", success: str = "", pending_id: str = "", new_pass: str = "", school_email: str = "", school_name: str = ""):
    ...
    if pending_id:
        ...
        if pending:
            code_display = f"<div style='background:white; border:2px dashed #f59e0b; border-radius:10px; padding:14px; margin-top:12px; text-align:center'><div style='font-size:11px; color:#92400e'>🔓 Your code:</div><div style='font-size:32px; font-weight:800; letter-spacing:8px'>🔑 {pending['auth_code']}</div></div>"
            verify_html = f"<div style='background:#fffbeb; border:2px solid #f59e0b; border-radius:12px; padding:20px; margin-bottom:16px'><b>🔐 Enter Code for 🏫 {pending['name']}</b>{code_display}<form method='post' action='/verify-school-code' style='display:flex; gap:8px; margin-top:14px'><input type='hidden' name='pending_id' value='{pending_id}'><input name='auth_code' placeholder='🔢 Enter 6-digit' required style='flex:1; padding:12px; border:1px solid #fcd34d; border-radius:8px; font-size:18px; text-align:center'><button style='background:#0f172a; color:white; padding:12px 20px; border:none; border-radius:8px'>✅ Verify</button></form><a href='/schools/manage' style='font-size:11px'>❌ Cancel</a></div>"
    return HTMLResponse(f"""
    <html><head><meta name='viewport' content='width=device-width, initial-scale=1'><style>.no-caret{{caret-color:transparent; user-select:none; outline:none;}} input,textarea{{caret-color:auto!important; user-select:text!important;}}</style></head><body style='font-family:Arial; background:#f8fafc; margin:0'>{header_html(initials, name, email)}
    <div style='padding:20px; display:grid; grid-template-columns:1fr 380px; gap:16px; align-items:start'>
        <div style='background:white; border:1px solid #e2e8f0; border-radius:12px; padding:16px; overflow-x:auto'>{success_banner}{verify_html}
            <div style='display:flex; justify-content:space-between; margin-bottom:12px'><div><b>📚 Registered Schools ({len(schools)})</b><div style='font-size:11px; color:#64748b'>👆 Click row to highlight 💙 → then ✏️ Edit / 🗑️ Delete becomes active</div></div></div>
    ...
    function selectSchool(id){{
        ...
        let eBtn = document.getElementById('edit-'+id);
        let dBtn = document.getElementById('del-'+id);
    ...
    function togglePwd(id){{
        let dot=document.getElementById('pwd-dot-'+id); let real=document.getElementById('pwd-real-'+id);
        if(dot.style.display==='none'){{ dot.style.display='inline'; real.style.display='none'; }}
        else {{ dot.style.display='none'; real.style.display='inline'; }}
    }}
