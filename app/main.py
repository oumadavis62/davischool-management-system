...
app = FastAPI()
app.add_middleware(SessionMiddleware, secret_key="davischool-v51-overview-fixed-staff-add")
...
def staff_manager_html(teachers, school_name, is_global=False):
    ...
    for i, t in enumerate(teachers, 1):
        # FIXED: no leading space in href/action
 ...
 .modal.active{{display:flex}}
    ...
    <div id='addStaffModal' class='modal'>
      <div style='background:white;border-radius:16px;width:520px;max-width:95%;max-height:90vh;overflow:auto'>
        ...
        <form method='post' action='{add_action}' style='padding:20px;display:grid;grid-template-columns:1fr 1fr;gap:12px'>
          ...
          <div style='grid-column:span 2;margin-top:8px'><button class='add-btn'>➕ Add Staff</button></div>
        </form>
      </div>
    </div>
    <script>
    ...
    function filterStaff(){{
      ...
      rows.forEach(r=>{{
        ...
        if(show){{ visible++; r.style.display=visible<=perPage?'':'none'; }} else {{ r.style.display='none'; }}
      }});
    }}
    function exportCSV(){{
...
def global_add_teacher(request: Request, name: str = Form(...), tsc_no: str = Form(""), id_no: str = Form(...), gender: str = Form(...), role: str = Form(...), phone: str = Form(...), email: str = Form(""), employment_type: str = Form("Teaching")):
    ...
    con.commit(); con.close()
    return RedirectResponse("/super/global-control/teachers",303)
