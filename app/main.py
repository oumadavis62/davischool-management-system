...
app = FastAPI()
app.add_middleware(SessionMiddleware, secret_key="davischool-v44-photocopy-final")
...
# ===== GLOBAL CONTROL — PHOTOCOPY OF SCHOOL SIDE =====
...
def global_control(request: Request):
    ...
    # Global tables like school side
    ...
    <div style='width:260px;background:white;border-right:1px solid #e2e8f0;padding:16px;position:sticky;top:0;height:100vh;overflow-y:auto'>
      <div style='padding:10px 6px 16px;border-bottom:1px solid #f1f5f9;margin-bottom:12px'>
        <div style='display:flex;align-items:center;gap:10px'>
          ...
          <div><b style='font-size:13px'>GLOBAL CONTROL</b><div style='font-size:10px;color:#64748b'>🔑 ALL | ALL SCHOOLS</div></div>
        </div>
      </div>
      {g_nav('dashboard','📊','Dashboard', active=True)}
      {g_nav('students','🎓','Students')}
      {g_nav('classes','🏫','Classes & Streams')}
      {g_nav('subjects','📚','Subjects')}
      <div style='margin-bottom:4px'>
        ...
        <div id='academicDropdown' style='display:block;margin-left:8px;border-left:1px solid #e2e8f0;padding-left:10px;margin-bottom:6px'>
          {g_sub_nav('dean-settings','⚙️','Dean Settings')}
          {g_sub_nav('exams','🔧','Exam Settings')}
          {g_sub_nav('marks','📄','Set Marks')}
          {g_sub_nav('subject-allocation','📋','Subject Allocation')}
          {g_sub_nav('marks','✏️','Record Marks')}
          {g_sub_nav('marks','📄','Edit Marks')}
          {g_sub_nav('marksheets','☰','Marks Status')}
          {g_sub_nav('analysis','📊','Exam Analysis')}
          {g_sub_nav('spreadsheet','📄','Spreadsheet')}
          {g_sub_nav('sba','📋','SBA (KNEC CBA)')}
        </div>
      </div>
      {g_nav('ranking','🏆','Ranking')}
      {g_nav('reports','📑','Student Reports')}
      {g_nav('teachers','👨‍🏫','Staff Manager')}
      {g_nav('timetable','🗓️','Smart Timetable')}
      {g_nav('fees','💰','Fees & Finance')}
      {g_nav('sms','💬','Bulk SMS Parents')}
      <div style='margin-top:16px;border-top:1px solid #f1f5f9;padding-top:12px'>
        <a href='/dashboard' style='display:flex;align-items:center;gap:10px;padding:11px 14px;border-radius:10px;text-decoration:none;font-size:13px;color:#0f172a;background:#f8fafc;font-weight:700'>⬅️ Back to Super Admin</a>
...
.push-btn{{width:100%;background:#0f172a;color:white;padding:12px;border:none;border-radius:10px;font-weight:800;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px}}
...
<div style='background:white;border-bottom:1px solid #e2e8f0;padding:12px 20px;display:flex;justify-content:space-between;align-items:center'>
  <div><b style='font-size:13px'>GLOBAL CONTROL (Code: ALL) — Photocopy of School Side</b><div style='font-size:10px;color:#64748b'>🌍 {total_schools} schools • Edit once → updates ALL — No impersonation needed</div></div>
  <div style='display:flex;align-items:center;gap:12px'><div style='width:28px;height:28px;background:#dbeafe;color:#1e40af;border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:800;font-size:11px'>{initials}</div><div style='font-size:11px'><b>{name}</b> <span style="background:#0f172a;color:white;padding:2px 6px;border-radius:6px;font-size:9px">SUPER</span></div></div>
</div>
...
<!-- PHOTOCOPY OF SCHOOL DASHBOARD OVERVIEW -->
<div style='background:linear-gradient(135deg,#0f172a 0%, #1e3a8a 60%, #1e40af 100%); border-radius:18px; padding:22px 24px; color:white; display:flex; justify-content:space-between; align-items:center; margin-bottom:16px'><div><div style='font-size:22px; font-weight:900'>DaviSchool Management System 🚀 — GLOBAL</div><div style='font-size:12px; color:#bfdbfe; margin-top:4px'>REVOLUTIONIZE ALL SCHOOLS MANAGEMENT! — Same as School Overview</div></div><div style='text-align:right'><div style='font-size:34px; font-weight:900'>{total_students}</div><div style='font-size:11px'>Total Students (ALL Schools)</div></div></div>

<div style='display:grid; grid-template-columns:repeat(4,1fr); gap:14px; margin-bottom:14px'>
<a class='ds-card'><div style='font-size:11px; color:#64748b; font-weight:700'>🎓 TOTAL STUDENTS (ALL)</div><div style='font-size:30px; font-weight:900; margin:10px 0'>{total_students}</div><div style='font-size:10px;color:#16a34a'>Across {total_schools} schools</div></a>
<a class='ds-card'><div style='font-size:11px; color:#64748b; font-weight:700'>🏫 CLASSES (ALL)</div><div style='font-size:30px; font-weight:900; margin:10px 0'>{total_classes}</div><div style='font-size:10px;color:#64748b'>Avg {total_classes//total_schools if total_schools else 0} per school</div></a>
<a class='ds-card'><div style='font-size:11px; color:#64748b; font-weight:700'>📝 EXAMS (ALL)</div><div style='font-size:30px; font-weight:900; margin:10px 0'>{total_exams}</div><div style='font-size:10px;color:#64748b'>Global exams</div></a>
<a class='ds-card'><div style='font-size:11px; color:#64748b; font-weight:700'>👨‍🏫 STAFF (ALL)</div><div style='font-size:30px; font-weight:900; margin:10px 0'>{total_teachers}</div><div style='font-size:10px;color:#64748b'>Across all schools</div></a>
</div>

<div style='background:white; border:1px solid #e2e8f0; border-radius:14px; padding:16px; margin-bottom:16px'><div style='display:flex; justify-content:space-between'><div><div style='font-weight:800; font-size:14px'>👥 Students by Gender — ALL Schools</div><div style='font-size:11px; color:#64748b'>Boys vs Girls enrollment per form — auto updates — Same as School Side Chart</div></div><div style='display:flex; gap:12px; font-size:11px'><span><span style='width:10px;height:10px;background:#0a84ff;display:inline-block'></span> Boys</span><span><span style='width:10px;height:10px;background:#ff2d92;display:inline-block'></span> Girls</span></div></div><div style='display:flex; gap:24px; overflow-x:auto; margin-top:18px'>{chart_html}</div><div style='display:grid; grid-template-columns:repeat(4,1fr); gap:12px; margin-top:18px; border-top:1px solid #f1f5f9; padding-top:14px'><div style='background:#f0f9ff; padding:12px; border-radius:10px; text-align:center'><div style='font-size:11px'>Total Boys (ALL)</div><div style='font-size:20px; font-weight:900; color:#0a84ff'>{tb}</div></div><div style='background:#fdf2f8; padding:12px; border-radius:10px; text-align:center'><div style='font-size:11px'>Total Girls (ALL)</div><div style='font-size:20px; font-weight:900; color:#ff2d92'>{tg}</div></div><div style='background:#f8fafc; padding:12px; border-radius:10px; text-align:center'><div style='font-size:11px'>Total (ALL)</div><div style='font-size:20px; font-weight:900'>{total}</div></div><div style='background:#f0fdf4; padding:12px; border-radius:10px; text-align:center'><div style='font-size:11px'>Ratio (ALL)</div><div style='font-size:20px; font-weight:900'>{ratio}:1</div></div></div></div>

<div style='display:grid; grid-template-columns:1.9fr 0.8fr; gap:14px; margin-bottom:16px'>
<div style='background:white; border:1px solid #e2e8f0; border-radius:14px; overflow:hidden'><div style='padding:14px 16px; border-bottom:1px solid #f1f5f9; display:flex; justify-content:space-between'><div style='font-weight:800'>🎓 Recent Students — ALL Schools — Same as School Side</div><a href='/schools/manage' style='font-size:11px; color:#3b82f6; text-decoration:none'>View All Schools →</a></div><table style='width:100%; border-collapse:collapse'><thead><tr style='background:#f8fafc; text-align:left; font-size:10px; color:#64748b'><th style='padding:10px 12px'>Name (School)</th><th>Adm No</th><th>Gender</th><th>Class</th></tr></thead><tbody>{stu_rows}</tbody></table></div>
<div style='background:#0f172a; border-radius:14px; padding:16px; color:white; height:fit-content'><div style='font-weight:800; font-size:14px'>📊 GLOBAL LIVE</div><div style='background:#1e293b; border-radius:10px; padding:12px; margin-top:10px'><div style='font-size:11px'>🏫 {total_schools} Schools</div><div style='font-size:11px'>👦 {tb} | 👧 {tg}</div><div style='font-size:11px; margin-top:6px; color:#22c55e'>Auto ✅ Global</div></div><a href='/schools/manage' style='display:block;margin-top:12px;text-align:center;background:white;color:#0f172a;padding:10px;border-radius:10px;text-decoration:none;font-weight:700;font-size:12px'>🏫 Manage Schools</a></div>
</div>

<!-- GLOBAL CONTROL SECTIONS — SAME AS SCHOOL SIDE WINDOWS WITH PUSH BUTTON -->
<div id='classes' style='display:grid; grid-template-columns:1.7fr 0.7fr; gap:16px; margin-bottom:16px'>
  <div style='background:white; border:1px solid #e2e8f0; border-radius:14px; overflow:hidden'><div style='padding:14px 16px; border-bottom:1px solid #f1f5f9; display:flex; justify-content:space-between; align-items:center'><b>🏫 Global Classes & Streams — Same as School Classes Window</b><span style='background:#0f172a;color:white;padding:4px 10px;border-radius:12px;font-size:10px'>🌍 Push to ALL</span></div><div style='overflow:auto; max-height:280px'><table style='width:100%; border-collapse:collapse'><thead><tr style='background:#f8fafc; text-align:left; font-size:11px; color:#64748b'><th style='padding:10px 12px'>CLASS</th><th style='padding:10px 12px'>STREAM</th><th style='padding:10px 12px'>SCHOOLS</th></tr></thead><tbody>{class_rows}</tbody></table></div></div>
...
<div id='subjects' style='display:grid; grid-template-columns:1.7fr 0.7fr; gap:16px; margin-bottom:16px'>
  <div style='background:white; border:1px solid #e2e8f0; border-radius:14px; overflow:hidden'><div style='padding:14px 16px; border-bottom:1px solid #f1f5f9; display:flex; justify-content:space-between'><b>📚 Global Subjects — Same as School Subjects Window</b><span style='background:#0f172a;color:white;padding:4px 10px;border-radius:12px;font-size:10px'>🌍 Push to ALL</span></div><div style='overflow:auto; max-height:280px'><table style='width:100%; border-collapse:collapse'><thead><tr style='background:#f8fafc; text-align:left; font-size:11px; color:#64748b'><th style='padding:10px 12px'>NAME</th><th style='padding:10px 12px'>CODE</th><th style='padding:10px 12px'>INITIAL</th><th style='padding:10px 12px'>SCHOOLS</th></tr></thead><tbody>{subj_rows}</tbody></table></div></div>
  <div style='background:white; border:1px solid #e2e8f0; border-radius:14px; padding:16px; height:fit-content'><div style='font-weight:800; margin-bottom:10px'>➕ Add Global Subject</div><form method='post' action='/super/global-control/add-subject'><input name='subject_name' required placeholder='Subject e.g. MATHEMATICS' class='input-field'><input name='code' placeholder='Code MAT' class='input-field'><input name='initial' placeholder='Initial M' class='input-field'><button class='push-btn'>🌍 Push to ALL {total_schools} Schools</button></form></div>
</div>

<div id='exams' style='display:grid; grid-template-columns:1fr 1fr; gap:16px; margin-bottom:16px'>
  <div style='background:white; border:1px solid #e2e8f0; border-radius:14px; padding:16px'><div style='font-weight:800; margin-bottom:10px'>⚙️ Global Dean Settings — Same as School Dean Settings</div><form method='post' action='/super/global-control/add-term'><div style='display:grid; grid-template-columns:1fr 1fr; gap:8px'><select name='term_name' required class='input-field'><option>Term 1</option><option>Term 2</option><option>Term 3</option></select><input name='year' required placeholder='2026' class='input-field'></div><div style='display:grid; grid-template-columns:1fr 1fr; gap:8px'><input name='start_date' type='date' required class='input-field'><input name='end_date' type='date' required class='input-field'></div><button class='push-btn'>🌍 Push Term to ALL</button></form></div>
  <div style='background:white; border:1px solid #e2e8f0; border-radius:14px; padding:16px'><div style='font-weight:800; margin-bottom:10px'>🔧 Global Exam Settings — Same as School Exam Settings</div><form method='post' action='/super/global-control/add-exam'><input name='exam_name' required placeholder='Exam e.g. END TERM' class='input-field'><div style='display:grid; grid-template-columns:1fr 1fr; gap:8px'><select name='term' required class='input-field'><option>Term 1</option><option>Term 2</option><option>Term 3</option></select><input name='year' required placeholder='2026' class='input-field'></div><select name='exam_type' required class='input-field'><option>Main Exam</option><option>End Term Exam</option></select><button class='push-btn'>🌍 Push Exam to ALL</button></form></div>
</div>

<div id='broadcast' style='display:grid; grid-template-columns:1.7fr 0.7fr; gap:16px'>
  <div style='background:white; border:1px solid #e2e8f0; border-radius:14px; padding:16px'><div style='font-weight:800; margin-bottom:10px'>📢 Broadcast Notice to ALL Schools — Shows on Every School Dashboard Banner</div><form method='post' action='/super/global-control/add-notice'><input name='message' required placeholder='Type notice e.g. System maintenance tonight 10pm — will show on every school dashboard' class='input-field'><button class='push-btn'>📢 Broadcast Notice to ALL {total_schools} Schools</button></form><div style='margin-top:12px; border:1px solid #f1f5f9; border-radius:10px; padding:0 12px; max-height:200px; overflow:auto'>{"".join([f"<div style='padding:10px 0;border-bottom:1px solid #f1f5f9;display:flex;justify-content:space-between'><div><b style='font-size:12px'>📢 {n['message']}</b><div style='font-size:10px;color:#64748b'>{n['created_at']}</div></div><a href='/super/global-control/delete-notice/{n['id']}' style='color:#dc2626;text-decoration:none'>🗑️</a></div>" for n in notices]) or "<div style='padding:20px;text-align:center;color:#94a3b8'>No notices yet</div>"}</div></div>
  <div style='background:#0f172a; border-radius:14px; padding:16px; color:white; height:fit-content'><div style='font-weight:800'>✅ Photocopy Complete</div><div style='font-size:11px; color:#94a3b8; margin-top:8px; line-height:1.6'>• Sidebar = same as school side<br>• Dashboard Overview = same<br>• Gender Chart = same<br>• Recent Students = same<br>• Classes/Subjects windows = same<br>• Each has 🌍 Push to ALL button<br>• Broadcast shows as banner on all schools</div><a href='/dashboard' style='display:block; margin-top:12px; text-align:center; background:white; color:#0f172a; padding:10px; border-radius:10px; text-decoration:none; font-weight:700'>⬅️ Back to Super Admin Overview</a></div>
</div>
