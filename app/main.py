...
def dashboard(request: Request):
    ...
    con = get_db(); cur = con.cursor(); cur.execute("SELECT COUNT(*) as c FROM schools"); total = cur.fetchone()["c"]; cur.execute("SELECT COUNT(*) as c FROM schools WHERE lower(COALESCE(status,'active')) IN ('active','enabled')"); active_total = cur.fetchone()["c"]; cur.execute("SELECT * FROM schools ORDER BY id DESC LIMIT 10"); recent = cur.fetchall(); con.close()
