...
app = FastAPI()
app.add_middleware(SessionMiddleware, secret_key="davischool-v38-final-fixed")
...
def profile_page(request: Request, tab: str = "personal"):
    ...
    if tab == "personal":
        ...
        log_rows = "".join([f"""<div style='padding:12px 0; border-bottom:1px solid #f1f5f9; display:flex; justify-content:space-between'><div><div style='font-weight:700; font-size:12px'>{l['action']}</div><div style='font-size:11px; color:#64748b'>{l['details']}</div></div><div style='font-size:10px; color:#64748b'>{l['timestamp']}</div></div>""" for l in logs]) or "<div style='padding:30px; text-align:center; color:#94a3b8'>No activity yet</div>"
...
# === WELCOME SCREEN - LOGO ON TOP + CENTERED + DAVISCHOOL + ASK DAVIS ===
...
.card{background:white;width:540px;max-width:100%;padding:48px 48px 40px;border-radius:4px;box-shadow:0 0 0 1px #e2e8f0;text-align:center}
.logo-box{width:64px;height:64px;background:#0f172a;color:white;border-radius:16px;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:28px;margin:0 auto}
...
<div class="main">
  <div class="card">
    ...
    <h1 style="margin:14px 0 0;font-size:38px;font-weight:900;color:#0f172a;letter-spacing:-0.5px">DaviSchool</h1>
    <div style="margin-top:10px;color:#334155;font-size:15px">Sign in to your Davischool account</div>
    <form method="post" action="/login" style="margin-top:28px;text-align:left">
...
# === RESTORED ADMIN OVERVIEW FULL v38.6 ===
...
def school_other(path: str, request: Request):
    ...
    return HTMLResponse(f"<html><body>{header}<div style='padding:30px'><div style='background:white;border:1px solid #e2e8f0;border-radius:14px;padding:40px;text-align:center'><h3>🚧 {path.upper()} — placeholder</h3><a href='/school/dashboard' style='padding:10px 16px; background:white; border:1px solid #e2e8f0; border-radius:10px; text-decoration:none; color:#0f172a; font-weight:700'>⬅️ Back</a></div></div></div></div></body></html>")
