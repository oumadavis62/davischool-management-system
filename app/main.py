...
app = FastAPI()
app.add_middleware(SessionMiddleware, secret_key="davischool-emojis-back-email-v17")
...
def send_email(to_email, subject, body):
    ...
    try:
        ...
        msg.add_alternative(f"<div style='font-family:Arial; padding:20px'><h3>{subject}</h3><div style='background:#f8fafc; border:1px solid #e2e8f0; padding:16px; border-radius:8px; white-space:pre-wrap'>{body}</div><p style='font-size:11px; color:#64748b; margin-top:16px'>🏫 Davischool Platform - {datetime.now(ZoneInfo('Africa/Nairobi')).strftime('%Y-%m-%d %I:%M %p')} EAT</p></div>", subtype="html")
...
def profile(request: Request, tab: str = "personal"):
    ...
    if tab=="security":
        ...
        right = f"""
        <div><b style='font-size:15px'>👤 Personal Information</b><p style='font-size:11px; color:#64748b; margin-top:4px'>Update your personal details and contact information</p>
        <form method='post' action='/update-profile' style='margin-top:18px'>
            ...
            <div style='text-align:right; margin-top:20px'><button type='submit' style='background:#0f172a; color:white; padding:11px 18px; border:none; border-radius:8px; cursor:pointer; font-weight:600'>💾 Save Changes</button></div>
        </form>
        </div>
        """
            ...
            <div style='background:white; border:1px solid #e2e8f0; border-radius:14px; padding:24px; text-align:center; position:sticky; top:20px'>
                ...
                <div style='margin-top:16px; border-top:1px solid #f1f5f9; padding-top:16px; text-align:left'>
                    ...
                    <div style='margin-top:14px; display:flex; flex-direction:column; gap:8px'>
                        <div style='display:flex; justify-content:space-between; font-size:11px'><span style='color:#64748b'>📊 Status</span><span style='color:#16a34a; font-weight:600'>● Active</span></div>
                        <div style='display:flex; justify-content:space-between; font-size:11px'><span style='color:#64748b'>👑 Role</span><span style='font-weight:600'>{badge}</span></div>
                    </div>
                </div>
            </div>
...
def update_password(request: Request, current_password: str = Form(...), new_password: str = Form(...), confirm_password: str = Form(...)):
    ...
    return HTMLResponse("<h3>✅ Password Updated Successfully! 🔐</h3><p>Your password has been changed.</p><a href='/profile?tab=security'>⬅️ Back to Security</a> | <a href='/dashboard'>📊 Dashboard</a>")
...
def manage_schools(request: Request, show: str = "", success: str = "", pending_id: str = ""):
    ...
    if success=="added":
        ...
        success_banner = "<div style='background:#e0f2fe; border:1px solid #7dd3fc; color:#0c4a6e; padding:12px 16px; border-radius:10px; margin-bottom:16px'>✅ <b>Updated! ✏️</b> School updated — Activity Log updated.</div>"
    elif success=="deleted":
        success_banner = "<div style='background:#fee2e2; border:1px solid #fca5a5; color:#991b1b; padding:12px 16px; border-radius:10px; margin-bottom:16px'>🗑️ <b>Deleted! 🗑️</b> School removed — Activity Log updated.</div>"
    rows_html = ""
    schools_json = {}
    for s in schools:
        sid = s["id"]
...
def register_school(school_name: str = Form(...), school_email: str = Form(...), location: str = Form(...), phone: str = Form(...), principal: str = Form(...), school_type: str = Form(...), request: Request = None):
    ...
    con.commit()
    con.close()
    send_email(SUPER_ADMIN, f"🔐 Authorization Code: {auth_code} - Add School {school_name}", f"Hello Davis,\n\n🏫 School: {school_name.upper()}\n📧 Email: {school_email}\n📍 Location: {location}\n📱 Phone: {phone}\n👨‍💼 Principal: {principal}\n🎓 Type: {school_type}\n🕒 Time: {ts} EAT\n\n🔑 YOUR AUTHORIZATION CODE IS: {auth_code}\n\nEnter this 6-digit code in Davischool to confirm and add the school.\n\n🏫 Davischool Security System")
...
def verify_school_code(pending_id: str = Form(...), auth_code: str = Form(...)):
    ...
    if pending["auth_code"]!= auth_code.strip():
        con.close()
        return HTMLResponse(f"<html><body style='font-family:Arial; padding:40px; text-align:center'><h3>❌ Wrong Code! 🔐</h3><p>Code sent to {SUPER_ADMIN} is <b>{pending['auth_code']}</b></p><a href='/schools/manage?pending_id={pending_id}'>🔄 Try Again</a></body></html>")
    ...
    con.commit()
    con.close()
    send_email(SUPER_ADMIN, f"✅ School Added: {pending['name']} - Code {code} 🏫", f"✅ School successfully authorized and added!\n\n🏫 {pending['name']}\n🔑 School Code: {code}\n📧 Login: {pending['email']}\n🔒 Default Password: School@2026!\n")
...
def update_school(school_id: int, school_name: str = Form(...), school_email: str = Form(...), location: str = Form(...), phone: str = Form(...), principal: str = Form(...), school_type: str = Form(...), request: Request = None):
    ...
    log_activity(email, f"✏️ Edited school: {school_name} ✏️", f"From {old_name} to {school_name} | {school_type}")
...
def delete_school(school_id: int, request: Request):
    ...
    con.close()
    email = request.session.get("email","")
    log_activity(email, f"🗑️ Deleted school: {sname} 🗑️", f"Code: {scode} removed")
