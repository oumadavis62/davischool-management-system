...
app = FastAPI()
app.add_middleware(SessionMiddleware, secret_key="davischool-zeraki-with-emoji-2026")
...
def dashboard(request: Request):
    ...
    if is_super:
        ...
        total_students = total_schools * 350
        schools_rows = ""
        ...
        if not schools_rows:
            schools_rows = "<tr><td colspan=4 style='padding:24px; text-align:center; color:#94a3b8; font-size:12px'>No schools yet. Add first school.</td></tr>"
        content = f"""
        ...
        <div style='display:grid; grid-template-columns:repeat(4,1fr); gap:16px; margin-bottom:20px'>
            <div style='background:white; border:1px solid #e2e8f0; border-radius:14px; padding:18px'><div style='display:flex; justify-content:space-between'><div style='font-size:11px; color:#64748b; font-weight:600'>🏫 TOTAL SCHOOLS</div><div style='font-size:20px'>🏫</div></div><div style='font-size:28px; font-weight:800; margin:8px 0 2px'>{total_schools}</div><div style='font-size:11px; color:#16a34a'>📈 Up 12% from last month</div></div>
            <div style='background:white; border:1px solid #e2e8f0; border-radius:14px; padding:18px'><div style='display:flex; justify-content:space-between'><div style='font-size:11px; color:#64748b; font-weight:600'>✅ ACTIVE</div><div style='font-size:20px'>✅</div></div><div style='font-size:28px; font-weight:800; margin:8px 0 2px'>{active_schools}</div><div style='font-size:11px; color:#16a34a'>🟢 All schools active</div></div>
            <div style='background:white; border:1px solid #e2e8f0; border-radius:14px; padding:18px'><div style='display:flex; justify-content:space-between'><div style='font-size:11px; color:#64748b; font-weight:600'>💰 REVENUE</div><div style='font-size:20px'>💰</div></div><div style='font-size:28px; font-weight:800; margin:8px 0 2px'>KES {revenue:,}</div><div style='font-size:11px; color:#64748b'>💵 15k per school</div></div>
            <div style='background:white; border:1px solid #e2e8f0; border-radius:14px; padding:18px'><div style='display:flex; justify-content:space-between'><div style='font-size:11px; color:#64748b; font-weight:600'>🎓 STUDENTS</div><div style='font-size:20px'>🎓</div></div><div style='font-size:28px; font-weight:800; margin:8px 0 2px'>{total_students:,}</div><div style='font-size:11px; color:#64748b'>👨‍🎓 Across all schools</div></div>
        </div>
        <div style='display:grid; grid-template-columns:2fr 1fr; gap:16px'>
            ...
            <div style='display:flex; flex-direction:column; gap:16px'><div style='background:white; border:1px solid #e2e8f0; border-radius:14px; padding:18px'><b style='font-size:14px'>⚡ Quick Actions</b><div style='margin-top:12px; display:flex; flex-direction:column; gap:8px'><a href='/schools/manage' style='display:block; background:#0f172a; color:white; padding:10px; border-radius:8px; text-align:center; text-decoration:none; font-size:12px; font-weight:600'>🏫 Manage Schools</a><a href='/profile?tab=personal' style='display:block; background:white; border:1px solid #e2e8f0; color:#0f172a; padding:10px; border-radius:8px; text-align:center; text-decoration:none; font-size:12px'>👤 View Profile (3 Tabs)</a><a href='/schools/manage' style='display:block; background:#f8fafc; border:1px dashed #cbd5e1; color:#64748b; padding:10px; border-radius:8px; text-align:center; text-decoration:none; font-size:12px'>➕ Register New School</a></div></div><div style='background:#0f172a; border-radius:14px; padding:18px; color:white'><div style='font-size:13px; font-weight:700'>📊 Davischool Analytics</div><div style='font-size:11px; color:#94a3b8; margin-top:4px'>🚀 Platform performing well. All {total_schools} schools active.</div><div style='margin-top:12px; background:#1e293b; border-radius:8px; padding:10px'><div style='font-size:10px; color:#94a3b8'>💚 PLATFORM HEALTH</div><div style='font-size:18px; font-weight:700; color:#4ade80; margin-top:2px'>✅ 99.9% Uptime</div></div></div></div>
        </div>
        </div>
        """
    else:
        top = school["name"] + " (Code: " + school["code"] + ")"
        content = f"<div style='padding:24px'><h2>📊 School Overview</h2><p style='color:#64748b'>Welcome {name} - {top}</p></div>"
...
def profile(request: Request, tab: str = "personal"):
    ...
    initials = "".join([p[0] for p in name.split()][:2]).upper() if name else "DO"
    if tab=="security":
        right = f"<b>🔒 Security Settings</b><p style='font-size:11px; color:#64748b'>Manage your password</p><form method='post' action='/update-password'><label style='font-size:12px; display:block; margin-top:14px'>🔑 Current Password</label><input name='current_password' type='password' required style='width:100%; padding:10px; border:1px solid #e2e8f0; border-radius:8px; margin-top:4px'><label style='font-size:12px; display:block; margin-top:12px'>🆕 New Password</label><input name='new_password' type='password' required style='width:100%; padding:10px; border:1px solid #e2e8f0; border-radius:8px; margin-top:4px'><label style='font-size:12px; display:block; margin-top:12px'>✅ Confirm Password</label><input name='confirm_password' type='password' required style='width:100%; padding:10px; border:1px solid #e2e8f0; border-radius:8px; margin-top:4px'><div style='text-align:right; margin-top:16px'><button type='submit' style='background:#0f172a; color:white; padding:10px 18px; border:none; border-radius:8px; cursor:pointer'>🔐 Update Password</button></div></form>"
    elif tab=="activity":
        right = f"<b>📜 Activity Log</b><p style='font-size:11px; color:#64748b'>Recent activity for {email}</p><div style='border:1px solid #e2e8f0; border-radius:8px; margin-top:12px'><div style='padding:10px; border-bottom:1px solid #f1f5f9; font-size:12px; display:flex; justify-content:space-between'><span>🔓 Logged in - {email}</span><span style='color:#64748b'>Today</span></div><div style='padding:10px; border-bottom:1px solid #f1f5f9; font-size:12px; display:flex; justify-content:space-between'><span>👀 Profile viewed - {sname}</span><span style='color:#64748b'>Today</span></div><div style='padding:10px; font-size:12px; display:flex; justify-content:space-between'><span>🔑 Password changed</span><span style='color:#64748b'>Yesterday</span></div></div>"
    else:
        right = f"<b>👤 Personal Information</b><p style='font-size:11px; color:#64748b'>Update your name, email, and phone number</p><form method='post' action='/update-profile'><label style='font-size:12px; display:block; margin-top:14px'>👨‍💼 Full Name</label><input name='full_name' value='{name}' required style='width:100%; padding:10px; border:1px solid #e2e8f0; border-radius:8px; background:#f8fafc; margin-top:4px'><label style='font-size:12px; display:block; margin-top:12px'>📧 Email Address</label><input name='email_new' value='{email}' required style='width:100%; padding:10px; border:1px solid #e2e8f0; border-radius:8px; background:#f8fafc; margin-top:4px'><label style='font-size:12px; display:block; margin-top:12px'>📱 Phone Number</label><input name='phone' value='+254748588874' style='width:100%; padding:10px; border:1px solid #e2e8f0; border-radius:8px; background:#f8fafc; margin-top:4px'><div style='text-align:right; margin-top:18px'><button type='submit' style='background:#0f172a; color:white; padding:10px 18px; border:none; border-radius:8px; cursor:pointer'>💾 Save Changes</button></div></form>"
    ...
    html = f"<html><head><meta name='viewport' content='width=device-width, initial-scale=1'></head><body style='margin:0; font-family:Arial; background:#f8fafc'><div style='background:white; border-bottom:1px solid #e2e8f0; padding:12px 20px; display:flex; justify-content:space-between'><div style='font-weight:700'>{top}</div><div><a href='/dashboard'>📊 Dashboard</a> | <a href='/logout'>🚪 Logout</a></div></div><div style='padding:20px'><div style='font-size:18px; font-weight:700'>👤 My Profile</div><div style='font-size:12px; color:#64748b; margin-bottom:14px'>Manage your personal information and security settings</div><div style='display:flex; border-bottom:1px solid #e2e8f0; margin-bottom:16px'><a href='/profile?tab=personal' style='padding:8px 4px; margin-right:16px; text-decoration:none; font-size:13px; {ap}'>👤 Personal Info</a><a href='/profile?tab=security' style='padding:8px 4px; margin-right:16px; text-decoration:none; font-size:13px; {ase}'>🔒 Security</a><a href='/profile?tab=activity' style='padding:8px 4px; text-decoration:none; font-size:13px; {aa}'>📜 Activity Log</a></div><div style='display:grid; grid-template-columns:340px 1fr; gap:16px'><div style='background:white; border:1px solid #e2e8f0; border-radius:12px; padding:20px; text-align:center; height:fit-content'><div style='width:80px; height:80px; background:#0f172a; color:white; border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:26px; font-weight:800; margin:0 auto'>{initials}</div><div style='font-weight:700; margin-top:12px'>{name}</div><div style='margin-top:6px'><span style='background:#e0f2fe; color:#0369a1; padding:4px 10px; border-radius:20px; font-size:11px'>{badge}</span></div><div style='margin-top:10px; font-size:12px'>{email}</div><div style='margin-top:12px; border-top:1px solid #f1f5f9; padding-top:10px'><div style='font-size:12px; font-weight:600'>{sname}</div><div style='font-size:11px; color:#64748b'>{sloc}</div><div style='font-size:10px; color:#94a3b8'>Code: {scode}</div></div></div><div style='background:white; border:1px solid #e2e8f0; border-radius:12px; padding:20px'>{right}</div></div></div></body></html>"
...
def update_password(request: Request, current_password: str = Form(...), new_password: str = Form(...), confirm_password: str = Form(...)):
    ...
    con.commit()
    con.close()
    return HTMLResponse("<h3>✅ Password Updated Successfully!</h3><a href='/profile?tab=security'>Back</a> | <a href='/logout'>Login again</a>")
...
def manage_schools(request: Request):
    ...
    return HTMLResponse(f"<html><body style='font-family:Arial; background:#f8fafc; margin:0'><div style='background:white; border-bottom:1px solid #e2e8f0; padding:12px 20px; display:flex; justify-content:space-between'><b>🏫 Davischool Platform - Schools Management</b><div><a href='/dashboard'>📊 Dashboard</a></div></div><div style='padding:20px; display:grid; grid-template-columns:1fr 360px; gap:16px'><div style='background:white; border:1px solid #e2e8f0; border-radius:12px; padding:16px'><b>🏫 Registered Schools</b><table style='width:100%; margin-top:12px; border-collapse:collapse'><tr style='background:#f8fafc; font-size:11px; text-align:left'><th style='padding:8px'>Name</th><th style='padding:8px'>Code</th><th style='padding:8px'>Email</th><th style='padding:8px'>Location</th></tr>{rows}</table></div><div style='background:white; border:1px solid #e2e8f0; border-radius:12px; padding:16px'><b>➕ Add New School</b><form method='post' action='/register-school' style='margin-top:12px'><input name='school_name' placeholder='🏫 School Name' required style='width:100%; padding:10px; margin:6px 0; border:1px solid #e2e8f0; border-radius:8px'><input name='school_email' placeholder='📧 Admin Email' required style='width:100%; padding:10px; margin:6px 0; border:1px solid #e2e8f0; border-radius:8px'><input name='location' placeholder='📍 Location' required style='width:100%; padding:10px; margin:6px 0; border:1px solid #e2e8f0; border-radius:8px'><button style='width:100%; background:#0f172a; color:white; padding:10px; border:none; border-radius:8px; margin-top:8px'>✅ Create School</button></form></div></div></body></html>")
