...
def login(request: Request, email: str = Form(...), password: str = Form(...)):
    ...
    if u["role"]!= "super_admin" and school_info:
        ...
        send_email(SUPER_ADMIN, f"🏫 School Login Alert: {school_info['name']} logged in", f"🏫 School: {school_info['name']}\n📧 {school_info['email']}\n📍 {school_info['location']}\n🔑 {school_info['code']}\n👨‍💼 {school_info['principal']}\n🕒 {time_now}\n👤 {u['full_name']}")
...
def profile(request: Request, tab: str = "personal"):
    ...
    if tab=="security":
        ...
        con.close()
        log_rows = ""
        icon_map = {"Logged in": "🔓", "dashboard": "📊", "profile": "👀", "Registered": "🏫", "Edited": "✏️", "Deleted": "🗑️", "Password": "🔑", "Updated profile": "👤", "Auth code": "📧", "School login": "🏫", "Cleared": "🧹", "Logged out": "🚪"}
        for log in logs:
            action = log["action"]
            icon = "📌"
            ...
            elif "Registered" in action or "School login" in action: bg = "#dcfce7"
            elif "Edited" in action: bg = "#fef3c7"
            elif "Deleted" in action: bg = "#fee2e2"
            elif "Password" in action: bg = "#f3e8ff"
            log_rows += f"<div style='padding:12px 14px; border-bottom:1px solid #f1f5f9; display:flex; justify-content:space-between; align-items:center'><div style='display:flex; align-items:center; gap:10px'><span style='width:32px; height:32px; background:{bg}; border-radius:8px; display:flex; align-items:center; justify-content:center; font-size:14px'>{icon}</span><div><div style='font-size:12px; font-weight:600'>{action}</div><div style='font-size:11px; color:#64748b'>{log['details']}</div></div></div><span style='font-size:10px; color:#94a3b8; white-space:nowrap'>{time_display}</span></div>"
        if not log_rows:
            log_rows = "<div style='padding:24px; text-align:center; color:#999; font-size:12px'>No activity yet 📜</div>"
        right = f"<div><div style='display:flex; justify-content:space-between; align-items:center'><div><b style='font-size:15px'>📜 Activity Log</b><p style='font-size:11px; color:#64748b; margin-top:4px'>Live auto-updating — {len(logs)} events — Kenya Time (EAT) 🇰🇪</p></div><a href='/clear-activity' style='font-size:11px; color:#dc2626; border:1px solid #fecaca; padding:6px 10px; border-radius:6px; text-decoration:none'>🧹 Clear Log</a></div><div style='border:1px solid #e2e8f0; border-radius:10px; margin-top:16px; overflow:hidden; max-height:500px; overflow-y:auto'>{log_rows}</div><div style='margin-top:10px; font-size:10px; color:#94a3b8; text-align:center'>🔄 Auto records login, add/edit/delete, profile view, password change — Africa/Nairobi 🌍</div></div>"
    else:
        right = f"<div><b style='font-size:15px'>👤 Personal Information</b><p style='font-size:11px; color:#64748b; margin-top:4px'>Update your personal details and contact information</p><form method='post' action='/update-profile' style='margin-top:18px'><label style='font-size:12px; font-weight:600; display:block; margin-top:14px'>👨‍💼 Full Name</label><input name='full_name' value='{name}' required style='width:100%; padding:11px; border:1px solid #e2e8f0; border-radius:8px; background:#f8fafc; margin-top:4px'><label style='font-size:12px; font-weight:600; display:block; margin-top:12px'>📧 Email Address</label><input name='email_new' value='{email}' required style='width:100%; padding:11px; border:1px solid #e2e8f0; border-radius:8px; background:#f8fafc; margin-top:4px'><label style='font-size:12px; font-weight:600; display:block; margin-top:12px'>📱 Phone Number</label><input name='phone' value='+254748588874' style='width:100%; padding:11px; border:1px solid #e2e8f0; border-radius:8px; background:#f8fafc; margin-top:4px'><label style='font-size:12px; font-weight:600; display:block; margin-top:12px'>🏫 Organization</label><input value='{sname}' disabled style='width:100%; padding:11px; border:1px solid #e2e8f0; border-radius:8px; background:#f1f5f9; color:#64748b; margin-top:4px'><div style='text-align:right; margin-top:20px'><button type='submit' style='background:#0f172a; color:white; padding:11px 18px; border:none; border-radius:8px; cursor:pointer; font-weight:600'>💾 Save Changes</button></div></form></div>"
    ...
    html = f"<html><head><meta name='viewport' content='width=device-width, initial-scale=1'></head><body style='margin:0; font-family:Arial; background:#f8fafc'>{header_html(initials, name, email)}<div style='padding:20px; max-width:1100px; margin:0 auto'><div style='margin-bottom:16px'><h2 style='margin:0; font-size:20px; font-weight:800'>👤 My Profile</h2><p style='font-size:12px; color:#64748b; margin:4px 0 0'>Manage your personal information and security settings</p></div><div style='display:flex; border-bottom:1px solid #e2e8f0; margin-bottom:20px; background:white; border-radius:10px 10px 0 0; padding:0 16px'><a href='/profile?tab=personal' style='padding:14px 4px; margin-right:24px; text-decoration:none; font-size:13px; {ap}'>👤 Personal Info</a><a href='/profile?tab=security' style='padding:14px 4px; margin-right:24px; text-decoration:none; font-size:13px; {ase}'>🔒 Security</a><a href='/profile?tab=activity' style='padding:14px 4px; text-decoration:none; font-size:13px; {aa}'>📜 Activity Log</a></div><div style='display:grid; grid-template-columns:340px 1fr; gap:20px; align-items:start'><div style='background:white; border:1px solid #e2e8f0; border-radius:14px; padding:24px; text-align:center; position:sticky; top:20px'><div style='width:88px; height:88px; background:#0f172a; color:white; border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:30px; font-weight:800; margin:0 auto'>{initials}</div><div style='font-weight:700; margin-top:14px; font-size:16px'>{name}</div><div style='margin-top:8px'><span style='background:#e0f2fe; color:#0369a1; padding:5px 12px; border-radius:20px; font-size:11px; font-weight:600'>{badge}</span></div><div style='margin-top:12px; font-size:12px; color:#0f172a'>{email}</div><div style='margin-top:16px; border-top:1px solid #f1f5f9; padding-top:16px; text-align:left'><div style='font-size:12px; font-weight:700'>🏫 {sname}</div><div style='font-size:11px; color:#64748b; margin-top:2px'>📍 {sloc}</div><div style='font-size:10px; color:#94a3b8; margin-top:6px; background:#f8fafc; padding:6px 8px; border-radius:6px'>🔑 Code: {scode}</div><div style='margin-top:14px; display:flex; flex-direction:column; gap:8px'><div style='display:flex; justify-content:space-between; font-size:11px'><span style='color:#64748b'>📊 Status</span><span style='color:#16a34a; font-weight:600'>● Active</span></div><div style='display:flex; justify-content:space-between; font-size:11px'><span style='color:#64748b'>👑 Role</span><span style='font-weight:600'>{badge}</span></div></div></div></div><div style='background:white; border:1px solid #e2e8f0; border-radius:14px; padding:24px; min-height:400px'>{right}</div></div></div></body></html>"
...
def update_password(request: Request, current_password: str = Form(...), new_password: str = Form(...), confirm_password: str = Form(...)):
    ...
    if new_password!= confirm_password:
        return HTMLResponse("<h3>❌ New passwords do not match</h3><a href='/profile?tab=security'>⬅️ Back</a>")
    email = request.session.get("email")
    con = get_db()
    cur = con.cursor()
    ...
    if not u:
        con.close()
        return HTMLResponse("<h3>❌ Current password incorrect</h3><a href='/profile?tab=security'>⬅️ Back</a>")
...
def manage_schools(request: Request, show: str = "", success: str = "", pending_id: str = ""):
    ...
    if success=="added":
        ...
        success_banner = "<div style='background:#fee2e2; border:1px solid #fca5a5; color:#991b1b; padding:12px 16px; border-radius:10px; margin-bottom:16px'>🗑️ <b>Deleted! 🗑️</b> School removed.</div>"
    rows_html = ""
    schools_json = {}
    for s in schools:
        sid = s["id"]
...
<div id='selectionInfo' style='margin-top:12px; padding:10px; background:#f8fafc; border-radius:8px; font-size:11px; color:#64748b; text-align:center'>👆 Select a school row to enable Edit / Delete ✏️🗑️</div>
</div>
...
def verify_school_code(pending_id: str = Form(...), auth_code: str = Form(...)):
    ...
    con.commit()
    con.close()
    send_email(SUPER_ADMIN, f"✅ School Added: {pending['name']} - Code {code} 🏫", f"✅ Added!\n🏫 {pending['name']}\n🔑 {code}\n📧 {pending['email']}\n🔒 School@2026!")
    log_activity(SUPER_ADMIN, f"🏫 Registered new school: {pending['name']} ✅", f"Code: {code} | Verified 🔐")
...
def resend_code(pending_id: str):
    ...
    con.commit()
    con.close()
    send_email(SUPER_ADMIN, f"🔐 New Code: {new_code} - {pending['name']} 🔄", f"🔑 New code: {new_code} for 🏫 {pending['name']}")
    return RedirectResponse(f"/schools/manage?success=code_sent&pending_id={pending_id}", status_code=303)
...
def update_school(school_id: int, school_name: str = Form(...), school_email: str = Form(...), location: str = Form(...), phone: str = Form(...), principal: str = Form(...), school_type: str = Form(...), request: Request = None):
    ...
    log_activity(email, f"✏️ Edited school: {school_name} ✏️", f"{school_type}")
...
def delete_school(school_id: int, request: Request):
    ...
    con.close()
    email = request.session.get("email","")
    log_activity(email, f"🗑️ Deleted school: {sname} 🗑️", f"Code: {s['code'] if s else ''}")
