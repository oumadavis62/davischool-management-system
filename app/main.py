...
def header_html(initials, name, email):
    return f"""<style>.do-avatar{{width:36px;height:36px;background:#dbeafe;color:#1e40af;border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:800;cursor:pointer;border:2px solid #e2e8f0}}.dropdown-item{{display:flex;align-items:center;gap:10px;padding:11px 14px;text-decoration:none;font-size:13px}}.dropdown-item:hover{{background:#0b3d91;color:white}}</style><div style='background:white;border-bottom:1px solid #e2e8f0;padding:10px 20px;display:flex;justify-content:space-between;align-items:center'><div><b style='font-size:14px'>🏫 Davischool Platform (Super Admin)</b><div style='font-size:11px;color:#64748b'>{name} • Super Admin</div></div><div style='position:relative'><div onclick='toggleProfileMenu()' class='do-avatar'>{initials}</div><div id='profileDropdown' style='display:none;position:absolute;right:0;top:44px;background:white;border:1px solid #e2e8f0;border-radius:12px;width:220px;box-shadow:0 10px 25px rgba(0,0,0,0.12);z-index:1000;overflow:hidden'><div style='padding:14px;border-bottom:1px solid #f1f5f9;background:#f8fafc'><div style='font-weight:700;font-size:13px'>{name}</div><div style='font-size:11px;color:#64748b'>{email}</div></div><a href='/super/global-control/dashboard' class='dropdown-item' style='color:#0b3d91'>🌍 Global Control</a><a href='/logout' class='dropdown-item' style='color:#dc2626'>🚪 Logout</a></div></div></div><script>function toggleProfileMenu(){{let m=document.getElementById('profileDropdown'); m.style.display=m.style.display==='none'||m.style.display===''? 'block':'none';}}</script>"""
...
def school_header(school, name, active="dashboard", is_impersonating=False):
    ...
    acad_bg = "background:#0b3d91;color:white" if is_academic_active else "background:white;color:#0b3d91;border:1px solid #e2e8f0"
    comm_bg = "background:#0b3d91;color:white" if is_comm_active else "background:white;color:#0b3d91;border:1px solid #e2e8f0"
    sys_bg = "background:#0b3d91;color:white" if is_system_active else "background:white;color:#0b3d91;border:1px solid #e2e8f0"
    def nav(link, icon, label):
        is_active = "background:#0b3d91;color:white;font-weight:800" if active==link else "color:#475569;background:transparent"
    ...
    def sub_nav(link, icon, label):
        is_sel = "background:#e2e8f0;color:#0b3d91;font-weight:800;border-radius:8px" if active==link else "color:#475569"
...
def global_header(name, active="dashboard"):
    ...
    con = get_db(); cur = con.cursor(); cur.execute("SELECT * FROM global_notices ORDER BY id DESC LIMIT 1"); notice = cur.fetchone(); con.close()
    notice_banner = f"""<div style='background:#0b3d91;color:white;padding:8px 20px;text-align:center;font-size:12px'>📢 {notice['message']}</div>""" if notice else ""
    return f"""<style>.nav-item:hover{{background:#f1f5f9!important;color:#0b3d91!important}}.ds-card{{background:white;border:1px solid #e2e8f0;border-radius:14px;padding:16px;text-decoration:none;color:#0b3d91;display:block}}.input-field{{width:100%;padding:11px 12px;border:1px solid #e2e8f0;border-radius:10px;margin:6px 0;font-size:13px;background:white}}.add-btn{{width:100%;background:#0b3d91;color:white;padding:12px;border:none;border-radius:10px;font-weight:700;cursor:pointer}}.section-header{{display:flex;align-items:center;justify-content:space-between;padding:11px 14px;border-radius:10px;cursor:pointer;font-size:13px;font-weight:800;margin-bottom:4px}}.academic-header{{ {acad_bg} }}.comm-header{{ {comm_bg} }}.system-header{{ {sys_bg} }}.section-header:hover{{background:#f1f5f9!important}}</style><div style='display:flex;min-height:100vh'><div style='width:260px;background:white;border-right:1px solid #e2e8f0;padding:16px;position:sticky;top:0;height:100vh;overflow-y:auto'><div style='padding:10px 6px 16px;border-bottom:1px solid #f1f5f9;margin-bottom:12px'><div style='display:flex;align-items:center;gap:10px'><div style='width:40px;height:40px;background:#0b3d91;color:white;border-radius:10px;display:flex;align-items:center;justify-content:center;font-weight:800'>🌍</div><div><b style='font-size:13px'>GLOBAL CONTROL</b><div style='font-size:10px;color:#64748b'>ALL SCHOOLS • Automatic</div></div></div></div>{nav('dashboard','📊','School Overview')}{nav('students','🎓','Students Manager')}{nav('classes','🏫','Classes & Streams')}<div style='margin-bottom:4px'><div class='section-header academic-header' onclick='toggleAcademic()'><span>📖 Academic Manager</span><span id='academicArrow'>{acad_arrow}</span></div><div id='academicDropdown' style='display:{acad_display};margin-left:8px;border-left:1px solid #e2e8f0;padding-left:10px;margin-bottom:6px'>{sub_nav('dean-settings','⚙️','Dean Settings')}{sub_nav('exams','🔧','Exam Settings')}{sub_nav('set-marks','📄','Set Marks')}{sub_nav('subject-allocation','📋','Subject Allocation')}{sub_nav('record-marks','✏️','Record Marks')}{sub_nav('edit-marks','📄','Edit Marks')}{sub_nav('marksheets','☰','Marks Status')}{sub_nav('analysis','📊','Exam Analysis')}{sub_nav('spreadsheet','📄','Spreadsheet')}{sub_nav('sba','📋','SBA (KNEC CBA)')}{sub_nav('report-cards','📄','Report Cards')}</div></div>{nav('teachers','👨‍🏫','Staff Manager')}{nav('timetable','🗓️','Smart Timetable')}{nav('attendance','🗓️','Attendance')}{nav('fees','💰','Fees & Finance')}<div style='margin-bottom:4px'><div class='section-header comm-header' onclick='toggleComm()'><span>💬 Communication</span><span id='commArrow'>{comm_arrow}</span></div><div id='commDropdown' style='display:{comm_display};margin-left:8px;border-left:1px solid #e2e8f0;padding-left:10px;margin-bottom:6px'>{sub_nav('sms','💬','Bulk SMS Parents')}{sub_nav('communication','📢','Announcements')}</div></div><div style='margin-bottom:4px;margin-top:6px'><div class='section-header system-header' onclick='toggleSystem()'><span>⚙️ System Settings</span><span id='systemArrow'>{sys_arrow}</span></div><div id='systemDropdown' style='display:{sys_display};margin-left:8px;border-left:1px solid #e2e8f0;padding-left:10px;margin-bottom:6px'>{sub_nav('system-settings/school-profile','🏢','School Profile')}{sub_nav('system-settings/classes','🏫','Classes')}{sub_nav('system-settings/user-management','👤+','User Management')}{sub_nav('system-settings/roles-permissions','🛡️','Roles & Permissi...')}{sub_nav('system-settings/database-backup','💾','Database Backup')}{sub_nav('system-settings/system-audit','📈','System Audit')}{sub_nav('system-settings/integrations','🔌','Integrations')}{sub_nav('system-settings/billing-payments','💳','Billing & Payments')}</div></div>{nav('my-profile','👤','My Profile')}<div style='margin-top:12px;padding-top:12px;border-top:1px solid #f1f5f9'><div style='font-size:11px;color:#94a3b8;font-weight:700;margin-bottom:8px'>Help</div>{nav('user-manual','❓','User Manual')}<div style='margin-top:14px;padding:10px;background:#f8fafc;border-radius:10px;border:1px solid #f1f5f9'><div style='display:flex;gap:8px;align-items:center'><div style='width:28px;height:28px;background:#0b3d91;color:white;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:800'>DO</div><div><div style='font-size:12px;font-weight:800'>{name}</div><div style='font-size:10px;color:#64748b'>Super Admin</div></div></div><div style='margin-top:10px;font-size:12px'>🌤️ 24°C<br><span style='color:#64748b'>Mostly cloudy</span></div></div></div><div style='margin-top:16px;border-top:1px solid #f1f5f9;padding-top:12px'><a href='/dashboard' class='nav-item' style='display:flex;align-items:center;gap:10px;padding:11px 14px;border-radius:10px;text-decoration:none;font-size:13px;color:#0b3d91;background:#f8fafc;font-weight:700'>⬅️ Back to Super Admin</a><a href='/logout' class='nav-item' style='display:flex;align-items:center;gap:10px;padding:11px 14px;border-radius:10px;text-decoration:none;font-size:13px;color:#dc2626'>🚪 Logout</a></div></div><div style='flex:1;background:#f8fafc'>{notice_banner}<div style='background:white;border-bottom:1px solid #e2e8f0;padding:12px 20px;display:flex;justify-content:space-between;align-items:center'><div><b style='font-size:13px'>GLOBAL CONTROL (ALL SCHOOLS)</b><div style='font-size:10px;color:#64748b'>Any change here updates all schools instantly</div></div><div style='display:flex;align-items:center;gap:12px'><div style='width:28px;height:28px;background:#0b3d91;color:white;border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:800;font-size:11px'>{initials}</div><div style='font-size:11px'><b>{name}</b> <span style="background:#0b3d91;color:white;padding:2px 6px;border-radius:6px;font-size:9px">SUPER</span></div></div></div><script>function toggleAcademic(){{let d=document.getElementById('academicDropdown'); let a=document.getElementById('academicArrow'); if(d.style.display==='none'||d.style.display===''){{d.style.display='block'; a.innerText='⌃';}} else {{d.style.display='none'; a.innerText='⌄';}}}}function toggleComm(){{let d=document.getElementById('commDropdown'); let a=document.getElementById('commArrow'); if(d.style.display==='none'||d.style.display===''){{d.style.display='block'; a.innerText='⌃';}} else {{d.style.display='none'; a.innerText='⌄';}}}}function toggleSystem(){{let d=document.getElementById('systemDropdown'); let a=document.getElementById('systemArrow'); if(d.style.display==='none'||d.style.display===''){{d.style.display='block'; a.innerText='⌃';}} else {{d.style.display='none'; a.innerText='⌄';}}}}</script>"""
...
def manage_schools(request: Request, success: str = "", pending_id: str = "", new_pass: str = "", school_email: str = "", school_name: str = "", message: str = ""):
    ...
    if success=="invalid_code":
        ...
        popup=f"""<div style='margin-bottom:16px;background:#fef3c7;border:2px solid #f4b400;border-radius:14px;padding:18px'><div style='font-size:18px;font-weight:900;color:#92400e'>⚠️ School account already exists</div><div style='margin-top:8px;color:#475569'>{school_email}</div></div>"""
...
<a href="/schools/manage" style="background:#0b3d91;color:white;text-decoration:none;border-radius:12px;padding:13px 30px;font-size:16px;font-weight:900">OK ✅</a>
</div>
</div></div></body></html>""")
    except Exception as exc:
        try:
...
body{margin:0;font-family:Arial;background:#f8fafc;color:#172033}.wrap{max-width:760px;margin:30px auto;padding:16px}.card{background:white;border:1px solid #e2e8f0;border-radius:18px;padding:24px}label{display:block;font-size:12px;font-weight:800;margin:12px 0 5px;color:#475569}input,select{width:100%;box-sizing:border-box;padding:12px;border:1px solid #dbe2e8;border-radius:10px}.grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}button,a{display:inline-block;padding:12px 18px;border-radius:10px;font-weight:800;text-decoration:none}button{border:0;background:#0b3d91;color:white;cursor:pointer}a{border:1px solid #e2e8f0;color:#0b3d91;background:white}@media(max-width:650px){.grid{grid-template-columns:1fr}}</style></head><body><div class='wrap'><div class='card'><h1 style='margin-top:0'>✏️ Edit School</h1><div style='color:#64748b;font-size:13px;margin-bottom:18px'>Update the registered school details and its school administrator account.</div>
...
def add_class(request: Request, class_name: str = Form(...), stream: str = Form(...)):
    ...
    if not school:
        ...
        if user and user["school_id"]:
            ...
            school = get_school_obj(request)
    if not school or not _role_permission(request,"classes.create"):
        return RedirectResponse("/school/classes", 303)
    con=get_db(); cur=con.cursor()
    nm=class_name.strip().upper(); st=stream.strip().upper()
    if not nm:
        con.close()
        return HTMLResponse("Class name is required.",400)
    if cur.execute("SELECT id FROM classes WHERE school_id=? AND upper(name)=? AND upper(stream)=?",
                   (school["id"],nm,st)).fetchone():
        con.close()
        return HTMLResponse("That class/stream already exists in this school.",409)
    cur.execute("INSERT INTO classes (school_id,name,stream) VALUES (?,?,?)",(school["id"],nm,st))
    con.commit(); con.close()
    return RedirectResponse("/school/classes",303)
...
def del_class(cid: int, request: Request):
    school=get_school_obj(request)
    if not school or not _role_permission(request,"classes.edit"):
        return RedirectResponse("/school/classes",303)
    con=get_db(); cur=con.cursor()
...
def school_system_settings(sub: str, request: Request):
    ...
    def pill(tab, label, icon):
        active_style = "background:#0b3d91;color:white" if sub==tab else "background:#f8fafc;color:#475569;border:1px solid #e2e8f0"
...
def portal_page(request, title, body):
    ...
    nav=f"<div style='background:#0b3d91;color:white;padding:16px 20px;display:flex;justify-content:space-between'><div><b>🏫 DaviSchool</b><div style='font-size:11px;opacity:.75'>{school['name'] if school else ''}</div></div><div>{name} • {role.title()} &nbsp; <a href='/logout' style='color:white'>Logout</a></div></div>"
    return HTMLResponse(f"<html><head><meta name='viewport' content='width=device-width,initial-scale=1'><style>body{{margin:0;font-family:Arial;background:#f8fafc;color:#0b3d91}}.wrap{{max-width:1200px;margin:auto;padding:22px}}.card{{background:white;border:1px solid #e2e8f0;border-radius:16px;padding:18px;margin-bottom:14px}}table{{width:100%;border-collapse:collapse}}th,td{{padding:10px;border-bottom:1px solid #f1f5f9;text-align:left;font-size:13px}}h1{{font-size:25px}}</style></head><body>{nav}<div class='wrap'><h1>{title}</h1>{body}</div></body></html>")
...
*{{box-sizing:border-box}}body{{margin:0;font-family:Inter,Arial,sans-serif;background:#f4f7fb;color:#0b3d91}}.mod{{padding:24px;max-width:1600px;margin:auto}}
...
.btn{{display:inline-flex;align-items:center;justify-content:center;gap:6px;background:#0b3d91;color:white;padding:10px 14px;border:0;border-radius:10px;font-weight:700;cursor:pointer;text-decoration:none;font-size:12px}}.btn.secondary{{background:#f8fafc;color:#0b3d91;border:1px solid #d8e3f0}}.btn.success{{background:#15803d}}.btn.danger{{background:#dc2626}}
table{{width:100%;border-collapse:collapse;overflow:hidden}}th{{background:#0b3d91;color:#fff;font-size:11px;text-transform:uppercase;letter-spacing:.04em}}th,td{{padding:11px 10px;border-bottom:1px solid #d8e3f0;text-align:left;font-size:12px;vertical-align:middle}}tr:hover td{{background:#f8fbff}}
...
@media(max-width:900px){{.kpis{{grid-template-columns:repeat(2,1fr)}}.grid-2,.grid-3{{grid-template-columns:1fr}}.mod{{padding:14px}}table{{display:block;overflow-x:auto;white-space:nowrap}}}}@media(max-width:600px){{.kpis{{grid-template-columns:1fr 1fr}}h1{{font-size:21px}}.search{{max-width:100%}}}}@media print{{@page{{size:A4;margin:8mm 10mm 12mm}}body{{background:#fff!important;color:#172033!important}}header,.side,.top,.no-print,.page-head,.mod>div:first-child{{}}.btn,.toolbar form,form,button{{display:none!important}}.mod{{padding:0!important;max-width:none!important}}.card{{border:0!important;box-shadow:none!important;border-radius:0!important}}th{{background:#0b3d91!important;color:#fff!important;-webkit-print-color-adjust:exact;print-color-adjust:exact}}}}
...
def school_report_cards(request:Request, student_id:int=0, exam_id:int=0):
    ...
    school=get_school_obj(request)
    legacy_school_logo=str(school['logo_data'] or '') if school and 'logo_data' in school.keys() else ''
    legacy_school_name=escape(str(school['name'] or 'DaviSchool')) if school else 'DaviSchool'
    legacy_postal=escape('P.O. Box '+str(school['postal_address'])) if school and 'postal_address' in school.keys() and school['postal_address'] else ''
    legacy_postal_code=escape(str(school['postal_code'])) if school and 'postal_code' in school.keys() and school['postal_code'] else ''
    legacy_phone=escape(str(school['phone'] or '')) if school else ''
    legacy_email=escape(str(school['email'] or '')) if school else ''
    legacy_doc_header="<div class='legacy-doc-header'><div class='legacy-doc-logo'>%s</div><div class='legacy-doc-school'><div class='legacy-doc-name'>%s</div>%s%s</div><div class='legacy-doc-right'>%s%s</div></div>" % ((("<img src='%s' alt='School logo'>" % escape(legacy_school_logo)) if legacy_school_logo else '🏫'),legacy_school_name,(("<div>%s</div>"%legacy_postal) if legacy_postal else ''),(("<div>%s</div>"%legacy_postal_code) if legacy_postal_code else ''),(("<div>☎ %s</div>"%legacy_phone) if legacy_phone else ''),(("<div>✉ %s</div>"%legacy_email) if legacy_email else ''))
    body=f"""<div class='card no-print'><form method='get' style='display:flex;gap:10px;flex-wrap:wrap'><select name='student_id' class='input-field' style='max-width:320px'>{opts}</select><select name='exam_id' class='input-field' style='max-width:260px'>{eopts}</select><button class='btn'>Load Report</button><button type='button' class='btn' onclick='window.print()'>🖨️ Print</button></form></div><div class='card' id='report-card'><div class='legacy-print-document'>{legacy_doc_header}<div class='legacy-report-title'>STUDENT REPORT CARD</div><div class='legacy-report-subtitle'>{ex['name'] if ex else ''} — {ex['term'] if ex else ''} {ex['year'] if ex else ''}</div><div style='display:flex;justify-content:space-between;margin:18px 0'><div><b>Student:</b> {st['name'] if st else ''}<br><b>Admission:</b> {st['admission_no'] if st else ''}</div><div><b>Total:</b> {total}<br><b>Average:</b> {avg}</div></div><table><tr><th>Subject</th><th>Marks</th><th>Grade</th></tr>{trs}</table><p style='margin-top:20px'><b>Teacher/School Comment:</b> ________________________________</p><p><b>Principal/Head Comment:</b> __________________________________</p></div></div><style>.legacy-doc-header{{display:flex;align-items:flex-start;gap:14px;border-top:2px solid #f4b400;border-bottom:3px solid #0b3d91;padding:8px 4px 10px;margin-bottom:10px}}.legacy-doc-logo{{width:86px;height:70px;display:flex;align-items:center;justify-content:center;flex:0 0 86px}}.legacy-doc-logo img{{max-width:82px;max-height:66px;object-fit:contain}}.legacy-doc-school{{flex:1;min-width:0;color:#0b3d91}}.legacy-doc-name{{font-size:20px;line-height:1.15;font-weight:900;text-transform:uppercase}}.legacy-doc-school>div:not(.legacy-doc-name){{font-size:10px;color:#334155;line-height:1.55}}.legacy-doc-right{{min-width:155px;font-size:10px;color:#0b3d91;line-height:1.65}}.legacy-report-title{{text-align:center;color:#0b3d91;font-size:22px;font-weight:900;padding:5px}}.legacy-report-subtitle{{text-align:center;color:#334155;font-size:11px;margin-bottom:8px}}@media print{{@page{{size:A4;margin:8mm 10mm 12mm}}.legacy-doc-header{{break-inside:avoid}}.legacy-print-document{{display:block!important}}.legacy-print-document table th{{background:#0b3d91!important;color:#fff!important;-webkit-print-color-adjust:exact;print-color-adjust:exact}}.legacy-print-document table th,.legacy-print-document table td{{border-color:#0b3d91!important}}.legacy-print-document .legacy-report-title{{color:#0b3d91!important}}}}</style>"""
