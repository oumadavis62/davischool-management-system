...
app = FastAPI()
app.add_middleware(SessionMiddleware, secret_key="davischool-zeraki-final-v8")
...
def dashboard(request: Request):
    ...
    if is_super:
        ...
        <div style='display:grid; grid-template-columns:repeat(4,1fr); gap:16px; margin-bottom:20px'>
            <div style='background:white; border:1px solid #e2e8f0; border-radius:14px; padding:18px'><div style='font-size:11px; color:#64748b; font-weight:600'>TOTAL SCHOOLS</div><div style='font-size:28px; font-weight:800; margin:8px 0 2px'>{total_schools}</div><div style='font-size:11px; color:#16a34a'>Up 12%</div></div>
            <div style='background:white; border:1px solid #e2e8f0; border-radius:14px; padding:18px'><div style='font-size:11px; color:#64748b; font-weight:600'>ACTIVE</div><div style='font-size:28px; font-weight:800; margin:8px 0 2px'>{active_schools}</div><div style='font-size:11px; color:#16a34a'>All active</div></div>
            <div style='background:white; border:1px solid #e2e8f0; border-radius:14px; padding:18px'><div style='font-size:11px; color:#64748b; font-weight:600'>TOTAL REVENUE</div><div style='font-size:28px; font-weight:800; margin:8px 0 2px'>KES {revenue:,}</div><div style='font-size:11px; color:#64748b'>15k per school</div></div>
            <div style='background:white; border:1px solid #e2e8f0; border-radius:14px; padding:18px'><div style='font-size:11px; color:#64748b; font-weight:600'>STUDENTS</div><div style='font-size:28px; font-weight:800; margin:8px 0 2px'>{total_students:,}</div><div style='font-size:11px; color:#64748b'>Across all schools</div></div>
        </div>
...
def profile(request: Request, tab: str = "personal"):
    ...
    if tab=="security":
        ...
        right = f"<b>Activity Log</b><div style='border:1px solid #e2e8f0; border-radius:8px; margin-top:12px'><div style='padding:10px; border-bottom:1px solid #f1f5f9; font-size:12px'>Logged in - {email}</div></div>"
    else:
...
def update_password(request: Request, current_password: str = Form(...), new_password: str = Form(...), confirm_password: str = Form(...)):
    ...
    if not u:
        con.close()
        return HTMLResponse("<h3>Error: Current password incorrect</h3><a href='/profile?tab=security'>Back</a>")
    cur.execute("UPDATE users SET password=? WHERE email=?", (new_password, email))
    con.commit()
    con.close()
    return HTMLResponse("<h3>Password Updated Successfully!</h3><a href='/profile?tab=security'>Back</a>")
...
def manage_schools(request: Request, show: str = ""):
    ...
    for s in schools:
        ...
        <td style='padding:10px; border-bottom:1px solid #eee; font-size:11px'>
            <a href='/schools/edit/{s['id']}' style='background:#e0f2fe; color:#0369a1; padding:5px 10px; border-radius:6px; text-decoration:none; margin-right:4px'>Edit</a>
            <a href='/schools/delete/{s['id']}' onclick="return confirm('Delete {s['name']}? This will also delete its admin login!')" style='background:#fee2e2; color:#dc2626; padding:5px 10px; border-radius:6px; text-decoration:none'>Delete</a>
        </td>
        </tr>
        """
    if not rows:
        rows = "<tr><td colspan=4 style='padding:20px; text-align:center; color:#999; font-size:12px'>No schools yet - Add your first school on the right</td></tr>"

    add_form_highlight = "border:2px solid #0f172a; box-shadow:0 0 0 3px #e0f2fe" if show=="add" else "border:1px solid #e2e8f0"
...
<div style='background:white; border-bottom:1px solid #e2e8f0; padding:12px 20px; display:flex; justify-content:space-between; align-items:center'><b>Davischool - Schools Management (Super Admin)</b><div><a href='/dashboard' style='text-decoration:none; border:1px solid #e2e8f0; padding:6px 12px; border-radius:6px; font-size:12px'>Back to Dashboard</a></div></div>
...
<div style='display:flex; justify-content:space-between; align-items:center'><b>Registered Schools ({len(schools)})</b><span style='font-size:11px; color:#64748b'>Edit / Delete enabled</span></div>
<table style='width:100%; margin-top:12px; border-collapse:collapse'>
<tr style='background:#f8fafc; font-size:11px; text-align:left'><th style='padding:10px'>School</th><th style='padding:10px'>Contact</th><th style='padding:10px'>Location / Principal</th><th style='padding:10px'>Actions</th></tr>
{rows}
</table>
</div>
<div style='background:white; {add_form_highlight}; border-radius:12px; padding:20px; position:sticky; top:20px'>
...
<input name='location' placeholder='Location / County * e.g. Busia, Kenya' required style='width:100%; padding:11px; border:1px solid #e2e8f0; border-radius:8px'>
<input name='phone' placeholder='Phone Number * e.g. 0748588874' required style='width:100%; padding:11px; border:1px solid #e2e8f0; border-radius:8px'>
<input name='principal' placeholder='Principal Name * e.g. John Doe' required style='width:100%; padding:11px; border:1px solid #e2e8f0; border-radius:8px'>
...
<div style='font-size:10px; color:#64748b; margin-top:4px'>System will auto-generate School Code + Admin Login: School@2026!</div>
</form>
</div>
</div>
</body></html>
...
<h3>Edit School - {s['name']}</h3><p style='font-size:11px; color:#64748b'>Code: {s['code']} (cannot be changed)</p>
...
<label style='font-size:12px'>School Type</label>
...
def register_school(school_name: str = Form(...), school_email: str = Form(...), location: str = Form(...), phone: str = Form(...), principal: str = Form(...), school_type: str = Form(...)):
    ...
    con.commit()
    con.close()
    return HTMLResponse(f"<h3>School {school_name} Created - Code {code}</h3><p>Type: {school_type} | Principal: {principal} | Phone: {phone}</p><p>Login: {school_email} / School@2026!</p><a href='/schools/manage'>Back to Manage</a> | <a href='/dashboard'>Dashboard</a>")
