...
def header_html(initials, name, email):
    return f"""
    <style>.do-avatar{{width:36px;height:36px;background:#dbeafe;color:#1e40af;border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:800;cursor:pointer;border:2px solid #e2e8f0}}
...
.back-btn{{display:inline-flex;align-items:center;gap:6px;padding:10px 16px;background:white;border:1px solid #e2e8f0;border-radius:10px;text-decoration:none;color:#0f172a;font-weight:700;font-size:12px}}</style>
...
.ds-card:hover{{background:#0f172a!important;color:white!important}}
...
# === NEW WELCOME SCREEN - DAVISCHOOL WITH LOGO + SHOW/HIDE + ASK DAVIS ===
...
.card{background:white;width:540px;max-width:100%;padding:50px 48px;border-radius:4px;box-shadow:0 0 0 1px #e2e8f0}
.logo-box{width:56px;height:56px;background:#0f172a;color:white;border-radius:14px;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:24px;margin-top:14px}
.input{width:100%;padding:14px 16px;border:1px solid #e2e8f0;border-radius:10px;background:#fcfcfc;font-size:14px;outline:none;box-sizing:border-box}
...
<div class="main">
  <div class="card">
    <h1 style="margin:0;font-size:40px;font-weight:900;color:#0f172a;letter-spacing:-0.5px">DaviSchool</h1>
    <div class="logo-box">D</div>
    <div style="margin-top:20px;color:#334155;font-size:15px">Sign in to your Davischool account</div>
...
<div class="ask" onclick="alert('Hello! I am Davis AI Assistant 🤖')">💬 Ask Davis</div>
<script>
function togglePwd(){
...
def dashboard(request: Request):
    ...
    content = f"""<div style='padding:20px; max-width:1400px; margin:auto'><h2 style='margin:0; font-size:22px; font-weight:800'>📊 School Overview</h2><p style='margin:4px 0 0; color:#64748b; font-size:13px'>Welcome {name}</p><div style='display:grid; grid-template-columns:repeat(4,1fr); gap:16px; margin:18px 0'><div style='background:white; border:1px solid #e2e8f0; border-radius:14px; padding:18px'><div style='font-size:11px; color:#64748b'>🏫 TOTAL SCHOOLS</div><div style='font-size:32px; font-weight:900; margin:12px 0'>{total}</div><div style='font-size:11px; color:#16a34a'>📈 Up 12% from last month</div></div><div style='background:white; border:1px solid #e2e8f0; border-radius:14px; padding:18px'><div style='font-size:11px; color:#64748b'>✅ ACTIVE SCHOOLS</div><div style='font-size:32px; font-weight:900; margin:12px 0'>{total}</div><div style='font-size:11px; color:#16a34a'>🟢 100% operational</div></div><div style='background:white; border:1px solid #e2e8f0; border-radius:14px; padding:18px'><div style='font-size:11px; color:#64748b'>🔥 TOTAL REVENUE</div><div style='font-size:26px; font-weight:900; margin:12px 0'>KES {total*15000}</div><div style='font-size:11px; color:#16a34a'>💹 +8% monthly growth</div></div><div style='background:white; border:1px solid #e2e8f0; border-radius:14px; padding:18px'><div style='font-size:11px; color:#64748b'>🎓 TOTAL STUDENTS</div><div style='font-size:32px; font-weight:900; margin:12px 0'>0</div></div></div><div style='background:white; border:1px solid #e2e8f0; border-radius:14px; overflow:hidden'><div style='padding:14px 16px; border-bottom:1px solid #f1f5f9'><b>🏫 Recently Added Schools</b></div><table style='width:100%; border-collapse:collapse'><thead><tr style='background:#f8fafc; text-align:left; font-size:11px; color:#64748b'><th style='padding:10px 14px'>Name</th><th>Location</th><th>Status</th><th>Date</th></tr></thead><tbody>{rows}</tbody></table></div></div>"""
...
def school_dashboard(request: Request):
    ...
    html = f"""<div style='padding:18px; max-width:1400px; margin:auto'>
        ...
        <div style='background:white; border:1px solid #e2e8f0; border-radius:14px; padding:16px; margin-bottom:16px'><div style='display:flex; justify-content:space-between'><div><div style='font-weight:800; font-size:14px'>👥 Students by Gender</div></div><div style='display:flex; gap:12px; font-size:11px'><span><span style='width:10px;height:10px;background:#0a84ff;display:inline-block'></span> Boys</span><span><span style='width:10px;height:10px;background:#ff2d92;display:inline-block'></span> Girls</span></div></div><div style='display:flex; gap:24px; overflow-x:auto; margin-top:18px'>{chart_html}</div><div style='display:grid; grid-template-columns:repeat(4,1fr); gap:12px; margin-top:18px; border-top:1px solid #f1f5f9; padding-top:14px'><div style='background:#f0f9ff; padding:12px; border-radius:10px; text-align:center'><div style='font-size:11px'>Total Boys</div><div style='font-size:20px; font-weight:900; color:#0a84ff'>{tb}</div></div><div style='background:#fdf2f8; padding:12px; border-radius:10px; text-align:center'><div style='font-size:11px'>Total Girls</div><div style='font-size:20px; font-weight:900; color:#ff2d92'>{tg}</div></div><div style='background:#f8fafc; padding:12px; border-radius:10px; text-align:center'><div style='font-size:11px'>Total</div><div style='font-size:20px; font-weight:900'>{total}</div></div><div style='background:#f0fdf4; padding:12px; border-radius:10px; text-align:center'><div style='font-size:11px'>Ratio</div><div style='font-size:20px; font-weight:900'>{ratio}:1</div></div></div></div>
...
def dean_settings(request: Request, tab: str = "terms"):
    ...
    for i, t in enumerate(terms, 1):
        rows += f"<tr style='border-bottom:1px solid #f1f5f9'><td style='padding:14px'><input type='checkbox'></td><td style='padding:14px; font-size:13px'>{i}</td><td style='padding:14px; font-size:13px; font-weight:600'>{t['term_name']}</td><td style='padding:14px; font-size:13px'>{t['year']}</td><td style='padding:14px; font-size:13px'>{t['start_date']}</td><td style='padding:14px; font-size:13px'>{t['end_date']}</td><td style='padding:14px; display:flex; gap:12px'><a href='#'>✏️</a><a href='/school/dean-settings/delete-term/{t['id']}' style='text-decoration:none'>🗑️</a></td></tr>"
    if not rows:
        rows = "<tr><td colspan='7' style='padding:30px; text-align:center; color:#94a3b8'>No terms yet — Add Term to see like screenshot (Term 3 2026 2026-08-24 to 2026-10-23)</td></tr>"
    ...
    <div style='padding:18px; max-width:1400px; margin:auto'>
      <div style='margin-bottom:14px'><h2 style='margin:0; font-size:22px; font-weight:800'>Dean Settings</h2><p style='margin:4px 0 0; color:#64748b; font-size:13px'>Manage terms, subjects, teacher allocation & promotions</p></div>
      <div style='background:white; border:1px solid #e2e8f0; border-radius:14px; padding:16px'>
        ...
        <div style='display:flex; justify-content:space-between; align-items:center; margin-bottom:14px; flex-wrap:wrap; gap:10px'>
          <div style='display:flex; gap:10px'>
            ...
            <select style='padding:9px 12px; border:1px solid #e2e8f0; border-radius:10px; font-size:13px; background:white'><option>All Years</option><option>2026</option><option>2025</option></select>
          </div>
          <div style='display:flex; gap:10px'>
            <button style='padding:9px 14px; border:1px solid #e2e8f0; border-radius:10px; background:white; font-size:13px; font-weight:600; cursor:pointer'>⬇️ Download</button>
            <button onclick="document.getElementById('addTermBox').style.display='block'" style='padding:9px 14px; background:#0f172a; color:white; border:none; border-radius:10px; font-size:13px; font-weight:700; cursor:pointer'>+ Add Term</button>
          </div>
        </div>
        <div id='addTermBox' style='display:none; background:#f8fafc; border:1px solid #e2e8f0; border-radius:10px; padding:14px; margin-bottom:14px'>
          <form method='post' action='/school/dean-settings/add-term' style='display:grid; grid-template-columns:1fr 1fr 1fr 1fr auto; gap:10px; align-items:end'>
            ...
            <div><button class='add-btn' style='padding:11px 18px'>Add</button></div>
          </form>
        </div>
        <div style='overflow:auto; border:1px solid #f1f5f9; border-radius:10px'>
          <table style='width:100%; border-collapse:collapse'><thead><tr style='background:#f8fafc; text-align:left; font-size:11px; color:#64748b'><th style='padding:12px'><input type='checkbox'></th><th style='padding:12px'>#</th><th style='padding:12px'>TERM ↕</th><th style='padding:12px'>YEAR ↕</th><th style='padding:12px'>START DATE ↕</th><th style='padding:12px'>END DATE ↕</th><th style='padding:12px'>ACTIONS</th></tr></thead><tbody id='termBody'>{rows}</tbody></table>
        </div>
        <div style='display:flex; justify-content:space-between; align-items:center; margin-top:14px; font-size:12px; color:#64748b'>
          <div style='display:flex; align-items:center; gap:8px'><span>Show</span><select style='padding:6px 10px; border:1px solid #e2e8f0; border-radius:8px'><option>10</option></select><span>items per page</span></div>
          <div>Showing 1–{len(terms) or 1} of {len(terms) or 1}</div>
          <div style='display:flex; gap:10px; align-items:center'><span>‹ Previous</span><span style='font-weight:700'>Page 1 of 1</span><span>Next ›</span></div>
        </div>
      </div>
    </div>
    <script>
    function filterTerms(){{let q=document.getElementById('searchTerm').value.toLowerCase(); document.querySelectorAll('#termBody tr').forEach(r=>{{r.style.display=r.textContent.toLowerCase().includes(q)?'':'none';}});}}
    </script>
    </div></div></body></html>""")
...
def add_term(request: Request, term_name: str = Form(...), year: str = Form(...), start_date: str = Form(...), end_date: str = Form(...)):
    school = get_school_obj(request); con = get_db(); cur = con.cursor(); cur.execute("INSERT INTO terms (school_id, term_name, year, start_date, end_date) VALUES (?,?,?,?,?)", (school["id"], term_name, year, start_date, end_date)); con.commit(); con.close(); return RedirectResponse("/school/dean-settings?tab=terms", status_code=303)
