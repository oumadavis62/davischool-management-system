...
def init_db():
    ...
    try:
        cur.execute("ALTER TABLE classes ADD COLUMN stream TEXT")
    except:
        pass
    try:
        cur.execute("ALTER TABLE students ADD COLUMN stream TEXT")
    except:
        pass
    try:
        cur.execute("ALTER TABLE students ADD COLUMN assessment_no TEXT")
    except:
        pass
    try:
        cur.execute("ALTER TABLE subjects ADD COLUMN initial TEXT")
    except:
        pass
...
def school_header(school, name, active="dashboard"):
   ...
   .back-btn:hover{{background:#0f172a!important;color:white!important;border-color:#0f172a!important;transform:translateY(-1px);box-shadow:0 6px 16px rgba(15,23,42,0.3)}}
    ...
    <div style='width:260px;background:white;border-right:1px solid #e2e8f0;padding:16px;position:sticky;top:0;height:100vh;overflow-y:auto'>
      ...
      {nav('teacher-allocation','📌','Teacher Allocation')}
...
def profile_page(request: Request, tab: str = "personal"):
    ...
    email = request.session.get("email","oumadavis62@gmail.com")
    initials = "".join([p[0] for p in name.split()][:2]).upper() if name else "DO"
    con = get_db()
    cur = con.cursor()
    cur.execute("SELECT COUNT(*) as c FROM activity_log WHERE email=?", (email,))
    total_events = cur.fetchone()["c"]
    cur.execute("SELECT * FROM activity_log WHERE email=? ORDER BY id DESC LIMIT 20", (email,))
    logs = cur.fetchall()
    con.close()
    ...
    if tab == "personal":
        ...
        right_card = f"""<div style='background:white; border:1px solid #e2e8f0; border-radius:16px; padding:20px'><div style='font-weight:800; margin-bottom:16px; font-size:14px'>🔒 Security Settings</div><form method='post' action='/profile/change-password'><label style='font-size:11px; font-weight:700; color:#475569'>🔑 Current Password</label><input type='password' name='current_pass' required style='width:100%; padding:11px; border:1px solid #e2e8f0; border-radius:10px; margin:4px 0 14px'><label style='font-size:11px; font-weight:700; color:#475569'>🆕 New Password</label><input type='password' name='new_pass' required style='width:100%; padding:11px; border:1px solid #e2e8f0; border-radius:10px; margin:4px 0 14px'><div style='text-align:right; margin-top:16px'><button style='background:#0f172a; color:white; padding:10px 18px; border:none; border-radius:10px; font-weight:700; font-size:12px'>🔒 Update Password</button></div></form></div>"""
    else:
        log_rows_html = ""
        for l in logs:
            log_rows_html += f"""<div style='padding:12px 0; border-bottom:1px solid #f1f5f9; display:flex; justify-content:space-between; gap:12px'><div style='flex:1'><div style='font-weight:700; font-size:12px'>{l['action']}</div><div style='font-size:11px; color:#64748b'>{l['details']}</div></div><div style='font-size:10px; color:#64748b; white-space:nowrap'>{l['timestamp']}</div></div>"""
        if not log_rows_html:
            log_rows_html = "<div style='padding:30px; text-align:center; color:#94a3b8'>No activity yet 📭</div>"
        right_card = f"""<div style='background:white; border:1px solid #e2e8f0; border-radius:16px; padding:20px'><div style='display:flex; justify-content:space-between; align-items:center; margin-bottom:4px'><div style='font-weight:800; font-size:14px'>📜 Activity Log</div><a href='/profile/clear-logs' style='border:1px solid #e2e8f0; padding:4px 10px; border-radius:8px; font-size:11px; text-decoration:none; color:#475569'>🧹 Clear</a></div><div style='font-size:11px; color:#64748b; margin-bottom:14px'>{total_events} events</div><div style='border-top:1px solid #f1f5f9; padding-top:8px; max-height:65vh; overflow:auto'>{log_rows_html}</div></div>"""
...
def clear_logs(request: Request):
    ...
    con = get_db()
    cur = con.cursor()
    cur.execute("DELETE FROM activity_log WHERE email=?", (request.session.get("email"),))
    con.commit()
    con.close()
...
def change_password(request: Request, current_pass: str = Form(...), new_pass: str = Form(...)):
    ...
    con = get_db()
    cur = con.cursor()
    cur.execute("SELECT * FROM users WHERE email=? AND password=?", (email, current_pass))
    u = cur.fetchone()
    if not u:
        con.close()
        return HTMLResponse(f"❌ Wrong password <a href='/profile?tab=security'>Back</a>")
...
def login(request: Request, email: str = Form(...), password: str = Form(...)):
    ...
    log_activity(u["email"], "🔓 Super Admin Logged in", "Viewed dashboard")
...
def dashboard(request: Request):
    ...
    cur.execute("SELECT * FROM schools ORDER BY id DESC LIMIT 10")
    recent = cur.fetchall()
    con.close()
    name = request.session.get("name","Davis Ouma")
    email = request.session.get("email","oumadavis62@gmail.com")
...
def manage_schools(request: Request, success: str = "", pending_id: str = "", new_pass: str = "", school_email: str = "", school_name: str = ""):
    ...
    if not rows_html:
        rows_html = "<tr><td colspan='7' style='padding:40px; text-align:center; color:#94a3b8'>No schools yet</td></tr>"
...
def verify_school_code(pending_id: str = Form(...), auth_code: str = Form(...)):
    ...
    if pending["auth_code"]!= auth_code.strip():
        con.close()
        return HTMLResponse(f"<h3>❌ Wrong Code!</h3><a href='/schools/manage?success=code_sent&pending_id={pending_id}'>Try Again</a>")
...
def resend_code(pending_id: str):
    ...
    p = cur.fetchone()
    if p:
        send_email(SUPER_ADMIN, f"🔐 Code: {p['auth_code']} - {p['name']}", f"🔑 CODE: {p['auth_code']}")
    con.close()
    return RedirectResponse(f"/schools/manage?success=code_sent&pending_id={pending_id}", status_code=303)
...
def edit_school_page(sid: int, request: Request):
    ...
    cur.execute("SELECT * FROM schools WHERE id=?", (sid,))
    s = cur.fetchone()
    cur.execute("SELECT * FROM users WHERE school_id=? AND role='school_admin'", (sid,))
    u = cur.fetchone()
    con.close()
    if not s:
...
def school_dashboard(request: Request):
    ...
    con = get_db()
    cur = con.cursor()
    cur.execute("SELECT COUNT(*) c FROM students WHERE school_id=?", (school["id"],))
    sc = cur.fetchone()["c"]
    cur.execute("SELECT COUNT(*) c FROM classes WHERE school_id=?", (school["id"],))
    cc = cur.fetchone()["c"]
    cur.execute("SELECT COUNT(*) c FROM exams WHERE school_id=?", (school["id"],))
    ec = cur.fetchone()["c"]
    cur.execute("SELECT COUNT(*) c FROM teachers WHERE school_id=?", (school["id"],))
    tc = cur.fetchone()["c"]
    cur.execute("SELECT SUM(paid) as t FROM fees WHERE school_id=?", (school["id"],))
    fees_row = cur.fetchone()
    fees_total = fees_row["t"] if fees_row and fees_row["t"] else 0
    cur.execute("SELECT * FROM students WHERE school_id=? ORDER BY id DESC LIMIT 5", (school["id"],))
    recent_students = cur.fetchall()
    con.close()
    stu_rows = ""
    for st in recent_students:
        stu_rows += f"<tr style='border-bottom:1px solid #f1f5f9'><td style='padding:10px 12px; font-size:12px'>{st['name']}</td><td style='padding:10px 12px; font-size:11px'>{st['admission_no'] or st['assessment_no'] or ''}</td><td style='padding:10px 12px; font-size:11px'>{st['gender']}</td><td style='padding:10px 12px; font-size:11px'>Class {st['class_id'] or ''}</td></tr>"
    if not stu_rows:
        stu_rows = "<tr><td colspan='4' style='padding:30px; text-align:center; color:#94a3b8; font-size:13px'>No students yet</td></tr>"
    header = school_header(school, name, "dashboard")
    html = f"""
    <div style='padding:18px; max-width:1400px; margin:auto'>
        <div style='background:linear-gradient(135deg,#0f172a 0%, #1e3a8a 60%, #1e40af 100%); border-radius:18px; padding:22px 24px; color:white; display:flex; justify-content:space-between; align-items:center; margin-bottom:16px; box-shadow:0 10px 30px rgba(15,23,42,0.3)'>
          <div>
            <div style='font-size:22px; font-weight:900; letter-spacing:0.5px'>DaviSchool Management System 🚀</div>
            <div style='font-size:12px; color:#bfdbfe; margin-top:4px'>REVOLUTIONIZE YOUR SCHOOL'S MANAGEMENT!</div>
            <div style='font-size:11px; background:rgba(255,255,255,0.15); border:1px solid rgba(255,255,255,0.2); display:inline-block; padding:6px 14px; border-radius:20px; margin-top:10px'>WORKS FOR ALL LEVELS: PRE-PRIMARY, PRIMARY, JUNIOR, SENIOR | 8-4-4 SUPPORTED</div>
          </div>
          <div style='text-align:right'>
            <div style='font-size:34px; font-weight:900'>{sc}</div>
            <div style='font-size:11px; color:#cbd5e1'>Total Students</div>
            <div style='font-size:11px; background:#16a34a; color:white; padding:7px 14px; border-radius:20px; margin-top:10px; display:inline-block; font-weight:700'>✅ GET EVERYTHING DONE IN MINUTES!</div>
          </div>
        </div>
        ...
        <div style='display:grid; grid-template-columns:1.9fr 0.8fr; gap:14px'>
          ...
          <div style='background:#0f172a; border-radius:14px; padding:16px; color:white; height:fit-content'><div style='font-weight:800; font-size:14px; margin-bottom:12px'>📊 DaviSchool Management System</div><div style='font-size:10px; color:#94a3b8; margin-bottom:12px'>STUDENT REPORT</div></div>
        </div>
    </div>
    """
...
def school_classes(request: Request):
    ...
    con = get_db()
    cur = con.cursor()
    cur.execute("SELECT * FROM classes WHERE school_id=? ORDER BY name, stream", (school["id"],))
    ...
    if not rows:
        rows = "<tr><td colspan='3' style='padding:30px; text-align:center; color:#94a3b8'>No classes yet</td></tr>"
    header = school_header(school, name, "classes")
    return HTMLResponse(f"""<html><head><meta name='viewport' content='width=device-width, initial-scale=1'><style>body{{margin:0;font-family:Arial;background:#f8fafc; caret-color:transparent}}</style></head><body>{header}
    <div style='padding:18px; max-width:1200px; margin:auto'><div style='display:grid; grid-template-columns:1.7fr 0.7fr; gap:16px'><div style='background:white; border:1px solid #e2e8f0; border-radius:14px; overflow:hidden'><div style='padding:14px 16px; border-bottom:1px solid #f1f5f9; display:flex; justify-content:space-between'><b>🏫 Classes & Streams ({len(classes)})</b><span style='font-size:11px; color:#64748b'>Click row = blue</span></div><table style='width:100%; border-collapse:collapse'><thead><tr style='background:#f8fafc; text-align:left; font-size:11px; color:#64748b'><th style='padding:10px 12px'>Class</th><th style='padding:10px 12px'>Stream</th><th style='padding:10px 12px'>Action</th></tr></thead><tbody>{rows}</tbody></table><div style='padding:12px'><a href='/school/dashboard' class='back-btn'>⬅️ Back to Overview</a></div></div><div style='background:white; border:1px solid #e2e8f0; border-radius:14px; padding:16px; height:fit-content'><b>➕ Add Class & Stream</b><form method='post' action='/school/classes/add' style='margin-top:10px'><input name='class_name' required placeholder='🏫 Class Name *' class='input-field'><input name='stream' required placeholder='🔀 Stream *' class='input-field'><button class='add-btn' style='margin-top:8px'>➕ Add Class</button></form></div></div></div></div></div></body></html>""")
...
def school_students(request: Request, success: str = "", student_name: str = ""):
    ...
    cur.execute("SELECT * FROM classes WHERE school_id=? ORDER BY name, stream", (school["id"],))
    classes = cur.fetchall()
    cur.execute("SELECT s.*, c.name as class_name, c.stream as class_stream FROM students s LEFT JOIN classes c ON s.class_id=c.id WHERE s.school_id=? ORDER BY s.id DESC", (school["id"],))
...
def add_student(request: Request, assessment_no: str = Form(...), student_name: str = Form(...), class_name: str = Form(...), stream: str = Form(...), gender: str = Form(...), parent_phone: str = Form(...)):
    ...
    cur.execute("INSERT INTO students (school_id, admission_no, assessment_no, name, class_id, gender, parent_phone, stream) VALUES (?,?,?,?,?,?,?,?)", (school["id"], assessment_no.strip().upper(), assessment_no.strip().upper(), student_name.strip().upper(), class_id, gender, parent_phone.strip(), stream.strip().upper()))
    con.commit()
    con.close()
...
def school_subjects(request: Request, success: str = "", subject_name: str = ""):
    ...
    con = get_db()
    cur = con.cursor()
    cur.execute("SELECT * FROM subjects WHERE school_id=? ORDER BY name", (school["id"],))
    subjects = cur.fetchall()
    con.close()
    success_html = ""
    if success == "added":
        success_html = f"""<div id='successToast' style='position:fixed; top:20px; left:50%; transform:translateX(-50%); background:#dcfce7; border:1.5px solid #16a34a; color:#166534; padding:14px 20px; border-radius:12px; font-weight:800; font-size:13px; z-index:9999'>✅ Subject Added! 📚</div><script>setTimeout(()=>{{let t=document.getElementById('successToast'); if(t) t.style.display='none';}}, 3500);</script>"""
    student_rows = ""
    for s in subjects:
        student_rows += f"""<tr id='row-{s['id']}' onclick='highlightRow(this)' style='cursor:pointer; border-bottom:1px solid #f1f5f9'><td style='padding:10px 12px; font-size:12px; font-weight:600'>📚 {s['name']}</td><td style='padding:10px 12px; font-size:11px'>{s['code'] or ''}</td><td style='padding:10px 12px; font-size:11px'>{s['initial'] or ''}</td><td style='padding:10px 12px; display:flex; gap:6px'><a href='/school/subjects/edit/{s["id"]}' style='background:#dbeafe; color:#1e40af; padding:4px 8px; border-radius:6px; text-decoration:none; font-size:12px; font-weight:700'>✏️</a><button onclick='openConfirm("delete", "{s["id"]}", "{s["name"]}")' style='background:#fee2e2; color:#991b1b; padding:4px 8px; border-radius:6px; text-decoration:none; font-size:12px; font-weight:700; border:none; cursor:pointer'>🗑️</button></td></tr>"""
    if not student_rows:
        student_rows = "<tr><td colspan='4' style='padding:40px; text-align:center; color:#94a3b8'>No subjects yet</td></tr>"
    header = school_header(school, name, "subjects")
    return HTMLResponse(f"""<html><head><meta name='viewport' content='width=device-width, initial-scale=1'><style>body{{margin:0;font-family:Arial;background:#f8fafc}}</style><script>let lastRow=null; function highlightRow(row){{if(lastRow) lastRow.classList.remove('highlight-row'); row.classList.add('highlight-row'); lastRow=row;}} let pendingAction=null; let pendingId=null; function openConfirm(action, id, name) {{pendingAction=action; pendingId=id; document.getElementById('confirmTitle').innerHTML='🗑️ Delete Subject?'; document.getElementById('confirmMsg').innerHTML='Are you sure you want to delete <b>'+name+'</b>?'; document.getElementById('confirmOverlay').style.display='flex';}} function closeConfirm(){{document.getElementById('confirmOverlay').style.display='none';}} function proceedConfirm(){{if(pendingId) window.location.href='/school/subjects/delete/'+pendingId;}}</script></head><body>{header}{success_html}<div id='confirmOverlay' class='confirm-overlay'><div class='confirm-box'><div id='confirmTitle' style='font-weight:900; font-size:16px; margin-bottom:8px'>Confirm?</div><div id='confirmMsg' style='font-size:12px; color:#475569; margin-bottom:18px'></div><div style='display:flex; gap:10px; justify-content:center'><button onclick='closeConfirm()' class='back-btn'>❌ Cancel</button><button onclick='proceedConfirm()' style='background:#dc2626; color:white; padding:10px 18px; border:none; border-radius:10px; font-weight:700; cursor:pointer; font-size:12px'>🗑️ Yes, Delete</button></div></div></div><div style='padding:18px; max-width:1200px; margin:auto'><div style='display:grid; grid-template-columns:1.7fr 0.7fr; gap:16px'><div style='background:white; border:1px solid #e2e8f0; border-radius:14px; overflow:hidden'><div style='padding:14px 16px; border-bottom:1px solid #f1f5f9; display:flex; justify-content:space-between'><b>📚 Subjects ({len(subjects)})</b><span style='font-size:11px; color:#64748b'>Click row = blue highlight</span></div><table style='width:100%; border-collapse:collapse'><thead><tr style='background:#f8fafc; text-align:left; font-size:11px; color:#64748b'><th style='padding:10px 12px'>Name</th><th>Code</th><th>Initial</th><th>Action</th></tr></thead><tbody>{student_rows}</tbody></table><div style='padding:12px'><a href='/school/dashboard' class='back-btn'>⬅️ Back to Overview</a></div></div><div style='background:white; border:1px solid #e2e8f0; border-radius:14px; padding:16px; height:fit-content'><b>➕ Add Subject</b><form method='post' action='/school/subjects/add' style='margin-top:10px'><input name='subject_name' required placeholder='📚 Subject Name *' class='input-field'><input name='code' placeholder='📝 Code' class='input-field'><input name='initial' placeholder='🔤 Initial e.g MAT' class='input-field'><button class='add-btn' style='margin-top:8px'>➕ Add Subject</button></form></div></div></div></div></div></body></html>""")
...
def add_subject(request: Request, subject_name: str = Form(...), code: str = Form(""), initial: str = Form("")):
    ...
    con.commit()
    con.close()
    return RedirectResponse("/school/subjects?success=added", status_code=303)
...
def edit_subject_page(sid: int, request: Request):
    ...
    con = get_db()
    cur = con.cursor()
    cur.execute("SELECT * FROM subjects WHERE id=? AND school_id=?", (sid, school["id"]))
    s = cur.fetchone()
    con.close()
    if not s:
        return RedirectResponse("/school/subjects")
    name = request.session.get("name","")
    header = school_header(school, name, "subjects")
    return HTMLResponse(f"""<html><body>{header}<div style='padding:20px; max-width:600px; margin:auto'><div style='background:white; border:1px solid #e2e8f0; border-radius:14px; padding:20px'><h3>✏️ Edit Subject - {s['name']}</h3><form method='post' action='/school/subjects/edit/{sid}'><input name='subject_name' value="{s['name']}" required class='input-field'><input name='code' value="{s['code'] or ''}" class='input-field'><input name='initial' value="{s['initial'] or ''}" class='input-field'><div style='display:flex; gap:10px; margin-top:12px'><button class='add-btn' style='flex:1'>💾 Save Changes</button><a href='/school/subjects' style='flex:1; text-align:center; padding:12px; background:white; border:1px solid #e2e8f0; border-radius:10px; text-decoration:none; color:#0f172a; font-weight:700'>❌ Cancel</a></div></form></div></div></div></div></body></html>""")

@app.post("/school/subjects/edit/{sid}")
def edit_subject_save(sid: int, request: Request, subject_name: str = Form(...), code: str = Form(""), initial: str = Form("")):
    school = get_school_obj(request)
    con = get_db()
    cur = con.cursor()
...
def add_exam(request: Request, exam_name: str = Form(...), term: str = Form(...), year: str = Form(...), exam_type: str = Form(...)):
    ...
    con.commit()
    con.close()
    return RedirectResponse("/school/exams?success=added", status_code=303)
...
def edit_exam_page(eid: int, request: Request):
    ...
    con = get_db()
    cur = con.cursor()
    cur.execute("SELECT * FROM exams WHERE id=? AND school_id=?", (eid, school["id"]))
    e = cur.fetchone()
    con.close()
    if not e:
        return RedirectResponse("/school/exams")
    name = request.session.get("name","")
    header = school_header(school, name, "exams")
...
def delete_exam(eid: int, request: Request):
    ...
    con = get_db()
    cur = con.cursor()
    cur.execute("DELETE FROM exams WHERE id=? AND school_id=?", (eid, school["id"]))
    con.commit()
    con.close()
    return PlainTextResponse("ok")
...
def teachers_page(request: Request):
    ...
    con = get_db()
    cur = con.cursor()
    cur.execute("SELECT * FROM teachers WHERE school_id=? ORDER BY id DESC", (school["id"],))
    ...
    if not rows:
        rows = "<tr><td colspan='6' style='padding:30px; text-align:center; color:#94a3b8'>No teachers yet</td></tr>"
    html = f"""
...
def add_teacher(request: Request, name: str = Form(...), tsc_no: str = Form(...), id_no: str = Form(""), gender: str = Form(...), phone: str = Form(...), email: str = Form("")):
    ...
    con = get_db()
    cur = con.cursor()
    cur.execute("INSERT INTO teachers (school_id,name,email,phone,tsc_no,gender,id_no) VALUES (?,?,?,?,?,?,?)", (school["id"],name,email,phone,tsc_no,gender,id_no))
    con.commit()
    con.close()
...
def delete_teacher(request: Request, tid: int):
    ...
    cur.execute("DELETE FROM teacher_allocations WHERE teacher_id=? AND school_id=?", (tid, school["id"]))
    con.commit()
    con.close()
    return PlainTextResponse("ok")
...
def teacher_allocation_page(request: Request):
    ...
    con = get_db()
    cur = con.cursor()
    cur.execute("SELECT * FROM teachers WHERE school_id=? ORDER BY name", (school["id"],))
    teachers = cur.fetchall()
    cur.execute("SELECT * FROM subjects WHERE school_id=? ORDER BY name", (school["id"],))
    subjects = cur.fetchall()
    cur.execute("SELECT * FROM classes WHERE school_id=? ORDER BY name", (school["id"],))
    classes = cur.fetchall()
    cur.execute("SELECT ta.*, t.name as teacher_name, s.name as subject_name, c.name as class_name FROM teacher_allocations ta LEFT JOIN teachers t ON ta.teacher_id=t.id LEFT JOIN subjects s ON ta.subject_id=s.id LEFT JOIN classes c ON ta.class_id=c.id WHERE ta.school_id=? ORDER BY ta.id DESC", (school["id"],))
    allocs = cur.fetchall()
    con.close()
    ...
    if not rows:
        rows = "<tr><td colspan='5' style='padding:30px; text-align:center; color:#94a3b8'>No allocations yet</td></tr>"
    html = f"""
    <div style='padding:18px; max-width:1200px; margin:auto'>
      <a href='/school/dashboard' class='back-btn' style='margin-bottom:14px'>⬅️ Back to Dashboard</a>
...
def add_allocation(request: Request, teacher_id: int = Form(...), subject_id: str = Form(""), class_id: str = Form(""), role: str = Form(...)):
    ...
    cur.execute("INSERT INTO teacher_allocations (school_id,teacher_id,subject_id,class_id,role) VALUES (?,?,?,?,?)", (school["id"],teacher_id,sid,cid,role))
    con.commit()
    con.close()
...
def school_generic(page: str, request: Request):
    if page in ["teachers","teacher-allocation","dashboard","classes","students","subjects","exams"]:
        return RedirectResponse(f"/school/{page}", status_code=303)
    if "email" not in request.session or request.session.get("role")!="school_admin":
        return RedirectResponse("/")
    school = get_school_obj(request)
    name = request.session.get("name","")
