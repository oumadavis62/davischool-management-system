    ...
    html += f"""
        <div style='padding:20px'>
            ...
            <div style='display:grid; grid-template-columns:2fr 1fr; gap:16px; margin-top:16px'>
                ...
                <div style='background:#0f172a; border-radius:14px; padding:16px; color:white'><b>📊 Zeraki Analytics</b><div style='margin-top:12px; background:#1e293b; border-radius:10px; padding:12px'><div style='font-size:11px; color:#94a3b8'>STUDENT REPORT</div><div style='margin-top:8px'><div style='display:flex; align-items:center; gap:8px; margin-bottom:6px'><span style='width:24px; height:24px; background:#3b82f6; border-radius:50%; display:inline-flex; align-items:center; justify-content:center; font-size:10px'>👤</span><span style='font-size:11px'>Top Student</span><span style='margin-left:auto; background:#22c55e; padding:2px 6px; border-radius:4px; font-size:10px'>G</span></div><div style='display:flex; align-items:center; gap:8px; margin-bottom:6px'><span style='width:24px; height:24px; background:#ec4899; border-radius:50%; display:inline-flex; align-items:center; justify-content:center; font-size:10px'>👤</span><span style='font-size:11px'>Second</span><span style='margin-left:auto; background:#3b82f6; padding:2px 6px; border-radius:4px; font-size:10px'>B</span></div><div style='display:flex; align-items:center; gap:8px'><span style='width:24px; height:24px; background:#f59e0b; border-radius:50%; display:inline-flex; align-items:center; justify-content:center; font-size:10px'>👤</span><span style='font-size:11px'>Third</span><span style='margin-left:auto; background:#f59e0b; padding:2px 6px; border-radius:4px; font-size:10px'>C</span></div></div></div><div style='margin-top:12px; font-size:10px; color:#94a3b8'>CONTACT US: 0111392013 | WhatsApp: 0111392013 — GET EVERYTHING DONE IN MINUTES!</div></div>
            </div>
        </div>
    </div></div>
    """
...
# MARKSHEETS, RANKING, ANALYSIS, REPORTS - ZERAKI STYLE
...
def school_marksheets(request: Request):
    ...
    html += f"<div style='padding:20px'><h3>📄 Generates MarkSheets - CREATE ACCURATE RECORD SHEETS QUICKLY</h3><p style='font-size:12px; color:#64748b'>Select exam to generate class-wise marksheets (like Zeraki)</p><div style='margin-top:16px'>{content}</div></div></div></div>"
    return HTMLResponse(f"<html><body style='margin:0; font-family:Arial; background:#f8fafc'>{html}</body></html>")
...
def school_reports(request: Request):
    ...
    html += f"<div style='padding:20px'><h3>📑 STUDENT REPORTS - AUTOMATED REPORT CARD GENERATION</h3><p style='font-size:11px; color:#64748b'>Zeraki style - One click report cards with grades, ranking, remarks</p><div style='display:grid; grid-template-columns:repeat(3,1fr); gap:12px; margin-top:16px'>{cards}</div></div></div></div>"
    return HTMLResponse(f"<html><body style='margin:0; font-family:Arial; background:#f8fafc'>{html}</body></html>")
...
def school_sms(request: Request):
    ...
    table = f"<b>💬 Bulk SMS to Parents - EASY BULK SMS NOTIFICATIONS ({sc} parents)</b><div style='margin-top:8px; background:#f0fdf4; border:1px solid #bbf7d0; padding:8px; border-radius:8px; font-size:11px'>✅ Will send to all {sc} parents - Fees, Exams, Events alerts</div><table style='width:100%; margin-top:12px; border-collapse:collapse'><tr style='background:#f8fafc; font-size:11px'><th style='padding:10px; text-align:left'>To</th><th style='padding:10px; text-align:left'>Message</th><th style='padding:10px; text-align:left'>Time</th></tr>{rows}</table>"
    form = f"<form method='post' action='/school/sms/send' style='display:flex; flex-direction:column; gap:10px; margin-top:12px'><select name='target' required style='padding:11px; border:1px solid #e2e8f0; border-radius:8px'><option value='all'>📢 All Parents ({sc})</option><option value='fees'>💰 Fees Defaulters</option><option value='exam'>📝 Exam Notifications</option></select><textarea name='message' placeholder='💬 Type SMS message e.g. Dear Parent, Term 2 fees balance is KES 2000. Please clear by Friday. - {school['name']}' required style='padding:11px; border:1px solid #e2e8f0; border-radius:8px; height:100px'></textarea><button style='background:#16a34a; color:white; padding:12px; border:none; border-radius:8px; font-weight:700'>📤 Send Bulk SMS Now</button><div style='font-size:10px; color:#64748b; text-align:center'>CONTACT US: 0111392013 | WhatsApp: 0111392013</div></form>"
...
# SUPER ADMIN MANAGE SCHOOLS - UNTOUCHED v23.4
...
def profile(request: Request, tab: str = "personal"):
    ...
    if is_super:
        ...
        hdr = f"<div style='background:white; border-bottom:1px solid #e2e8f0; padding:10px 20px'><b>🏫 {sname}</b> - {name}</div>"
    ...
    if is_super:
        ...
        html = f"<html><head><meta name='viewport' content='width=device-width, initial-scale=1'></head><body style='margin:0; font-family:Arial; background:#f8fafc'><div style='background:white; padding:12px 20px; border-bottom:1px solid #e2e8f0'><b>🏫 {sname}</b> - {name}</div><div style='padding:20px; max-width:1100px; margin:0 auto'><h2>👤 My Profile - {sname}</h2><div style='display:flex; gap:16px; margin-top:16px'><div style='background:white; border:1px solid #e2e8f0; border-radius:12px; padding:16px; width:260px; text-align:center'><div style='width:80px; height:80px; background:#dcfce7; color:#166534; border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:28px; font-weight:800; margin:0 auto'>{initials}</div><div style='font-weight:700; margin-top:10px'>{name}</div><div style='font-size:11px; color:#64748b'>{email}</div></div><div style='flex:1; background:white; border:1px solid #e2e8f0; border-radius:12px; padding:20px'>{right}</div></div></div></body></html>"
