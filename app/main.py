...
app = FastAPI()
app.add_middleware(SessionMiddleware, secret_key="davischool-v43-global-school-style-final")
...
# ===== GLOBAL CONTROL EXACTLY LIKE SCHOOL SYSTEM SIDE — WITH PUSH BUTTON AT EVERY WINDOW + BROADCAST =====
...
def global_control(request: Request):
    ...
    cur.execute("SELECT name, stream, COUNT(*) as cnt FROM classes GROUP BY name, stream ORDER BY name"); g_classes = cur.fetchall()
    cur.execute("SELECT name, code, initial, COUNT(*) as cnt FROM subjects GROUP BY name ORDER BY name"); g_subjects = cur.fetchall()
    cur.execute("SELECT term_name, year, start_date, end_date, COUNT(*) as cnt FROM terms GROUP BY term_name, year ORDER BY year DESC"); g_terms = cur.fetchall()
    cur.execute("SELECT name, term, year, exam_type, COUNT(*) as cnt FROM exams GROUP BY name, year ORDER BY year DESC"); g_exams = cur.fetchall()
    cur.execute("SELECT * FROM global_notices ORDER BY id DESC LIMIT 10"); notices = cur.fetchall()
    con.close()

    # Build rows exactly like school side tables
    class_rows = "".join([f"<tr style='border-bottom:1px solid #f1f5f9'><td style='padding:10px 12px;font-size:12px'>{c['name']}</td><td style='padding:10px 12px;font-size:12px'>{c['stream'] or ''}</td><td style='padding:10px 12px'><span style='background:#dcfce7;color:#166534;padding:3px 8px;border-radius:12px;font-size:10px'>In {c['cnt']} schools</span></td><td style='padding:10px 12px'><a href='/super/global-control/delete-class/{c['name']}/{c['stream'] or ''}' style='background:#fee2e2;color:#991b1b;padding:4px 8px;border-radius:6px;text-decoration:none;font-size:11px'>🗑️</a></td></tr>" for c in g_classes]) or "<tr><td colspan='4' style='padding:30px;text-align:center;color:#94a3b8'>No classes yet</td></tr>"
    subj_rows = "".join([f"<tr style='border-bottom:1px solid #f1f5f9'><td style='padding:10px 12px;font-size:12px'>{s['name']}</td><td style='padding:10px 12px;font-size:12px'>{s['code'] or ''}</td><td style='padding:10px 12px;font-size:12px'>{s['initial'] or ''}</td><td style='padding:10px 12px'><span style='background:#dbeafe;color:#1e40af;padding:3px 8px;border-radius:12px;font-size:10px'>{s['cnt']} schools</span></td></tr>" for s in g_subjects]) or "<tr><td colspan='4' style='padding:30px;text-align:center;color:#94a3b8'>No subjects yet</td></tr>"
    term_rows = "".join([f"<tr style='border-bottom:1px solid #f1f5f9'><td style='padding:10px 12px;font-size:12px'>{t['term_name']}</td><td style='padding:10px 12px;font-size:12px'>{t['year']}</td><td style='padding:10px 12px;font-size:11px'>{t['start_date']}</td><td style='padding:10px 12px;font-size:11px'>{t['end_date']}</td></tr>" for t in g_terms]) or "<tr><td colspan='4' style='padding:20px;text-align:center;color:#94a3b8'>No terms</td></tr>"
    exam_rows = "".join([f"<tr style='border-bottom:1px solid #f1f5f9'><td style='padding:10px 12px;font-size:12px'>{e['name']}</td><td style='padding:10px 12px;font-size:12px'>{e['term']}</td><td style='padding:10px 12px;font-size:12px'>{e['year']}</td><td style='padding:10px 12px'><span style='background:#0f172a;color:white;padding:3px 8px;border-radius:12px;font-size:10px'>{e['exam_type'] or ''}</span></td></tr>" for e in g_exams]) or "<tr><td colspan='4' style='padding:20px;text-align:center;color:#94a3b8'>No exams</td></tr>"
    notice_rows = "".join([f"<div style='padding:12px 0;border-bottom:1px solid #f1f5f9;display:flex;justify-content:space-between;gap:12px'><div><div style='font-weight:700;font-size:12px'>📢 {n['message']}</div><div style='font-size:10px;color:#64748b'>{n['created_at']}</div></div><a href='/super/global-control/delete-notice/{n['id']}' style='color:#dc2626;text-decoration:none;font-size:12px'>🗑️</a></div>" for n in notices]) or "<div style='padding:30px;text-align:center;color:#94a3b8'>No broadcast notices yet 📭</div>"
...
.push-btn{{width:100%;background:#0f172a;color:white;padding:12px;border:none;border-radius:10px;font-weight:800;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;margin-top:8px}}
.push-btn:hover{{background:#1e293b}}
</style></head><body>
{header_html(initials, name, email)}
<div style='padding:18px;max-width:1600px;margin:auto'>
<div style='margin-bottom:18px'><h2 style='margin:0;font-size:22px;font-weight:800'>🌍 Global School Control</h2><p style='margin:4px 0 0;color:#64748b;font-size:13px'>Edit once → updates ALL {total_schools} schools — No impersonation needed — Exactly like school system side</p></div>

<!-- SCHOOL SIDE STYLE: 2 columns like /school/classes -->
<div style='display:grid;grid-template-columns:1.7fr 0.7fr;gap:16px;margin-bottom:16px'>
  <div class='card'>
    <div style='padding:14px 16px;border-bottom:1px solid #f1f5f9;display:flex;justify-content:space-between;align-items:center'><b>🏫 Global Classes ({len(g_classes)}) — Same as School Classes Window</b><span style='background:#0f172a;color:white;padding:4px 10px;border-radius:12px;font-size:10px'>🌍 Push to ALL</span></div>
    <div style='overflow:auto;max-height:320px'><table style='width:100%;border-collapse:collapse'><thead><tr style='background:#f8fafc;text-align:left;font-size:11px;color:#64748b'><th style='padding:10px 12px'>CLASS</th><th style='padding:10px 12px'>STREAM</th><th style='padding:10px 12px'>SCHOOLS</th><th style='padding:10px 12px'>ACTION</th></tr></thead><tbody>{class_rows}</tbody></table></div>
    <div style='padding:12px;border-top:1px solid #f1f5f9'><a href='/schools/manage' style='padding:8px 12px;background:white;border:1px solid #e2e8f0;border-radius:8px;text-decoration:none;color:#0f172a;font-weight:700;font-size:11px'>⬅️ Back</a></div>
  </div>
  <div class='card' style='padding:16px;height:fit-content'>
    <div style='font-weight:800;margin-bottom:10px'>➕ Add Global Class</div>
    <form method='post' action='/super/global-control/add-class'>
      <input name='class_name' required placeholder='Class e.g. GRADE 7' class='input-field'>
      <input name='stream' required placeholder='Stream e.g. EAST' class='input-field'>
      <button class='push-btn'>🌍 Push to ALL {total_schools} Schools</button>
    </form>
    <div style='margin-top:12px;background:#f0fdf4;padding:10px;border-radius:10px;font-size:11px;color:#166534'>✅ Same window as school side Classes & Streams — with Push to ALL button</div>
  </div>
</div>

<div style='display:grid;grid-template-columns:1.7fr 0.7fr;gap:16px;margin-bottom:16px'>
  <div class='card'>
    <div style='padding:14px 16px;border-bottom:1px solid #f1f5f9;display:flex;justify-content:space-between'><b>📚 Global Subjects ({len(g_subjects)}) — Same as School Subjects Window</b><span style='background:#0f172a;color:white;padding:4px 10px;border-radius:12px;font-size:10px'>🌍 Push to ALL</span></div>
    <div style='overflow:auto;max-height:320px'><table style='width:100%;border-collapse:collapse'><thead><tr style='background:#f8fafc;text-align:left;font-size:11px;color:#64748b'><th style='padding:10px 12px'>NAME</th><th style='padding:10px 12px'>CODE</th><th style='padding:10px 12px'>INITIAL</th><th style='padding:10px 12px'>SCHOOLS</th></tr></thead><tbody>{subj_rows}</tbody></table></div>
  </div>
  <div class='card' style='padding:16px;height:fit-content'>
    <div style='font-weight:800;margin-bottom:10px'>➕ Add Global Subject</div>
    <form method='post' action='/super/global-control/add-subject'>
      <input name='subject_name' required placeholder='Subject e.g. MATHEMATICS' class='input-field'>
      <input name='code' placeholder='Code MAT' class='input-field'>
      <input name='initial' placeholder='Initial M' class='input-field'>
      <button class='push-btn'>🌍 Push to ALL {total_schools} Schools</button>
    </form>
    <div style='margin-top:12px;background:#eff6ff;padding:10px;border-radius:10px;font-size:11px;color:#1e40af'>✅ Same as school Subjects window — with Push to ALL</div>
  </div>
</div>

<div style='display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:16px'>
  <div class='card'>
    <div style='padding:14px 16px;border-bottom:1px solid #f1f5f9'><b>📅 Global Terms — Same as Dean Settings Window</b></div>
    <div style='padding:16px'>
      <form method='post' action='/super/global-control/add-term' style='display:grid;grid-template-columns:1fr 1fr 1fr 1fr auto;gap:8px;align-items:end'>
        <select name='term_name' required class='input-field'><option>Term 1</option><option>Term 2</option><option>Term 3</option></select>
        <input name='year' required placeholder='2026' class='input-field'>
        <input name='start_date' type='date' required class='input-field'>
        <input name='end_date' type='date' required class='input-field'>
        <button class='add-btn' style='padding:11px'>🌍 Push Term to ALL</button>
      </form>
      <div style='overflow:auto;max-height:200px;margin-top:12px;border:1px solid #f1f5f9;border-radius:10px'><table style='width:100%;border-collapse:collapse'><thead><tr style='background:#f8fafc;text-align:left;font-size:11px;color:#64748b'><th style='padding:8px 12px'>TERM</th><th>YEAR</th><th>START</th><th>END</th></tr></thead><tbody>{term_rows}</tbody></table></div>
    </div>
  </div>
  <div class='card'>
    <div style='padding:14px 16px;border-bottom:1px solid #f1f5f9'><b>📝 Global Exams — Same as Exam Settings Window</b></div>
    <div style='padding:16px'>
      <form method='post' action='/super/global-control/add-exam' style='display:grid;grid-template-columns:1fr 1fr 1fr 1fr auto;gap:8px;align-items:end'>
        <input name='exam_name' required placeholder='Exam e.g. END TERM' class='input-field'>
        <select name='term' required class='input-field'><option>Term 1</option><option>Term 2</option><option>Term 3</option></select>
        <input name='year' required placeholder='2026' class='input-field'>
        <select name='exam_type' required class='input-field'><option>Main Exam</option><option>End Term Exam</option></select>
        <button class='add-btn' style='padding:11px'>🌍 Push Exam to ALL</button>
      </form>
      <div style='overflow:auto;max-height:200px;margin-top:12px;border:1px solid #f1f5f9;border-radius:10px'><table style='width:100%;border-collapse:collapse'><thead><tr style='background:#f8fafc;text-align:left;font-size:11px;color:#64748b'><th style='padding:8px 12px'>NAME</th><th>TERM</th><th>YEAR</th><th>TYPE</th></tr></thead><tbody>{exam_rows}</tbody></table></div>
    </div>
  </div>
</div>

<!-- BROADCAST NOTICE — SAME AS SCHOOL SIDE BANNER + CARD -->
<div style='display:grid;grid-template-columns:1.7fr 0.7fr;gap:16px'>
  <div class='card' style='padding:16px'>
    <div style='font-weight:800;margin-bottom:10px'>📢 Broadcast Notice to ALL Schools — Same as School Side Notice</div>
    <form method='post' action='/super/global-control/add-notice'>
      <input name='message' required placeholder='Type notice e.g. System maintenance tonight 10pm — will show on every school dashboard' class='input-field'>
      <button class='push-btn'>📢 Broadcast Notice to ALL {total_schools} Schools</button>
    </form>
    <div style='margin-top:14px;border:1px solid #f1f5f9;border-radius:10px;max-height:220px;overflow:auto;padding:0 12px'>{notice_rows}</div>
  </div>
  <div class='card' style='padding:16px;height:fit-content'>
    <div style='font-weight:800;margin-bottom:10px'>⚡ Global Actions</div>
    <div style='font-size:12px;color:#64748b;margin-bottom:12px'>Total Schools: {total_schools} — Every window has Push to ALL</div>
    <a href='/schools/manage' style='display:block;text-align:center;background:white;border:1px solid #e2e8f0;padding:10px;border-radius:10px;text-decoration:none;color:#0f172a;font-weight:600;font-size:13px;margin-bottom:10px'>🏫 Manage Schools (👁️ View)</a>
    <a href='/dashboard' style='display:block;text-align:center;background:#0f172a;color:white;padding:10px;border-radius:10px;text-decoration:none;font-weight:700;font-size:13px'>⬅️ Back to Overview</a>
    <div style='margin-top:12px;background:#f0fdf4;padding:12px;border-radius:10px;font-size:11px;line-height:1.5'>✅ <b>Exactly like school side:</b><br>• Classes window = same as /school/classes<br>• Subjects = same as /school/subjects<br>• Terms = same as Dean Settings<br>• Exams = same as Exam Settings<br>• Each has 🌍 Push to ALL Schools button<br>• Broadcast Notice shows on every school dashboard like school banner</div>
  </div>
</div>
...
def global_add_notice(request: Request, message: str = Form(...)):
    ...
    con.commit(); con.close()
    log_activity(request.session.get("email"), f"📢 Broadcast: {message[:30]}", "To all schools")
    return RedirectResponse("/super/global-control",303)
...
@app.get("/super/global-control/delete-class/{cname}/{stream}")
def delete_global_class(cname: str, stream: str, request: Request):
    if request.session.get("role")!="super_admin": return RedirectResponse("/")
    con = get_db(); cur = con.cursor()
    cur.execute("DELETE FROM classes WHERE name=? AND (stream=? OR?='')", (cname, stream, stream))
    con.commit(); con.close()
    return RedirectResponse("/super/global-control",303)
...
def school_other(path: str, request: Request):
    ...
    return HTMLResponse(f"<html><body>{header}<div style='padding:30px'><div style='background:white;border:1px solid #e2e8f0;border-radius:14px;padding:40px;text-align:center'><h3>🚧 {path.upper()} — placeholder (original intact)</h3><p style='color:#64748b; font-size:13px'>This page is inside Academic Manager — with Push to ALL button available in Global Control</p><a href='/school/dashboard' style='padding:10px 16px; background:white; border:1px solid #e2e8f0; border-radius:10px; text-decoration:none; color:#0f172a; font-weight:700'>⬅️ Back to Overview</a></div></div></div></div></body></html>")
