...
.dropdown-item {{ display:flex; align-items:center; gap:10px; padding:11px 14px; text-decoration:none; font-size:13px; cursor:pointer; }}
...
def dashboard(request: Request):
    ...
    content = f"<div style='padding:24px'><h2>📊 School Overview</h2><p>Welcome {name}</p><div style='display:grid; grid-template-columns:repeat(4,1fr); gap:16px;'><div style='background:white; border:1px solid #e2e8f0; border-radius:14px; padding:18px'>🏫 TOTAL SCHOOLS<br><b style='font-size:24px'>{total}</b></div><div style='background:white; border:1px solid #e2e8f0; border-radius:14px; padding:18px'>✅ ACTIVE<br><b style='font-size:24px'>{total}</b></div><div style='background:white; border:1px solid #e2e8f0; border-radius:14px; padding:18px'>💰 REVENUE<br><b style='font-size:24px'>KES {total*15000:,}</b></div><div style='background:white; border:1px solid #e2e8f0; border-radius:14px; padding:18px'>🎓 STUDENTS<br><b style='font-size:24px'>{total*350:,}</b></div></div><div style='background:white; border:1px solid #e2e8f0; border-radius:14px; margin-top:16px'><table style='width:100%; border-collapse:collapse'><tr style='background:#f8fafc'><th>Name</th><th>Location</th><th>Status</th><th>Date</th></tr>{rows}</table></div></div>"
...
# RESTORED SCHOOLS MANAGE - EXACT SCREENSHOT + 6-DIGIT + EYE + OK/CANCEL
...
# SCHOOL SIDE ROUTES - FULL ZERAKI
...
def school_dashboard(request: Request):
    ...
    html += f"<div style='padding:20px'><h2>MATOKEO ANALYTICS 🚀</h2><p>Total Students {sc} | Classes {cc} | Exams {ec}</p></div></div></div>"
...
def school_students(request: Request):
    ...
    students = cur.fetchall(); cur.execute("SELECT * FROM classes WHERE school_id=?", (school["id"],)); classes = cur.fetchall(); con.close()
    ...
    html += f"<div style='padding:20px'><b>🎓 Students ({len(students)})</b><table style='width:100%; border-collapse:collapse'><tr style='background:#f8fafc'><th>Name</th><th>Adm</th><th>Class</th><th>Parent Phone</th><th>Action</th></tr>{rows}</table><form method='post' action='/school/students/add' style='margin-top:12px'><input name='admission_no' required placeholder='Adm No' style='padding:8px; border:1px solid #e2e8f0; border-radius:8px'><input name='student_name' required placeholder='Student Name' style='padding:8px; border:1px solid #e2e8f0; border-radius:8px'><select name='class_id' style='padding:8px'>{opts}</select><select name='gender' style='padding:8px'><option>Male</option><option>Female</option></select><input name='parent_name' placeholder='Parent' style='padding:8px; border:1px solid #e2e8f0; border-radius:8px'><input name='parent_phone' required placeholder='Parent Phone' style='padding:8px; border:1px solid #e2e8f0; border-radius:8px'><button style='background:#0f172a; color:white; padding:8px 14px; border-radius:8px; border:none'>Add Student</button></form></div></div></div>"
...
def school_classes(request: Request):
    ...
    html += f"<div style='padding:20px'><table style='width:100%'><tr><th>Class</th><th>Level</th><th>Action</th></tr>{rows}</table><form method='post' action='/school/classes/add'><input name='class_name' required placeholder='Class Name'><select name='level'><option>Primary</option><option>Junior School</option><option>Senior School</option></select><button>Add Class</button></form></div></div></div>"
    return HTMLResponse(f"<html><body>{html}</body></html>")
...
def school_subjects(request: Request):
    ...
    html += f"<div style='padding:20px'><b>📚 Subjects</b><table><tr><th>Subject</th><th>Code</th><th>Action</th></tr>{rows}</table><form method='post' action='/school/subjects/add'><input name='subject_name' required placeholder='Subject'><input name='subject_code' placeholder='Code'><button>Add</button></form></div></div></div>"
    return HTMLResponse(f"<html><body>{html}</body></html>")
...
def school_marksheets(request: Request):
    ...
    content = "".join([f"<div style='background:white; padding:14px; border-radius:12px'><b>{e['name']}</b><a href='/school/marksheets/{e['id']}' style='display:block; background:#0f172a; color:white; padding:8px; border-radius:8px; text-align:center; margin-top:10px; text-decoration:none'>Generate</a></div>" for e in exams]) or "No exams"
...
def school_ranking(request: Request):
    ...
    rows = ""
    if ex:
        cur.execute("SELECT s.name, SUM(m.score) total FROM students s JOIN marks m ON s.id=m.student_id WHERE m.exam_id=? AND m.school_id=? GROUP BY s.id ORDER BY total DESC", (ex["id"], school["id"]))
        for idx, r in enumerate(cur.fetchall(), 1):
            rows += f"<tr><td>{idx}</td><td>{r['name']}</td><td><b>{r['total']}</b></td></tr>"
    con.close()
    html = school_header(school, request.session.get("name",""), "ranking")
    html += f"<div style='padding:20px'><table><tr><th>Rank</th><th>Student</th><th>Total</th></tr>{rows or '<tr><td colspan=3>No data</td></tr>'}</table></div></div></div>"
    return HTMLResponse(f"<html><body>{html}</body></html>")
...
def school_analysis(request: Request):
    ...
    html += f"<div style='padding:20px'><b>📈 Analysis Avg: {int(avg)}%</b></div></div></div>"
    return HTMLResponse(f"<html><body>{html}</body></html>")
...
def school_reports(request: Request):
    ...
    cards = "".join([f"<div style='background:white; padding:14px; border-radius:12px'><b>{s['name']}</b><a href='/school/report/{s['id']}' style='display:block; background:#0f172a; color:white; padding:8px; border-radius:8px; text-align:center; margin-top:10px; text-decoration:none'>Report</a></div>" for s in students]) or "No students"
    html = school_header(school, request.session.get("name",""), "reports")
    html += f"<div style='padding:20px'><div style='display:grid; grid-template-columns:repeat(3,1fr); gap:12px'>{cards}</div></div></div></div>"
    return HTMLResponse(f"<html><body>{html}</body></html>")

@app.get("/school/fees", response_class=HTMLResponse)
def school_fees(request: Request):
    school = get_school_obj(request); con = get_db(); cur = con.cursor(); cur.execute("SELECT * FROM fees WHERE school_id=?", (school["id"],)); fees = cur.fetchall(); cur.execute("SELECT id, name FROM students WHERE school_id=?", (school["id"],)); students = cur.fetchall(); con.close()
    html = school_header(school, request.session.get("name",""), "fees")
    html += f"<div style='padding:20px'>Fees {len(fees)}</div></div></div>"
    return HTMLResponse(f"<html><body>{html}</body></html>")

@app.get("/school/timetable", response_class=HTMLResponse)
def school_timetable(request: Request):
    school = get_school_obj(request); html = school_header(school, request.session.get("name",""), "timetable")
    html += f"<div style='padding:20px'>Timetable</div></div></div>"
    return HTMLResponse(f"<html><body>{html}</body></html>")

@app.get("/school/sms", response_class=HTMLResponse)
def school_sms(request: Request):
    school = get_school_obj(request); html = school_header(school, request.session.get("name",""), "sms")
    html += f"<div style='padding:20px'>Bulk SMS</div></div></div>"
    return HTMLResponse(f"<html><body>{html}</body></html>")
