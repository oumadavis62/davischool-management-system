...
BUILD_COMMIT = "c27c393dac7536cd015c5a98c338ca98fc7e212d"
...
def school_timetable(request: Request):
    school=get_school_obj(request)
    if not school or not _role_permission(request,"timetable.view"):
        return RedirectResponse("/portal" if request.session.get("role") not in ("school_admin","super_admin") else "/",303)
    con=get_db(); cur=con.cursor(); school_id=school["id"]
    classes=cur.execute("SELECT id,name,stream FROM classes WHERE school_id=? ORDER BY name,stream",(school_id,)).fetchall()
    class_id=request.query_params.get("class_id","")
    selected_class=None
    if str(class_id).isdigit():
        selected_class=cur.execute("SELECT id,name,stream FROM classes WHERE id=? AND school_id=?",(int(class_id),school_id)).fetchone()
    rows=cur.execute("SELECT * FROM timetable WHERE school_id=? ORDER BY CASE day WHEN 'Monday' THEN 1 WHEN 'Tuesday' THEN 2 WHEN 'Wednesday' THEN 3 WHEN 'Thursday' THEN 4 WHEN 'Friday' THEN 5 ELSE 6 END,start_time,id",(school_id,)).fetchall()
    subjects=cur.execute("SELECT id,name FROM subjects WHERE school_id=? ORDER BY name",(school_id,)).fetchall()
    teachers=cur.execute("SELECT id,name FROM teachers WHERE school_id=? ORDER BY name",(school_id,)).fetchall()
    con.close()
    msg=request.query_params.get("msg","")
    class_opts="".join(f"<option value='{c['id']}' {'selected' if selected_class and int(selected_class['id'])==int(c['id']) else ''}>{escape(c['name'])} {escape(c['stream'] or '')}</option>" for c in classes)
    subject_opts="".join(f"<option value='{s['id']}'>{escape(s['name'])}</option>" for s in subjects)
    teacher_opts="<option value=''>-- Optional teacher --</option>"+"".join(f"<option value='{t['id']}'>{escape(t['name'])}</option>" for t in teachers)
    ...
    weekly=""
    if selected_class:
        selected_rows=[r for r in rows if str(r["class_name"]).strip()==str(selected_class["name"]).strip() and str(r["stream"] or "").strip()==str(selected_class["stream"] or "").strip()]
        by_slot={}
        for r in selected_rows:
            by_slot.setdefault((r["day"],r["start_time"],r["end_time"]),[]).append(r)
        slot_keys=sorted(by_slot.keys(), key=lambda x: (["Monday","Tuesday","Wednesday","Thursday","Friday"].index(x[0]) if x[0] in ["Monday","Tuesday","Wednesday","Thursday","Friday"] else 9,x[1]))
        weekly_rows=""
        for key in slot_keys:
            day,st,et=key
            cells=[]
            for d in ["Monday","Tuesday","Wednesday","Thursday","Friday"]:
                found=[r for r in selected_rows if r["day"]==d and r["start_time"]==st and r["end_time"]==et]
                textv="<br>".join(f"<b>{escape(str(r['subject']))}</b>{('<br>'+escape(str(r['teacher']))) if r['teacher'] else ''}" for r in found) or "—"
                cells.append(f"<td style='min-width:150px'>{textv}</td>")
            weekly_rows+=f"<tr><th style='white-space:nowrap'>{escape(st)}–{escape(et)}</th>{''.join(cells)}</tr>"
        weekly=f"<div class='card'><div class='toolbar'><div><h3 style='margin:0'>📅 Weekly Timetable — {escape(selected_class['name'])} {escape(selected_class['stream'] or '')}</h3><p style='margin:4px 0'>Class/stream view generated from the saved timetable.</p></div><button class='btn' onclick='window.print()'>🖨️ Print Timetable</button></div>"
        weekly+=f"<div style='overflow:auto'><table class='timetable-grid'><tr><th>TIME</th><th>MONDAY</th><th>TUESDAY</th><th>WEDNESDAY</th><th>THURSDAY</th><th>FRIDAY</th></tr>{weekly_rows or '<tr><td colspan=6 style="text-align:center;padding:30px">No timetable entries for this class yet.</td></tr>'}</table></div></div>"

    message=f"<div class='card' style='background:#ecfdf5;border-color:#a7f3d0;color:#065f46'>✅ {escape(msg)}</div>" if msg else ""
    body=f"""<style>
.timetable-grid td,.timetable-grid th{{padding:12px;border:1px solid #e5e7eb;vertical-align:top}}.timetable-grid th{{background:#f8fafc;font-size:11px}}
.timetable-grid td{{font-size:12px;line-height:1.45}}.danger{{background:#fee2e2!important;color:#991b1b!important}}
@media print{{.side,.top,.page-head,.no-print{{display:none!important}}body{{background:white!important}}.main{{margin:0!important}}.mod{{padding:0!important;max-width:none!important}}.card{{box-shadow:none!important;border:0!important}}}}
</style>
<div class='page-head'><div><div class='eyebrow'>School Operations</div><h1>🗓️ Smart Timetable</h1><p>Create, generate, manage and print the school's weekly timetable.</p></div></div>
{message}
<div class='card no-print'><h3>⚡ Generate Weekly Timetable</h3>
<p>Select a class/stream and let DaviSchool distribute its subjects across the week. Existing entries for that class can be replaced safely.</p>
<form method='post' action='/school/modules/timetable/generate' style='display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px'>
<select name='class_id' required class='input-field'><option value=''>-- Select Class / Stream --</option>{class_opts}</select>
<input name='start_time' type='time' value='08:00' required class='input-field' title='First period start'>
<input name='period_minutes' type='number' min='20' max='180' value='40' required class='input-field' placeholder='Minutes per period'>
<input name='periods_per_day' type='number' min='1' max='12' value='7' required class='input-field' placeholder='Periods per day'>
<input name='break_after' type='number' min='0' max='12' value='4' required class='input-field' placeholder='Break after period'>
<input name='break_minutes' type='number' min='0' max='120' value='30' required class='input-field' placeholder='Break minutes'>
<input name='lessons_per_subject' type='number' min='1' max='10' value='3' required class='input-field' placeholder='Lessons per subject / week'>
<label style='display:flex;align-items:center;gap:8px;padding:10px;border:1px solid #e5e7eb;border-radius:10px;background:#fff;font-size:12px'><input type='checkbox' name='replace_existing' value='1' checked style='width:auto'> Replace this class timetable</label>
<button class='btn' style='grid-column:1/-1'>✨ Generate Timetable</button></form></div>

<div class='card no-print'><h3>➕ Add a Single Period</h3>
<form method='post' action='/school/modules/timetable/add' style='display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:8px'>
<select name='day' required class='input-field'><option>Monday</option><option>Tuesday</option><option>Wednesday</option><option>Thursday</option><option>Friday</option></select>
<input name='start_time' required type='time' class='input-field'><input name='end_time' required type='time' class='input-field'>
<select name='class_id' required class='input-field'><option value=''>-- Class / Stream --</option>{class_opts}</select>
<select name='subject_id' required class='input-field'><option value=''>-- Subject --</option>{subject_opts}</select>
<select name='teacher_id' class='input-field'>{teacher_opts}</select>
<input name='room' placeholder='Room / Venue' class='input-field'><button class='btn'>Add Period</button></form></div>

{weekly}

<div class='card'><div class='toolbar'><div><h3 style='margin:0'>📋 All Timetable Entries</h3><p style='margin:4px 0'>Saved permanently for this school.</p></div><form method='get' class='no-print'><select name='class_id' onchange='this.form.submit()' class='input-field' style='min-width:230px'><option value=''>View a class timetable</option>{class_opts}</select></form></div>
<div style='overflow:auto'><table><tr><th>Day</th><th>Time</th><th>Class / Stream</th><th>Subject</th><th>Teacher</th><th>Room</th><th>Action</th></tr>{cell_rows(rows) or "<tr><td colspan='7' style='padding:30px;text-align:center;color:#94a3b8'>No timetable entries yet.</td></tr>"}</table></div></div>"""
    return module_page(request,'🗓️ Smart Timetable','timetable',body)

@app.post("/school/modules/timetable/add")
def add_timetable(request:Request,day:str=Form(...),start_time:str=Form(...),end_time:str=Form(...),class_id:int=Form(...),subject_id:int=Form(...),teacher_id:str=Form(""),room:str=Form("")):
    school=get_school_obj(request)
    if not school or not _role_permission(request,"timetable.edit"): return RedirectResponse("/portal",303)
    con=get_db(); cur=con.cursor()
    cls=cur.execute("SELECT id,name,stream FROM classes WHERE id=? AND school_id=?",(class_id,school["id"])).fetchone()
    sub=cur.execute("SELECT id,name FROM subjects WHERE id=? AND school_id=?",(subject_id,school["id"])).fetchone()
    teacher=cur.execute("SELECT id,name FROM teachers WHERE id=? AND school_id=?",(int(teacher_id),school["id"])).fetchone() if str(teacher_id).isdigit() else None
    if not cls or not sub:
        con.close(); return RedirectResponse("/school/timetable?msg=Invalid+class+or+subject",303)
    if start_time >= end_time:
        con.close(); return RedirectResponse("/school/timetable?msg=End+time+must+be+after+start+time",303)
    class_name,stream=cls["name"],cls["stream"] or ""
    conflict=cur.execute("SELECT id FROM timetable WHERE school_id=? AND day=? AND start_time=? AND end_time=? AND class_name=? AND COALESCE(stream,'')=? LIMIT 1",(school["id"],day,start_time,end_time,class_name,stream)).fetchone()
    teacher_conflict=cur.execute("SELECT id FROM timetable WHERE school_id=? AND day=? AND start_time=? AND end_time=? AND teacher=? LIMIT 1",(school["id"],day,start_time,end_time,teacher["name"] if teacher else "")).fetchone() if teacher else None
    if conflict:
        con.close(); return RedirectResponse("/school/timetable?msg=Class+already+has+a+lesson+at+that+time",303)
    if teacher_conflict:
        con.close(); return RedirectResponse("/school/timetable?msg=Teacher+conflict:+teacher+is+already+scheduled+at+that+time",303)
    cur.execute("INSERT INTO timetable(school_id,day,start_time,end_time,class_name,stream,subject,teacher,room) VALUES(?,?,?,?,?,?,?,?,?)",(school["id"],day,start_time,end_time,class_name,stream,sub["name"],teacher["name"] if teacher else "",room.strip()))
    con.commit(); con.close()
    return RedirectResponse(f"/school/timetable?class_id={class_id}&msg=Period+added+successfully",303)

@app.post("/school/modules/timetable/generate")
def generate_timetable(request:Request,class_id:int=Form(...),start_time:str=Form("08:00"),period_minutes:int=Form(40),periods_per_day:int=Form(7),break_after:int=Form(4),break_minutes:int=Form(30),lessons_per_subject:int=Form(3),replace_existing:str=Form("")):
    school=get_school_obj(request)
    if not school or not _role_permission(request,"timetable.edit"): return RedirectResponse("/portal",303)
    if period_minutes < 20 or periods_per_day < 1 or periods_per_day > 12 or lessons_per_subject < 1:
        return RedirectResponse("/school/timetable?msg=Invalid+timetable+settings",303)
    con=get_db(); cur=con.cursor(); sid_value=school["id"]
    cls=cur.execute("SELECT id,name,stream FROM classes WHERE id=? AND school_id=?",(class_id,sid_value)).fetchone()
    if not cls:
        con.close(); return RedirectResponse("/school/timetable?msg=Class+not+found",303)
    allocations=cur.execute("SELECT ta.subject_id,s.name subject_name,t.name teacher_name FROM teacher_allocations ta JOIN subjects s ON s.id=ta.subject_id LEFT JOIN teachers t ON t.id=ta.teacher_id WHERE ta.school_id=? AND ta.class_id=? ORDER BY ta.subject_id",(sid_value,class_id)).fetchall()
    subjects=allocations or cur.execute("SELECT id,name subject_name,'' teacher_name FROM subjects WHERE school_id=? ORDER BY id",(sid_value,)).fetchall()
    subjects=[dict(x) for x in subjects]
    if not subjects:
        con.close(); return RedirectResponse(f"/school/timetable?class_id={class_id}&msg=Add+subjects+or+teacher+allocations+before+generating",303)
    if replace_existing:
        cur.execute("DELETE FROM timetable WHERE school_id=? AND class_name=? AND COALESCE(stream,'')=?",(sid_value,cls["name"],cls["stream"] or ""))
    ...
    for d in days:
        for p in range(1,periods_per_day+1):
            ...
            slots.append((d,st,et,p))
    occupied=cur.execute("SELECT day,start_time,end_time,teacher FROM timetable WHERE school_id=?",(sid_value,)).fetchall()
    teacher_busy={(r["day"],r["start_time"],r["end_time"],str(r["teacher"] or "").strip().lower()) for r in occupied if str(r["teacher"] or "").strip()}
    class_name,stream=cls["name"],cls["stream"] or ""
    class_busy={(r["day"],r["start_time"],r["end_time"]) for r in cur.execute("SELECT day,start_time,end_time FROM timetable WHERE school_id=? AND class_name=? AND COALESCE(stream,'')=?",(sid_value,class_name,stream)).fetchall()}
    schedule=[]
    remaining=[]
    for subj in subjects:
        for _ in range(lessons_per_subject):
            remaining.append(subj)
    # Spread subjects through the week and avoid repeating the same subject in adjacent slots.
    for idx,subj in enumerate(remaining):
        ordered=slots[idx % len(slots):]+slots[:idx % len(slots)]
        placed=False
        for day,st,et,p in ordered:
            ...
            recent=[x["subject"] for x in schedule[-1:] if x["day"]==day]
            if recent and recent[0]==subj["subject_name"]:
                continue
            schedule.append({"day":day,"start_time":st,"end_time":et,"subject":subj["subject_name"],"teacher":teacher_name})
            class_busy.add((day,st,et))
            if teacher_name: teacher_busy.add((day,st,et,teacher_name.lower()))
            placed=True; break
        if not placed:
            break
    for item in schedule:
        cur.execute("INSERT INTO timetable(school_id,day,start_time,end_time,class_name,stream,subject,teacher,room) VALUES(?,?,?,?,?,?,?,?,?)",(sid_value,item["day"],item["start_time"],item["end_time"],class_name,stream,item["subject"],item["teacher"],""))
    con.commit(); con.close()
    if len(schedule)<len(remaining):
        msg=f"Generated {len(schedule)} of {len(remaining)} lessons; remaining slots were blocked by timetable conflicts or capacity."
    else:
        msg=f"Generated {len(schedule)} lessons for {class_name} {stream}".strip()
    return RedirectResponse(f"/school/timetable?class_id={class_id}&msg={quote(msg)}",303)
