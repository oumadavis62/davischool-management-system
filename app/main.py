...
app = FastAPI()
app.add_middleware(SessionMiddleware, secret_key="davischool-v25-profile-true-copy-final")
...
def profile_page(request: Request, tab: str = "personal"):
    ...
    if tab == "personal":
        ...
        content_html = f"""<div style='background:white; border:1px solid #e2e8f0; border-radius:16px; padding:0; overflow:hidden; max-width:800px'><div style='padding:16px; font-weight:800; border-bottom:1px solid #f1f5f9'>📜 Activity Log - Last 20 actions</div><div style='max-height:60vh; overflow:auto'>{log_rows}</div></div>"""
    return HTMLResponse(f"""<html><head><meta name='viewport' content='width=device-width, initial-scale=1'><style>body{{margin:0;font-family:Arial;background:#f8fafc}}</style></head><body>{header_html(initials, name, email)}<div style='padding:20px; max-width:1100px; margin:auto'><div style='margin-bottom:16px'><h2 style='margin:0; font-size:20px; display:flex; align-items:center; gap:8px'>👤 My Profile</h2></div><div style='background:white; border:1px solid #e2e8f0; border-radius:12px; padding:0 16px; display:flex; gap:20px; margin-bottom:16px'><a href='/profile?tab=personal' style='padding:12px 4px; text-decoration:none; font-size:13px; {active_personal}'>👤 Personal</a><a href='/profile?tab=security' style='padding:12px 4px; text-decoration:none; font-size:13px; {active_security}'>🔒 Security</a><a href='/profile?tab=activity' style='padding:12px 4px; text-decoration:none; font-size:13px; {active_log}'>📜 Activity Log</a></div>{content_html}</div></body></html>""")
...
def manage_schools(request: Request, success: str = "", pending_id: str = "", new_pass: str = "", school_email: str = "", school_name: str = ""):
    ...
    for s in schools:
        ...
        <tr onclick="highlightRow(this, {sid})" style='cursor:pointer; border-bottom:1px solid #f1f5f9; user-select:none; -webkit-user-select:none; caret-color:transparent'>
            ...
            <td style='padding:12px 10px; user-select:none'><span id='actions-{sid}' style='display:none; gap:6px; align-items:center'><a href='/schools/edit/{sid}' style='background:#dbeafe; color:#1e40af; padding:6px 10px; border-radius:6px; text-decoration:none; font-size:11px; font-weight:700; border:1px solid #bfdbfe'>✏️ Edit</a><a href='/schools/delete/{sid}' onclick="return confirm('Delete {s['name']}?')" style='background:#fee2e2; color:#991b1b; padding:6px 10px; border-radius:6px; text-decoration:none; font-size:11px; font-weight:700; border:1px solid #fecaca'>🗑️ Delete</a></span><span id='hint-{sid}' style='font-size:10px; color:#94a3b8'>Click to activate</span></td>
        </tr>"""
    ...
    <style>body{{margin:0;font-family:Arial;background:#f8fafc;caret-color:transparent}}.card{{background:white;border:1px solid #e2e8f0;border-radius:16px;padding:18px}}.highlight{{background:#e0f2fe!important;border-left:4px solid #0ea5e9!important}}input,select{{width:100%;padding:11px 12px;border:1px solid #e2e8f0;border-radius:10px;margin:6px 0;font-size:13px;caret-color:auto}}table{{user-select:none;-webkit-user-select:none}}tr,td,th{{user-select:none;-webkit-user-select:none;outline:none}}::selection{{background:transparent}}input::selection{{background:#bfdbfe}}</style>
    ...
    <div style='display:grid; grid-template-columns:1.7fr 0.7fr; gap:16px; padding:16px; max-width:1500px; margin:auto'>
      <div><div class='card' style='margin-bottom:16px'>{popup_html}<div style='font-weight:800; font-size:15px'>📚 Registered Schools ({len(schools)})</div><div style='font-size:11px; color:#64748b; margin-bottom:8px'>👉 Click row to highlight 💙 → then ✏️ Edit / 🗑️ Delete becomes active (No blinking cursor)</div><div style='overflow:auto; max-height:65vh; border:1px solid #f1f5f9; border-radius:10px'><table style='width:100%; border-collapse:collapse; font-size:13px'><thead style='position:sticky; top:0; background:#f8fafc'><tr style='text-align:left; color:#475569; font-size:11px'><th style='padding:10px'>🏫 School</th><th style='padding:10px'>📞 Contact</th><th style='padding:10px'>📧 Email</th><th style='padding:10px'>📍 Location</th><th style='padding:10px'>👤 Username</th><th style='padding:10px'>🔑 Password</th><th style='padding:10px'>⚙️ Action</th></tr></thead><tbody>{rows_html}</tbody></table></div><a href='/dashboard' style='display:inline-flex; align-items:center; gap:6px; margin-top:14px; padding:10px 16px; background:white; border:1px solid #e2e8f0; border-radius:10px; text-decoration:none; color:#0f172a; font-weight:700; font-size:12px'>⬅️ Back to Dashboard</a></div></div>
      <div class='card' style='height:fit-content; position:sticky; top:16px'><div style='font-weight:800'>➕ Register New School 🏫</div><div style='font-size:11px; color:#64748b; margin-bottom:10px'>Unique password auto-created 🔑</div><form method='post' action='/register-school'><input name='school_name' required placeholder='🏫 School Name *'><input name='school_email' required type='email' placeholder='📧 Admin Email *'><input name='location' required placeholder='📍 Location *'><input name='phone' required placeholder='📱 Phone *'><input name='principal' required placeholder='👤 Principal *'><select name='school_type' required><option value=''>🎓 Type *</option><option>Primary</option><option>Secondary</option><option>Primary & Secondary</option></select><button style='width:100%; background:#0f172a; color:white; padding:12px; border:none; border-radius:10px; margin-top:10px'>📧 Send Code & Create Login</button><a href='/dashboard' style='display:block; text-align:center; padding:10px; border:1px solid #e2e8f0; border-radius:10px; margin-top:8px; text-decoration:none; color:#64748b'>❌ Cancel</a></form></div>
    </div>
