...
.back-btn:hover{{background:#0f172a!important; color:white!important; border-color:#0f172a!important; transform:translateY(-1px); box-shadow:0 6px 16px rgba(15,23,42,0.3)}}
    ...
    <div style='flex:1;background:#f8fafc'><div style='background:white;border-bottom:1px solid #e2e8f0;padding:12px 20px;display:flex;justify-content:space-between;align-items:center'><div><b style='font-size:13px'>DaviSchool Management System 🚀</b><div style='font-size:11px;color:#64748b'>{name.upper()} • {school['name']} | Revolutionize Your School's Management!</div></div><div style='display:flex;align-items:center;gap:10px'><span style='font-size:11px;background:#dbeafe;color:#1e40af;padding:6px 10px;border-radius:20px'>{school['name']}</span><div style='width:32px;height:32px;background:#dcfce7;color:#166534;border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:800;font-size:12px'>{initials}</div></div></div>
    """
...
def profile_page(request: Request, tab: str = "personal"):
    ...
    if tab == "personal":
        ...
        log_rows_html = ""
        for l in logs: log_rows_html += f"""<div style='padding:12px 0; border-bottom:1px solid #f1f5f9; display:flex; justify-content:space-between; gap:12px'><div style='flex:1'><div style='font-weight:700; font-size:12px'>{l['action']}</div><div style='font-size:11px; color:#64748b'>{l['details']}</div></div><div style='font-size:10px; color:#64748b; white-space:nowrap'>{l['timestamp']}</div></div>"""
...
def dashboard(request: Request):
    ...
    name = request.session.get("name","Davis Ouma"); email = request.session.get("email","oumadavis62@gmail.com"); initials = "".join([p[0] for p in name.split()][:2]).upper() if name else "DO"
    rows = ""
    for s in recent: rows += f"<tr><td style='padding:10px 14px; font-size:12px; font-weight:600'>{s['name']}</td><td style='padding:10px 14px; font-size:12px'>{s['location']}</td><td style='padding:10px 14px'><span style='background:#dcfce7;color:#166534;padding:3px 8px;border-radius:12px;font-size:10px'>Active</span></td><td style='padding:10px 14px; font-size:11px; color:#64748b'>Today</td></tr>"
...
def edit_school_save(sid: int, request: Request, school_name: str = Form(...), school_email: str = Form(...), location: str = Form(...), phone: str = Form(...), principal: str = Form(...), new_password: str = Form("")):
    if request.session.get("role")!="super_admin": return RedirectResponse("/")
    con = get_db(); cur = con.cursor(); cur.execute("UPDATE schools SET name=?, email=?, location=?, phone=?, principal=? WHERE id=?", (school_name.strip().upper(), school_email.strip(), location.strip(), phone.strip(), principal.strip(), sid))
...
def school_dashboard(request: Request):
    ...
    <div style='padding:18px; max-width:1400px; margin:auto'>
        ...
        <div style='display:grid; grid-template-columns:repeat(4,1fr); gap:14px; margin-bottom:14px'>
          <a href='/school/students' class='ds-card'><div style='font-size:11px; color:#64748b; font-weight:700'>🎓 TOTAL STUDENTS</div><div style='font-size:30px; font-weight:900; margin:10px 0'>{sc}</div><div style='font-size:11px; color:#16a34a'>Active</div></a>
          <a href='/school/classes' class='ds-card'><div style='font-size:11px; color:#64748b; font-weight:700'>🏫 CLASSES</div><div style='font-size:30px; font-weight:900; margin:10px 0'>{cc}</div><div style='font-size:11px; color:#64748b'>Streams & Levels</div></a>
          <a href='/school/exams' class='ds-card'><div style='font-size:11px; color:#64748b; font-weight:700'>📝 EXAMS</div><div style='font-size:30px; font-weight:900; margin:10px 0'>{ec}</div><div style='font-size:11px; color:#3b82f6'>Deep Insights</div></a>
          <a href='/school/fees' class='ds-card'><div style='font-size:11px; color:#64748b; font-weight:700'>💰 FEES COLLECTED</div><div style='font-size:22px; font-weight:900; margin:10px 0'>KES {fees_total}</div><div style='font-size:11px; color:#16a34a'>Managed</div></a>
        </div>
        <div style='display:grid; grid-template-columns:repeat(4,1fr); gap:14px; margin-bottom:14px'>
          <a href='/school/analysis' class='ds-card'><div style='font-size:22px'>📊</div><div style='font-size:12px; font-weight:800; margin-top:6px'>EXAM MARKS ANALYSIS</div><div style='font-size:10px; color:#64748b'>DEEP PERFORMANCE INSIGHTS</div></a>
          <a href='/school/ranking' class='ds-card'><div style='font-size:22px'>🏆</div><div style='font-size:12px; font-weight:800; margin-top:6px'>ACCURATE RANKING</div><div style='font-size:10px; color:#64748b'>INSTANT CLASS & LEVEL RANKING</div></a>
          <a href='/school/marksheets' class='ds-card'><div style='font-size:22px'>📄</div><div style='font-size:12px; font-weight:800; margin-top:6px'>GENERATES MARKSHEETS</div><div style='font-size:10px; color:#64748b'>CREATE RECORD SHEETS QUICKLY</div></a>
          <a href='/school/reports' class='ds-card'><div style='font-size:22px'>📑</div><div style='font-size:12px; font-weight:800; margin-top:6px'>STUDENT REPORTS</div><div style='font-size:10px; color:#64748b'>AUTOMATED REPORT CARD GENERATION</div></a>
        </div>
        <div style='display:grid; grid-template-columns:repeat(4,1fr); gap:14px; margin-bottom:18px'>
          <a href='/school/marksheets' class='ds-card'><div style='font-size:22px'>📁</div><div style='font-size:12px; font-weight:800; margin-top:6px'>GENERATES MARKSHEETS</div><div style='font-size:10px; color:#64748b'>CREATE ACCURATE RECORD SHEETS</div></a>
          <a href='/school/timetable' class='ds-card'><div style='font-size:22px'>🗓️</div><div style='font-size:12px; font-weight:800; margin-top:6px'>SMART TIMETABLING</div><div style='font-size:10px; color:#64748b'>LESSON ALERTS FOR TEACHERS</div></a>
          <a href='/school/fees' class='ds-card'><div style='font-size:22px'>💰</div><div style='font-size:12px; font-weight:800; margin-top:6px'>FINANCING & FEES</div><div style='font-size:10px; color:#64748b'>MANAGE ACCOUNTS & FEES</div></a>
          <a href='/school/sms' class='ds-card'><div style='font-size:22px'>💬</div><div style='font-size:12px; font-weight:800; margin-top:6px'>BULK SMS TO PARENTS</div><div style='font-size:10px; color:#64748b'>EASY BULK SMS NOTIFICATIONS</div></a>
        </div>
    </div>
    """
...
def add_student(request: Request, assessment_no: str = Form(...), student_name: str = Form(...), class_name: str = Form(...), stream: str = Form(...), gender: str = Form(...), parent_name: str = Form(""), parent_phone: str = Form(...)):
    ...
    school = get_school_obj(request)
    if not school: return RedirectResponse("/")
    con = get_db(); cur = con.cursor(); cur.execute("SELECT id FROM classes WHERE school_id=? AND name=? AND (stream=? OR level=?)", (school["id"], class_name.strip().upper(), stream.strip().upper(), stream.strip().upper())); cls = cur.fetchone()
    ...
    cur.execute("INSERT INTO students (school_id, admission_no, assessment_no, name, class_id, gender, parent_name, parent_phone, stream) VALUES (?,?,?,?,?,?,?,?,?)", (school["id"], assessment_no.strip().upper(), assessment_no.strip().upper(), student_name.strip().upper(), class_id, gender, parent_name.strip().upper(), parent_phone.strip(), stream.strip().upper())); con.commit(); con.close()
...
def edit_student_page(sid: int, request: Request):
    ...
    school = get_school_obj(request)
    if not school: return RedirectResponse("/")
    con = get_db(); cur = con.cursor(); cur.execute("SELECT * FROM students WHERE id=? AND school_id=?", (sid, school["id"],)); st = cur.fetchone(); cur.execute("SELECT * FROM classes WHERE school_id=? ORDER BY name, stream", (school["id"],)); classes = cur.fetchall(); con.close()
    if not st: return RedirectResponse("/school/students")
    name = request.session.get("name",""); header = school_header(school, name, "students"); class_opts = ""
    for cl in classes: sel = "selected" if cl["id"]==st["class_id"] else ""; class_opts += f"<option value='{cl['id']}' {sel}>{cl['name']} - {cl['stream']}</option>"
...
def edit_student_save(sid: int, request: Request, assessment_no: str = Form(...), student_name: str = Form(...), class_id: str = Form(...), gender: str = Form(...), parent_name: str = Form(""), parent_phone: str = Form(...)):
    ...
    school = get_school_obj(request)
    if not school: return RedirectResponse("/")
    con = get_db(); cur = con.cursor(); cur.execute("SELECT stream FROM classes WHERE id=?", (class_id,)); cl = cur.fetchone(); stream_val = cl["stream"] if cl else ""; cur.execute("UPDATE students SET assessment_no=?, admission_no=?, name=?, class_id=?, gender=?, parent_name=?, parent_phone=?, stream=? WHERE id=? AND school_id=?", (assessment_no.strip().upper(), assessment_no.strip().upper(), student_name.strip().upper(), int(class_id), gender, parent_name.strip().upper(), parent_phone.strip(), stream_val, sid, school["id"])); con.commit(); con.close()
    return RedirectResponse(f"/school/students?success=updated&student_name={student_name.strip().upper()}", status_code=303)

# SUBJECTS WITH INITIAL + ACTION
@app.get("/school/subjects", response_class=HTMLResponse)
def school_subjects(request: Request, success: str = "", subject_name: str = ""):
    if "email" not in request.session or request.session.get("role")!="school_admin": return RedirectResponse("/")
    school = get_school_obj(request)
    if not school: return RedirectResponse("/")
    name = request.session.get("name",""); con = get_db(); cur = con.cursor(); cur.execute("SELECT * FROM subjects WHERE school_id=? ORDER BY name", (school["id"],)); subjects = cur.fetchall(); con.close()
    success_html = ""
...
def edit_subject_save(sid: int, request: Request, subject_name: str = Form(...), initial: str = Form(...), code: str = Form(...)):
    ...
    school = get_school_obj(request)
    if not school: return RedirectResponse("/")
    con = get_db(); cur = con.cursor(); cur.execute("UPDATE subjects SET name=?, code=?, initial=? WHERE id=? AND school_id=?", (subject_name.strip().upper(), code.strip().upper(), initial.strip().upper(), sid, school["id"])); con.commit(); con.close()
    return RedirectResponse(f"/school/subjects?success=updated&subject_name={subject_name.strip().upper()}", status_code=303)

# EXAMS WITH EXAM TYPE DROPDOWN
@app.get("/school/exams", response_class=HTMLResponse)
def school_exams(request: Request, success: str = "", exam_name: str = ""):
    if "email" not in request.session or request.session.get("role")!="school_admin": return RedirectResponse("/")
    school = get_school_obj(request)
    if not school: return RedirectResponse("/")
    name = request.session.get("name",""); con = get_db(); cur = con.cursor(); cur.execute("SELECT * FROM exams WHERE school_id=? ORDER BY id DESC", (school["id"],)); exams = cur.fetchall(); con.close()
    success_html = ""
...
def edit_exam_save(eid: int, request: Request, exam_name: str = Form(...), term: str = Form(...), year: str = Form(...), exam_type: str = Form(...)):
    ...
    school = get_school_obj(request)
    if not school: return RedirectResponse("/")
    con = get_db(); cur = con.cursor(); cur.execute("UPDATE exams SET name=?, term=?, year=?, exam_type=? WHERE id=? AND school_id=?", (exam_name.strip().upper(), term.strip(), year.strip(), exam_type.strip(), eid, school["id"])); con.commit(); con.close()
...
def school_generic(page: str, request: Request):
    ...
    school = get_school_obj(request)
    if not school: return RedirectResponse("/")
    name = request.session.get("name",""); header = school_header(school, name, page)
