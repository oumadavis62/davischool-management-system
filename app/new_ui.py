...
function printReportCard(){
  ...
  setTimeout(function(){w.print();},300);
    ...
    print_btn=((
        f"<a class='btn' style='display:inline-block;margin-top:8px;text-decoration:none;background:#176B3A' target='_blank' href='/app/report-cards/pdf?exam_ids={report_pdf_query}&student_id={stid}&inline=1'>🖨️ Print MarkSheet / Report Preview</a> "
        + ("<button class='btn' style='margin-top:8px' onclick='printReportCard()'>Print Report</button> " if report_final else "")
    ...
    overall_comment_editor="<div class='card section no-print'><h2>Performance Comments by Overall Grade</h2><div class='muted'>Set the class teacher and principal performance comment for each overall grade. The matching comments are automatically placed on the student's report card according to the calculated overall grade.</div><table><thead><tr><th>Grade</th><th>Range</th><th>Class Teacher Performance Comment</th><th>Principal Performance Comment</th><th>Action</th></tr></thead><tbody>"+("".join("<tr><td><b>%s</b></td><td>%.1f%% – %.1f%%</td><td colspan='2'><form method='post' action='/app/report-cards/overall-grade-comments'><input type='hidden' name='rule_id' value='%s'><div class='grid'><textarea name='class_teacher_comment' rows='3' class='field' placeholder='Class teacher performance comment'>%s</textarea><textarea name='principal_comment' rows='3' class='field' placeholder='Principal performance comment'>%s</textarea></div><button class='btn' style='margin-top:8px'>Save Performance Comments</button></form></td><td></td></tr>"%(escape(str(r["grade"] or "")),float(r["min_total"] or 0),float(r["max_total"] or 0),r["id"],escape(str(r["class_teacher_comment"] or "")),escape(str(r["principal_comment"] or ""))) for r in overall_rows)) if overall_rows else "<tr><td colspan='5'>No overall grading bands configured. Create the overall grade bands first.</td></tr>")+" </tbody></table><div class='grid' style='margin-top:14px'><div><b>Class Teacher:</b> "+escape(str(class_teacher_name or "Not Assigned"))+"<div style='margin-top:18px;border-bottom:1px solid #172033;width:85%'></div><small>Signature</small></div><div><b>Principal:</b> "+escape(str(principal_name or "Not Assigned"))+"<div style='margin-top:18px;border-bottom:1px solid #172033;width:85%'></div><small>Signature</small></div></div></div>"
    body=f"""<div class='page'><h1>Report Cards</h1><div class='muted'>Select one or more assessments. Multiple selections are averaged subject-by-subject before grade, points, total and overall average are calculated.</div><div class='muted' style='margin-top:4px'>Printable and downloadable documents include the DaviSchool Management System footer and exact generation time.</div><div class='card section no-print'><form method='get' style='display:grid;grid-template-columns:1fr 1fr auto;gap:10px'><select name='exam_ids' class='field' multiple size='3'>{eopts}</select><select name='class_id' class='field'><option value=''>Choose class for bulk report cards</option>{copts}</select><select name='student_id' class='field'><option value=''>Choose individual student (optional)</option>{sopts}</select><button class='btn'>Generate</button></form>{bulk_btn}</div><div class='card section no-print' style='display:flex;gap:8px;flex-wrap:wrap'><a class='btn' href='/app/report-cards?exam_id={eid}&student_id={stid}&tab=report'>📄 Report Card</a><a class='btn' href='/app/report-cards?exam_id={eid}&student_id={stid}&tab=overall-comments'>📝 Overall Grade Comments & Signatures</a><a class='btn' href='/app/report-card-settings?exam_id={eid}'>📅 Report Dates</a></div>{report_html}{teacher_editor}<div class='card section no-print'><h2>Subject Performance Comments</h2><div class='muted'>Enter an individual performance comment for each subject. These comments appear on the printed report card.</div>{subject_editor or '<div class="muted" style="margin-top:10px">Select a student and examination first.</div>'}</div></div><style>.field{{width:100%;padding:11px;border:1px solid #dbe2ea;border-radius:9px}}.btn{{padding:11px 16px;border:0;border-radius:9px;background:#111827;color:#fff}}@media print{{.no-print{{display:none!important}}}}</style>"""
...
def class_marksheets_pdf(request: Request, exam_id: str = "", class_id: str = "", term: str = "", year: str = "", stream: str = ""):
    ...
    try:
        ...
        col_widths=[22*mm,68*mm] + ([18*mm] if combined_mode else []) + [15*mm]*subject_width_count + [12*mm,12*mm,14*mm,12*mm,12*mm]
