...
def davischool_login_page(request: Request):
    ...
    return HTMLResponse(f"""<!doctype html><html><head><meta name='viewport' content='width=device-width,initial-scale=1'><title>DaviSchool Login</title><style>body{{margin:0;background:#eef5fb;font-family:Arial,sans-serif;display:flex;min-height:100vh;align-items:center;justify-content:center}}.box{{width:min(430px,92vw);background:white;border:1px solid #d8e3f0;border-top:4px solid #D4AF37;border-radius:18px;padding:32px;box-shadow:0 18px 50px #7A1F2B20}}.logo{{font-size:25px;font-weight:900;color:#7A1F2B;margin-bottom:5px}}.sub{{color:#64748b;margin-bottom:25px}}label{{display:block;font-size:13px;font-weight:800;color:#334155;margin:14px 0 7px}}input{{width:100%;box-sizing:border-box;padding:13px;border:1px solid #dbe2ea;border-radius:10px;font-size:15px}}button{{width:100%;margin-top:20px;padding:14px;border:0;border-radius:10px;background:#7A1F2B;color:white;font-weight:900;font-size:15px;cursor:pointer}}.err{{background:#fff1f2;border:1px solid #fda4af;color:#9f1239;padding:11px;border-radius:10px;margin-bottom:14px}}</style></head><body><div class='box'><div class='logo'>🏫 DaviSchool Management System</div><div class='sub'>Secure school management platform</div>{error}<form method='post' action='/login'><label>Email / Username</label><input name='email' type='email' autocomplete='username' required placeholder='Enter your email'><label>Password</label><input name='password' type='password' autocomplete='current-password' required placeholder='Enter password'><button type='submit'>Sign In</button></form></div></body></html>""")
...
def _pdf_styles():
    ...
    return {
        "title": ParagraphStyle("DaviTitle", parent=styles["Title"], fontSize=15, leading=18, alignment=TA_CENTER, spaceAfter=5, textColor="#7A1F2B"),
        "subtitle": ParagraphStyle("DaviSubtitle", parent=styles["Normal"], fontSize=8, leading=10, alignment=TA_CENTER, spaceAfter=8, textColor="#7A1F2B"),
...
def _pdf_school_header(school_row, styles, title, subtitle=""):
    ...
    table.setStyle(TableStyle([
        ("VALIGN",(0,0),(-1,-1),"TOP"),
        ("LINEBELOW",(0,0),(-1,-1),1.8,colors.HexColor("#7A1F2B")),
        ("LINEABOVE",(0,0),(-1,-1),1.0,colors.HexColor("#D4AF37")),
...
<style>
*{{box-sizing:border-box}}body{{margin:0;font-family:Inter,Arial,sans-serif;background:#eef5fb;color:#172033;--navy:#7A1F2B;--navy-dark:#082f6b;--gold:#D4AF37;--ink:#172033;--line:#d8e3f0}}
...
.nav{{display:flex;gap:11px;align-items:center;color:#dbeafe;text-decoration:none;padding:10px 12px;border-radius:10px;font-size:13px;margin:3px 0;border-left:3px solid transparent}}.nav:hover{{background:rgba(244,180,0,.16);color:white;border-left-color:var(--gold)}}
...
function printDocument(){
  ...
  var css='*{box-sizing:border-box}body{margin:0;background:#fff;color:#172033;font-family:Arial,sans-serif}.marksheet-card{display:block!important;width:100%!important;margin:0!important;padding:0!important;border:0!important;box-shadow:none!important}.no-print{display:none!important}.doc-header{display:flex;align-items:flex-start;gap:14px;border-top:2px solid #D4AF37;border-bottom:3px solid #7A1F2B;padding:8px 4px 10px;margin-bottom:10px}.doc-logo{width:86px;height:70px;display:flex;align-items:center;justify-content:center;flex:0 0 86px}.doc-logo img{max-width:82px;max-height:66px;object-fit:contain}.doc-school-block{flex:1;min-width:0}.doc-school{font-size:20px;line-height:1.15;font-weight:900;text-transform:uppercase;color:#7A1F2B}.doc-contact{font-size:10px;color:#334155;margin-top:5px;line-height:1.55}.doc-contact div{display:block;margin:1px 0}.doc-right{font-size:10px;color:#7A1F2B;line-height:1.65;text-align:left;min-width:155px}.doc-right div{display:block;margin:1px 0}.marksheet-school{display:none!important}.marksheet-meta{font-size:14px;font-weight:800;padding:8px 4px;border-top:1px solid #7A1F2B;border-bottom:1px solid #7A1F2B}.marksheet{border-collapse:collapse;width:100%;font-family:Arial,sans-serif;table-layout:fixed}.marksheet th,.marksheet td{border:1.25px solid #7A1F2B;padding:5px 6px;text-align:center;font-size:10px;white-space:nowrap}.marksheet th{background:#7A1F2B!important;color:#fff!important;font-weight:900}.marksheet thead tr:nth-child(2) th{background:#D4AF37!important;color:#5A1620!important;font-weight:900}.marksheet tbody td{border-top:1px solid #7A1F2B;border-bottom:1px solid #7A1F2B}.marksheet .adm-no-col{width:78px}.marksheet .name-col{width:190px;min-width:190px;max-width:190px}.marksheet .stream-col,.marksheet .stream-cell{width:70px;min-width:70px;max-width:70px}.marksheet .mks-col,.marksheet .points-col{width:58px;min-width:58px;max-width:58px}.marksheet .grade-col{width:50px;min-width:50px;max-width:50px}.marksheet .overall-marks-col,.marksheet .overall-points-col{width:62px}.marksheet .overall-avg-col{width:68px}.marksheet .overall-grade-col{width:58px}.marksheet .overall-pos-col{width:50px}.marksheet .subjecthead{font-size:11px;color:#fff;text-transform:uppercase}.marksheet .name-head,.marksheet .name-cell{text-align:left;min-width:190px;width:190px;max-width:190px}.marksheet td b{font-weight:800}.print-footer{position:fixed;left:0;right:0;bottom:0;text-align:center;border-top:2px solid #D4AF37;padding-top:4px;font-size:8px;color:#7A1F2B;background:#fff}@page{size:A4 landscape;margin:8mm 8mm 12mm}';
  ...
  setTimeout(function(){w.print();},300);
    ...
    body = (
        ...
        ".marksheet-card{background:#fff;min-width:0;overflow:hidden}.marksheet-scroll{display:block;width:100%;max-width:100%;overflow-x:auto;overflow-y:hidden;-webkit-overflow-scrolling:touch;overscroll-behavior-x:contain;padding-bottom:8px;scrollbar-gutter:stable}.marksheet-scroll:focus{outline:2px solid #94a3b8;outline-offset:2px}.marksheet{width:max-content;min-width:100%}.marksheet-pagination{display:flex;align-items:center;justify-content:center;gap:16px;padding:10px 0}.marksheet-pagination .btnlink:disabled{opacity:.45;cursor:not-allowed}.marksheet thead tr:first-child th{background:#7A1F2B;color:#fff}.marksheet thead tr:nth-child(2) th{background:#D4AF37;color:#5A1620}.marksheet th:nth-child(1),.marksheet td:nth-child(1){position:sticky;left:0;background:#fff;z-index:10}.marksheet th:nth-child(2),.marksheet td:nth-child(2){position:sticky;left:78px;background:#fff;z-index:10}.marksheet thead tr:first-child th:nth-child(1),.marksheet thead tr:first-child th:nth-child(2),.marksheet thead tr:nth-child(2) th:nth-child(1),.marksheet thead tr:nth-child(2) th:nth-child(2){z-index:13}.doc-header{display:flex;align-items:flex-start;gap:14px;border-top:2px solid #D4AF37;border-bottom:3px solid #7A1F2B;padding:8px 4px 10px;margin-bottom:10px}.doc-logo{width:86px;height:70px;display:flex;align-items:center;justify-content:center;flex:0 0 86px}.doc-logo img{max-width:82px;max-height:66px;object-fit:contain}.doc-school-block{flex:1;min-width:0}.doc-school{font-size:20px;line-height:1.15;font-weight:900;text-transform:uppercase;color:#7A1F2B}.doc-contact{font-size:10px;color:#334155;margin-top:5px;line-height:1.55}.doc-contact div{display:block;margin:1px 0}.doc-right{font-size:10px;color:#7A1F2B;line-height:1.65;text-align:left;min-width:155px;padding-top:2px}.doc-right div{display:block;margin:1px 0}.marksheet-title{text-align:center;font-size:24px;font-weight:900;color:#7A1F2B;padding:5px 4px 6px}.marksheet-school{display:none!important}.marksheet-meta{font-size:14px;font-weight:800;padding:8px 4px;border-top:1px solid #111;border-bottom:1px solid #111}.marksheet{border-collapse:collapse;width:max-content;min-width:0;font-family:Arial,sans-serif;table-layout:fixed}.marksheet th,.marksheet td{border:1.25px solid #111;padding:6px 8px;text-align:center;font-size:12px;white-space:nowrap;box-sizing:border-box}.marksheet th{background:#7A1F2B;color:#fff;text-transform:none;font-weight:900}.marksheet thead tr:nth-child(2) th{background:#D4AF37;color:#5A1620;font-weight:900}.marksheet tbody td{border-top:1px solid #111;border-bottom:1px solid #111}.marksheet .adm-no-col{width:78px;min-width:78px;max-width:78px}.marksheet .name-col{width:190px;min-width:190px;max-width:190px}.marksheet .stream-col,.marksheet .stream-cell{width:70px;min-width:70px;max-width:70px}.marksheet .mks-col,.marksheet .points-col{width:58px;min-width:58px;max-width:58px}.marksheet .grade-col{width:50px;min-width:50px;max-width:50px}.marksheet .overall-marks-col,.marksheet .overall-points-col{width:62px}.marksheet .overall-avg-col{width:68px}.marksheet .overall-grade-col{width:58px}.marksheet .overall-pos-col{width:50px}.marksheet .mks-cell,.marksheet .points-cell{vertical-align:middle;width:58px;min-width:58px;max-width:58px}.marksheet .grade-cell{vertical-align:middle;width:50px;min-width:50px;max-width:50px}.marksheet .subjecthead{font-size:13px;color:#fff;text-transform:uppercase;white-space:nowrap;overflow:hidden;max-width:166px}.marksheet .name-head,.marksheet .name-cell{text-align:left;min-width:190px;width:190px;max-width:190px}.marksheet td b{font-weight:800}.subject-mean-summary{margin-top:12px;border:1px solid #111827;padding:9px;background:#fff}.subject-mean-title{font-size:12px;font-weight:900;text-align:center;border-bottom:1px solid #111827;padding-bottom:5px;margin-bottom:7px}.subject-mean-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:6px}.subject-summary{border-collapse:collapse;width:100%;table-layout:fixed}.subject-summary th,.subject-summary td{border:1px solid #111;padding:8px 12px;text-align:center}.subject-summary th{font-weight:900}.subject-summary th:first-child,.subject-summary td:first-child{text-align:left}.subject-mean-summary{page-break-before:always;break-before:page;margin-top:18px;padding-top:12px}.subject-mean-item{border:1px solid #cbd5e1;padding:6px;text-align:center}.subject-mean-item span{display:block;font-size:10px;font-weight:800;text-transform:uppercase}.subject-mean-item b{display:block;font-size:14px;margin:2px 0}.subject-mean-item small{font-size:8px;color:#64748b}.subject-mean-empty{font-size:10px;color:#64748b;text-align:center;padding:5px}"
...
.blank-marksheet-card{{background:#fff;overflow:hidden}}.blank-doc-head{{display:flex;align-items:flex-start;gap:14px;border-top:2px solid #D4AF37;border-bottom:3px solid #7A1F2B;padding:8px 4px 10px}}.blank-brand-left{{display:flex;align-items:flex-start;gap:12px;flex:1;min-width:0}}.blank-doc-logo{{width:86px;height:70px;display:flex;align-items:center;justify-content:center;flex:0 0 86px}}.blank-doc-logo img{{max-width:82px;max-height:66px;object-fit:contain}}.blank-school{{font-size:20px;line-height:1.15;font-weight:900;text-transform:uppercase;color:#7A1F2B}}.blank-contact{{font-size:10px;color:#334155;margin-top:5px;line-height:1.55}}.blank-contact div{{display:block;margin:1px 0}}.blank-doc-right{{font-size:10px;color:#7A1F2B;line-height:1.65;min-width:155px}}.blank-doc-right div{{display:block;margin:1px 0}}.blank-title{{font-size:16px;font-weight:900;text-align:right;color:#7A1F2B}}.blank-meta{{font-size:11px;font-weight:800;padding:8px 4px;border-bottom:1px solid #111827}}.blank-scroll{{overflow-x:auto;padding-bottom:8px}}.blank-marksheet{{border-collapse:collapse;width:max-content;min-width:100%;font-family:Arial,sans-serif}}.blank-marksheet th,.blank-marksheet td{{border:1px solid #111;padding:7px 8px;text-align:center;font-size:10px;white-space:nowrap;height:28px}}.blank-marksheet th{{background:#7A1F2B;color:#fff;font-weight:900}}.blank-marksheet th:first-child,.blank-marksheet td:first-child{{width:58px;min-width:58px}}.blank-marksheet th:nth-child(2),.blank-marksheet td:nth-child(2){{width:125px;min-width:125px;max-width:125px;text-align:left}}.blank-marksheet th:not(:first-child):not(:nth-child(2)){{min-width:52px;width:52px}}.blank-marksheet .blank-sub{{font-size:8px}}.blank-marksheet .blank-cell{{height:30px;min-width:72px}}.blank-note{{font-size:9px;color:#64748b;margin-top:8px}}
@media(max-width:900px){{.blank-controls{{grid-template-columns:1fr 1fr}}}}
@media print{{@page{{size:A4 landscape;margin:8mm}}body{{background:#fff;color:#172033}}.side,.top,.no-print,.page>h1,.page>.muted{{display:none!important}}.main{{margin-left:0!important;padding:0!important}}.page{{padding:0!important;margin:0!important;max-width:none!important}}.blank-marksheet-card{{border:0!important;box-shadow:none!important;margin:0!important;padding:0!important}}.blank-scroll{{overflow:visible!important}}.blank-marksheet{{width:100%!important}}.blank-marksheet th,.blank-marksheet td{{font-size:8px;padding:5px;border-color:#7A1F2B}}.blank-marksheet th{{background:#7A1F2B;color:#fff}}.blank-school{{font-size:16px}}.blank-title{{font-size:13px}}}}
</style>"""
...
function printReportCard(){
  ...
  var css='*{box-sizing:border-box}body{margin:0;background:#fff;color:#172033;font-family:Arial,sans-serif}.report-document{display:block!important;width:100%!important;margin:0!important;padding:0!important;border:0!important;box-shadow:none!important}.no-print{display:none!important}.doc-header{display:flex;align-items:flex-start;gap:14px;border-top:2px solid #D4AF37;border-bottom:3px solid #7A1F2B;padding:8px 4px 10px;margin-bottom:10px}.doc-logo{width:86px;height:70px;display:flex;align-items:center;justify-content:center;flex:0 0 86px}.doc-logo img{max-width:82px;max-height:66px;object-fit:contain}.doc-school-block{flex:1;min-width:0}.doc-school{font-size:20px;line-height:1.15;font-weight:900;text-transform:uppercase;color:#7A1F2B}.doc-contact{font-size:10px;color:#334155;margin-top:5px;line-height:1.55}.doc-contact div{display:block;margin:1px 0}.doc-right{font-size:10px;color:#7A1F2B;line-height:1.65;text-align:left;min-width:155px}.doc-right div{display:block;margin:1px 0}.marksheet-school{display:none!important}table{width:100%;border-collapse:collapse}th{background:#7A1F2B!important;color:#fff!important}th,td{border:1px solid #7A1F2B;padding:6px;font-size:10px;text-align:left}.kpi{font-size:18px;font-weight:900}.card{border:0;box-shadow:none}.report-comment-form{display:none}.print-footer{position:fixed;left:0;right:0;bottom:0;text-align:center;border-top:2px solid #D4AF37;padding-top:4px;font-size:8px;color:#7A1F2B;background:#fff}@page{size:A4;margin:10mm 10mm 15mm}';
  var footer='<div class="print-footer"><i>DaviSchool Management System</i> · Generated: '+generatedAt+'</div>';
  ...
  setTimeout(function(){w.print();},300);
    ...
    print_btn=(("<button class='btn' style='margin-top:8px' onclick='printReportCard()'>Print Report</button> " if report_final else "") + "<a class='btn' style='display:inline-block;margin-top:8px;text-decoration:none' href='/app/report-cards/pdf?exam_ids={quote(",".join(str(x) for x in selected_exam_ids))}&student_id={stid}'>⬇️ Download PDF</a>") if st and eid else ""
...
<style>.timetable-doc-header{{display:flex;align-items:flex-start;gap:14px;border-top:2px solid #D4AF37;border-bottom:3px solid #7A1F2B;padding:8px 4px 10px}}.timetable-logo{{width:86px;height:70px;display:flex;align-items:center;justify-content:center;flex:0 0 86px}}.timetable-logo img{{max-width:82px;max-height:66px;object-fit:contain}}.timetable-school{{flex:1;min-width:0;color:#7A1F2B;font-weight:900;text-transform:uppercase;font-size:20px;line-height:1.15}}.timetable-school small{{display:block;color:#334155;font-weight:500;font-size:10px;line-height:1.55;margin:1px 0}}.timetable-right{{color:#7A1F2B;font-size:10px;line-height:1.65;min-width:155px;padding-top:2px}}.timetable-right small{{display:block;margin:1px 0}}.timetable-title{{text-align:center;color:#7A1F2B;font-size:15px;font-weight:900;padding:8px}}.week td,.week th{{border:1px solid #7A1F2B;padding:12px;vertical-align:top}}.week th{{background:#7A1F2B;color:#fff;font-size:11px}}.week td{{min-width:150px;font-size:12px;line-height:1.45}}.mini{{border:0;padding:6px 8px;border-radius:7px;cursor:pointer}}.danger{{background:#fee2e2;color:#991b1b}}@media print{{@page{{size:A4 landscape;margin:8mm}}.side,.top,.page>h1,.page>.muted,.section:not(:has(.week)),button,form{{display:none!important}}.page{{padding:0!important}}.card{{border:0!important;box-shadow:none!important}}.timetable-print-card{{display:block!important}}.timetable-doc-header{{margin-top:0}}}}</style></div>"""
...
def class_marksheets_pdf(request: Request, exam_id: str = "", class_id: str = "", term: str = "", year: str = "", stream: str = ""):
    ...
    try:
        ...
        table.setStyle(TableStyle([
            ("GRID",(0,0),(-1,-1),0.65,colors.black),("LINEABOVE",(0,2),(-1,-1),0.45,colors.black),("LINEBELOW",(0,2),(-1,-1),0.45,colors.black),("BACKGROUND",(0,0),(-1,1),colors.HexColor("#eef2f7")),
            ("FONTNAME",(0,0),(-1,1),"Helvetica-Bold"),("FONTSIZE",(0,0),(-1,-1),6),
            ("ALIGN",(0,0),(-1,-1),"CENTER"),("VALIGN",(0,0),(-1,-1),"MIDDLE"),
            ("ALIGN",(1,2),(1,-1),"LEFT"),("FONTSIZE",(1,2),(1,-1),7)
        ]))
        story += [table, PageBreak(), Paragraph("Subject Means",styles["title"]), Spacer(1,6)]
        ...
        mean_table=Table(mean_data,colWidths=[90*mm,35*mm,35*mm,35*mm],repeatRows=1)
        mean_table.setStyle(TableStyle([("GRID",(0,0),(-1,-1),0.65,colors.black),("BACKGROUND",(0,0),(-1,0),colors.HexColor("#7A1F2B")),("TEXTCOLOR",(0,0),(-1,0),colors.white),("FONTNAME",(0,0),(-1,0),"Helvetica-Bold"),("FONTSIZE",(0,0),(-1,-1),9),("TOPPADDING",(0,0),(-1,-1),7),("BOTTOMPADDING",(0,0),(-1,-1),7),("LEFTPADDING",(0,0),(-1,-1),10),("RIGHTPADDING",(0,0),(-1,-1),10),("ALIGN",(1,1),(-1,-1),"CENTER"),("ALIGN",(0,1),(0,-1),"LEFT")]))
...
def class_analysis_pdf(request: Request, exam_id: str = "", exam_ids: str = "", class_id: str = ""):
    ...
    try:
        ...
        t=Table(data,colWidths=[70*mm,25*mm,30*mm,30*mm,30*mm],repeatRows=1);t.setStyle(TableStyle([("GRID",(0,0),(-1,-1),.4,colors.black),("BACKGROUND",(0,0),(-1,0),colors.HexColor("#7A1F2B")),("TEXTCOLOR",(0,0),(-1,0),colors.white),("FONTNAME",(0,0),(-1,0),"Helvetica-Bold"),("FONTSIZE",(0,0),(-1,-1),8)]));story += [t,Spacer(1,8),Paragraph("Learner Ranking",styles["normal"])]
        ...
        t=Table(data,colWidths=[20*mm,30*mm,65*mm,25*mm,30*mm,25*mm],repeatRows=1);t.setStyle(TableStyle([("GRID",(0,0),(-1,-1),.4,colors.black),("BACKGROUND",(0,0),(-1,0),colors.HexColor("#7A1F2B")),("TEXTCOLOR",(0,0),(-1,0),colors.white),("FONTNAME",(0,0),(-1,0),"Helvetica-Bold"),("FONTSIZE",(0,0),(-1,-1),8)]));story.append(t)
...
def student_analysis_pdf(request: Request, exam_id: str = "", exam_ids: str = "", student_id: str = ""):
    ...
    try:
        ...
        t=Table(data,colWidths=[85*mm,30*mm,30*mm,30*mm],repeatRows=1);t.setStyle(TableStyle([("GRID",(0,0),(-1,-1),.4,colors.black),("BACKGROUND",(0,0),(-1,0),colors.HexColor("#7A1F2B")),("TEXTCOLOR",(0,0),(-1,0),colors.white),("FONTNAME",(0,0),(-1,0),"Helvetica-Bold"),("FONTSIZE",(0,0),(-1,-1),9)]));story.append(t)
...
def report_cards_class_pdf(request: Request, exam_id: str = "", exam_ids: str = "", class_id: str = "", inline: str = ""):
    try:
        ...
        for student_index,st in enumerate(students):
            ...
            t.setStyle(TableStyle([("GRID",(0,0),(-1,-1),.4,colors.black),("BACKGROUND",(0,0),(-1,0),colors.HexColor("#7A1F2B")),("TEXTCOLOR",(0,0),(-1,0),colors.white),("FONTNAME",(0,0),(-1,0),"Helvetica-Bold"),("FONTSIZE",(0,0),(-1,-1),7),("VALIGN",(0,0),(-1,-1),"TOP")]))
...
def report_card_pdf(request: Request, exam_id: str = "", exam_ids: str = "", student_id: str = ""):
    ...
    try:
        ...
        if len(data)==1:data.append(["No marks recorded.","","","",""])
        t=Table(data,colWidths=[35*mm,18*mm,20*mm,20*mm,80*mm],repeatRows=1);t.setStyle(TableStyle([("GRID",(0,0),(-1,-1),.4,colors.black),("BACKGROUND",(0,0),(-1,0),colors.HexColor("#7A1F2B")),("TEXTCOLOR",(0,0),(-1,0),colors.white),("FONTNAME",(0,0),(-1,0),"Helvetica-Bold"),("FONTSIZE",(0,0),(-1,-1),7),("VALIGN",(0,0),(-1,-1),"TOP")]))
