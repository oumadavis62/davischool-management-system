...
function printReportCard(){
  ...
  setTimeout(function(){w.print();},300);
    ...
    print_btn=((
        "<a class='btn' style='display:inline-block;margin-top:8px;text-decoration:none;background:#176B3A' target='_blank' href='/app/report-cards/pdf?exam_ids={quote(",".join(str(x) for x in selected_exam_ids))}&student_id={stid}&inline=1'>🖨️ Print MarkSheet / Report Preview</a> "
        + ("<button class='btn' style='margin-top:8px' onclick='printReportCard()'>Print Report</button> " if report_final else "")
        + "<a class='btn' style='display:inline-block;margin-top:8px;text-decoration:none' href='/app/report-cards/pdf?exam_ids={quote(",".join(str(x) for x in selected_exam_ids))}&student_id={stid}'>⬇️ Download PDF</a>"
    ) if st and eid else ""
