...
function printReportCard(){
  ...
  w.document.close();w.focus();setTimeout(function(){w.print();},300);
    ...
    print_btn="<button class='btn' style='margin-top:8px' onclick='printReportCard()'>Print Report</button>" if report_final else ""
...
def class_marksheets_pdf(request: Request, exam_id: str = "", class_id: str = "", term: str = "", year: str = "", stream: str = ""):
    ...
    for st in students:
        ...
        computed.append((st,total,points,count,vals))
    computed.sort(key=lambda x:x[1],reverse=True)
    school=cur.execute("SELECT * FROM schools WHERE id=?",(sid,)).fetchone()
    con.close()
    styles=_pdf_styles()
    ...
    for idx,(st,total,points,count,vals) in enumerate(computed,1):
        if last_total is None or total!=last_total: pos=idx; last_total=total
        avg=(total/count) if count else 0
        data.append([str(idx),str(st["admission_no"] or ""),str(st["name"] or "")]+vals+[f"{total:.1f}",f"{points:.1f}",f"{avg:.1f}",str(_overall_grade(cur,sid,total) if count else "—"),str(pos)])
    if len(data)==2:
