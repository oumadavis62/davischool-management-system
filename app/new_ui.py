...
def timetable_page(request: Request):
    ...
    school=cur.execute("SELECT * FROM schools WHERE id=?",(sid,)).fetchone()
    con.close()
    periods_per_day=int(settings["periods_per_day"] if settings else 7)
    period_minutes=int(settings["period_minutes"] if settings else 40)
    periods_per_week=int(settings["periods_per_week"] if settings else 35)
    school_name=escape(str(school["name"] or "DaviSchool")) if school else "DaviSchool"
    ...
    period_map={int(p["period_no"]):p for p in periods}
    period_tabs=[]
    for n in range(1,periods_per_day+1):
        p=period_map.get(n)
        st=str(p["start_time"]) if p else ""
        et=str(p["end_time"]) if p else ""
        active=" active" if n==1 else ""
        period_tabs.append(f"<button type='button' class='period-tab{active}' onclick='showPeriod({n})'>Period {n}</button><div id='period-panel-{n}' class='period-panel{active}'><h3>Period {n}</h3><div class='muted'>Set the exact start and end time for Period {n}.</div><div class='period-time-grid'><label>Start time<input name='start_{n}' type='time' value='{escape(st)}' required class='field'></label><label>End time<input name='end_{n}' type='time' value='{escape(et)}' required class='field'></label></div></div>")
    ...
    slot_keys=sorted({(r["start_time"],r["end_time"]) for r in selected_rows} | {(b["start_time"],b["end_time"]) for b in breaks},key=lambda x:x[0])
    ...
    for day in ("Monday","Tuesday","Wednesday","Thursday","Friday"):
        ...
        for st,et in slot_keys:
            ...
            if br: cells.append("<td class='break-cell'><b>☕ "+escape(str(br["name"]))+"</b></td>")
            else:
    ...
    table_rows="".join(f"<tr><td>{escape(str(r['day']))}</td><td>{escape(str(r['start_time']))}–{escape(str(r['end_time']))}</td><td>{escape(str(r['class_name']))} {escape(str(r['stream'] or ''))}</td><td><b>{escape(str(r['subject']))}</b></td><td>{escape(str(r['teacher'] or ''))}</td><td>{escape(str(r['room'] or ''))}</td><td><form method='post' action='/app/timetable/delete/{r['id']}' onsubmit='return confirm(\"Delete this lesson?\")'><button class='mini danger'>🗑️</button></form></td></tr>" for r in rows) or "<tr><td colspan='7' style='padding:30px;text-align:center'>No timetable entries yet.</td></tr>"

    body=f"""<div class='page'><h1>🗓️ Smart Timetable</h1><div class='muted'>Set your school's periods once, define breaks, then generate the weekly timetable automatically.</div>
{("<div class='card' style='background:#ecfdf5;border-color:#a7f3d0;color:#065f46'>✅ "+escape(msg)+"</div>") if msg else ""}
<div class='card section'><h2>⚙️ Timetable Settings</h2><div class='muted'>Set periods per day, the default length of one period, and the total periods required each week.</div>
<form method='post' action='/app/timetable/settings/save' class='settings-grid'><label>Periods per day<input name='periods_per_day' type='number' min='1' max='12' value='{periods_per_day}' required class='field'></label><label>Time of one period (minutes)<input name='period_minutes' type='number' min='20' max='180' value='{period_minutes}' required class='field'></label><label>No. of periods per week<input name='periods_per_week' type='number' min='1' max='60' value='{periods_per_week}' required class='field'></label><button class='btn'>💾 Save Timetable Settings</button></form></div>
<div class='card section'><h2>🕐 Period Setup</h2><div class='muted'>Open each tab to set Period 1, Period 2, Period 3 and their exact start/end times.</div><form method='post' action='/app/timetable/periods/save'><div class='period-tabs'>{''.join(period_tabs)}</div><div style='margin-top:14px'><button class='btn'>💾 Save All Period Times</button></div></form></div>
<div class='card section'><h2>☕ Break Periods</h2><div class='muted'>Saved breaks are automatically excluded by the generator and displayed by name in the timetable.</div><form method='post' action='/app/timetable/breaks/save' style='display:grid;grid-template-columns:2fr 1fr 1fr auto;gap:10px'><input name='name' required maxlength='80' placeholder='Break name' class='field'><input name='start_time' required type='time' class='field'><input name='end_time' required type='time' class='field'><button class='btn'>💾 Save Break</button></form><div style='overflow:auto;margin-top:10px'><table><thead><tr><th>Name</th><th>Start</th><th>End</th><th>Action</th></tr></thead><tbody>{break_rows_html}</tbody></table></div></div>
<div class='card section'><h2>🚀 Generate Weekly Timetable</h2><div class='muted'>Select a class/stream. DaviSchool will use your saved periods, skip saved breaks, check teacher conflicts and distribute the configured weekly periods across the class subjects/teacher allocations.</div><form method='post' action='/app/timetable/generate' style='display:grid;grid-template-columns:1fr auto auto;gap:10px;margin-top:12px'><select name='class_id' required class='field'><option value=''>-- Select Class / Stream --</option>{co}</select><label class='field' style='display:flex;align-items:center;gap:8px'><input type='checkbox' name='replace_existing' value='1' checked style='width:auto'> Replace existing timetable</label><button class='btn'>🚀 Generate Timetable</button></form></div>
...
<div class='card section'><div style='display:flex;justify-content:space-between;align-items:center;gap:10px'><div><h2>📋 Saved Lessons ({len(rows)})</h2><div class='muted'>Generated lessons are retained until you replace or delete them.</div></div><form method='get'><select name='class_id' onchange='this.form.submit()' class='field' style='min-width:230px'><option value=''>View class timetable</option>{co}</select></form></div><div style='overflow:auto'><table><thead><tr><th>Day</th><th>Time</th><th>Class / Stream</th><th>Subject</th><th>Teacher</th><th>Room</th><th>Action</th></tr></thead><tbody>{table_rows}</tbody></table></div></div>
<style>.settings-grid{{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px;margin-top:12px}}.settings-grid label,.period-time-grid label{{font-size:11px;font-weight:800;color:#475569}}.period-tabs{{display:flex;gap:6px;overflow:auto;padding-bottom:10px;border-bottom:1px solid #e2e8f0}}.period-tab{{border:1px solid #cbd5e1;background:#f8fafc;color:#334155;padding:9px 13px;border-radius:9px;cursor:pointer;white-space:nowrap}}.period-tab.active{{background:#176B3A;color:#fff;border-color:#176B3A}}.period-panel{{display:none;padding-top:14px}}.period-panel.active{{display:block}}.period-time-grid{{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:10px}}.timetable-doc-header{{display:flex;align-items:flex-start;gap:14px;border-top:2px solid #2E8B57;border-bottom:3px solid #176B3A;padding:8px 4px 10px}}.timetable-logo{{width:86px;height:70px;display:flex;align-items:center;justify-content:center;flex:0 0 86px}}.timetable-logo img{{max-width:82px;max-height:66px;object-fit:contain}}.timetable-school{{flex:1;min-width:0;color:#176B3A;font-weight:900;text-transform:uppercase;font-size:20px;line-height:1.15}}.timetable-school small{{display:block;color:#334155;font-weight:500;font-size:10px;line-height:1.55;margin:1px 0}}.timetable-right{{color:#176B3A;font-size:10px;line-height:1.65;min-width:155px;padding-top:2px}}.timetable-right small{{display:block;margin:1px 0}}.timetable-title{{text-align:center;color:#176B3A;font-size:15px;font-weight:900;padding:8px}}.week td,.week th{{border:1px solid #176B3A;padding:10px;vertical-align:top}}.week th{{background:#176B3A;color:#fff;font-size:11px;white-space:nowrap}}.week td{{min-width:125px;font-size:12px;line-height:1.45}}.week .day-cell{{min-width:90px}}.week .break-cell{{background:#fff7ed;color:#9a3412;text-align:center;font-weight:800}}.mini{{border:0;padding:6px 8px;border-radius:7px;cursor:pointer}}.danger{{background:#fee2e2;color:#991b1b}}@media(max-width:800px){{.settings-grid{{grid-template-columns:1fr 1fr}}}}@media print{{@page{{size:A4 landscape;margin:8mm}}.side,.top,.page>h1,.page>.muted,.section:not(:has(.week)),button,form{{display:none!important}}.page{{padding:0!important}}.card{{border:0!important;box-shadow:none!important}}.timetable-print-card{{display:block!important}}.timetable-doc-header{{margin-top:0}}}}</style>
<script>
function showPeriod(n){{document.querySelectorAll('.period-tab').forEach(function(x){{x.classList.remove('active')}});document.querySelectorAll('.period-panel').forEach(function(x){{x.classList.remove('active')}});var tab=document.querySelectorAll('.period-tab')[n-1];if(tab)tab.classList.add('active');var panel=document.getElementById('period-panel-'+n);if(panel)panel.classList.add('active');}}
</script></div>"""
    return _school_page(request,"Timetable",body)

@router.post("/app/timetable/settings/save")
def timetable_settings_save(request: Request,periods_per_day:int=Form(...),period_minutes:int=Form(...),periods_per_week:int=Form(...)):
    sid=_school_session(request)
    if not sid:return RedirectResponse("/",303)
    if not _require_permission(request,sid,"timetable.edit"):
        return HTMLResponse("You do not have permission to edit timetable settings.",403)
    if periods_per_day<1 or periods_per_day>12 or period_minutes<20 or period_minutes>180 or periods_per_week<1 or periods_per_week>60:
    ...
    existing=cur.execute("SELECT period_no,start_time,end_time FROM timetable_periods WHERE school_id=? ORDER BY period_no",(sid,)).fetchall()
    by_no={int(x["period_no"]):x for x in existing}
    ...
    con.commit();con.close()
    return RedirectResponse("/app/timetable?msg=Timetable+settings+saved",303)
...
async def timetable_periods_save(request: Request):
    ...
    if not sid:return RedirectResponse("/",303)
    if not _require_permission(request,sid,"timetable.edit"):
        return HTMLResponse("You do not have permission to edit timetable periods.",403)
    form=await request.form()
    con=_db();cur=con.cursor()
    settings=cur.execute("SELECT periods_per_day FROM timetable_settings WHERE school_id=?",(sid,)).fetchone()
    count=int(settings["periods_per_day"] if settings else 7)
    for n in range(1,count+1):
        st=str(form.get(f"start_{n}") or "").strip()
        et=str(form.get(f"end_{n}") or "").strip()
        if not st or not et or et<=st:
            con.close();return RedirectResponse(f"/app/timetable?msg=Invalid+time+for+Period+{n}",303)
        overlap=cur.execute("SELECT period_no FROM timetable_periods WHERE school_id=? AND period_no<>? AND start_time<? AND end_time>? LIMIT 1",(sid,n,et,st)).fetchone()
        if overlap:
            con.close();return RedirectResponse(f"/app/timetable?msg=Period+{n}+overlaps+another+period",303)
        cur.execute("UPDATE timetable_periods SET start_time=?,end_time=? WHERE school_id=? AND period_no=?",(st,et,sid,n))
    con.commit();con.close()
    return RedirectResponse("/app/timetable?msg=Period+times+saved",303)
...
def timetable_generate(request: Request,class_id:int=Form(...),replace_existing:str=Form("")):
    sid=_school_session(request)
    if not sid:return RedirectResponse("/",303)
    if not _require_permission(request,sid,"timetable.edit"):
        return HTMLResponse("You do not have permission to edit the timetable.",403)
    ...
    periods=cur.execute("SELECT * FROM timetable_periods WHERE school_id=? ORDER BY period_no",(sid,)).fetchall()
    breaks=cur.execute("SELECT * FROM timetable_breaks WHERE school_id=? ORDER BY start_time,id",(sid,)).fetchall()
    if not settings or not periods:
        con.close();return RedirectResponse(f"/app/timetable?class_id={class_id}&msg=Set+and+save+your+periods+before+generating",303)
    cls=cur.execute("SELECT id,name,stream FROM classes WHERE id=? AND school_id=?",(class_id,sid)).fetchone()
    if not cls:
    ...
    break_ranges=[(str(b["start_time"]),str(b["end_time"])) for b in breaks]
    slots=[]
    for day in days:
        for p in periods:
            st=str(p["start_time"]);et=str(p["end_time"])
            if any(st<be and et>bs for bs,be in break_ranges): continue
            slots.append((day,st,et,int(p["period_no"])))
    max_weekly=min(int(settings["periods_per_week"]),len(slots))
    if max_weekly<1:
        con.close();return RedirectResponse(f"/app/timetable?class_id={class_id}&msg=No+teaching+periods+remain+after+breaks",303)
    ...
    target_counts=[max_weekly//len(subjects) for _ in subjects]
    for i in range(max_weekly%len(subjects)): target_counts[i]+=1
    remaining=[]
    for subj,count in zip(subjects,target_counts):
        for _ in range(count): remaining.append(subj)
    schedule=[];class_busy=set();last_subject_by_day={}
    for subj in remaining:
        placed=False
        for day,st,et,pno in slots:
            if (day,st,et) in class_busy: continue
            if last_subject_by_day.get(day)==subj["subject_name"]: continue
            teacher_name=str(subj.get("teacher_name") or "").strip()
            if teacher_name and (day,st,et,teacher_name.lower()) in teacher_busy: continue
            schedule.append({"day":day,"start_time":st,"end_time":et,"subject":subj["subject_name"],"teacher":teacher_name,"period_no":pno})
            class_busy.add((day,st,et));last_subject_by_day[day]=subj["subject_name"]
    ...
    con.commit();con.close()
    msg=f"Generated {len(schedule)} lessons using {len(periods)} configured periods; {len(breaks)} saved breaks applied"
